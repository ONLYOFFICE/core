  ...
  //          m_pFont->StringGID = 1;
	...
	HRESULT CASCSVGWriter::SaveFile(const std::wstring& strFileSave)
	{
		...
		if(oFile.CreateFileW(strFileSave))
		{
			...
			oFile.WriteStringUTF8(std::wstring(strStart.GetString()));

			CString s = m_pVectorWriter->m_oSVG.m_oDocument.GetCString();
			oFile.WriteStringUTF8(std::wstring(s.GetString()));

			CString strEnd = _T("</svg>");
			oFile.WriteStringUTF8(std::wstring(strEnd.GetString()));
		}
		oFile.CloseFile();
		return S_OK;
	}
