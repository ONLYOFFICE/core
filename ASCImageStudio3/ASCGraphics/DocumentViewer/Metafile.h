...
__interface IAVSMetafile : IAVSRenderer
{
	...
	[id(20004), propget] HRESULT Hypelinks([out, retval] BSTR* bsLinks);
...
public:
	...
	void DrawOnRenderer(IAVSRenderer* pRenderer, BSTR strFile, double dX, double dY, double dW, double dH, BOOL bIsFromFileUse = TRUE)
	{
		if (NULL == pRenderer)
			return;
		...
		IAVSGraphicsRenderer* pGR = NULL;
		pRenderer->QueryInterface(__uuidof(IAVSGraphicsRenderer), (void**)&pGR);
...
private:
	...
	IAVSMetafile* m_pAdditionalCommands;
...
public:
	BOOL Draw(IAVSRenderer* pRenderer, BOOL* pBreak)
	{
		if (NULL == pRenderer)
			return FALSE;
	...
	BOOL Draw2(IAVSRenderer* pRenderer, BOOL* pBreak)
	{
		if (NULL == pRenderer)
			return FALSE;
		...
		IAVSRenderer2* pRenderer2 = NULL;
		pRenderer->QueryInterface(__uuidof(IAVSRenderer2), (void**)&pRenderer2);
	...
	void SetAdditionalCommands(BSTR bsXML)
	{
		CTemporaryCS oCS(&m_oCS);

		RELEASEOBJECT(m_pAdditionalCommands);

		if ((NULL != bsXML) && (_T("") != (CString)bsXML))
		{
			GenerateMetafile(bsXML);
		}
	}
	void GenerateMetafile(BSTR bsXML);
};