...
[object, uuid("A7273F50-3DF1-4b63-ADFA-C643C1E16EC6"), dual, pointer_default(unique)]
__interface IAVSDocumentRenderer : IAVSRenderer
{
...
[dispinterface, uuid("AD0F631B-3CED-427d-8621-D4B2AB919C98")]
__interface _IAVSDocumentRendererEvents
{
	[id(100)] HRESULT OnNewPage(double dWidthMm, double dHeightMm);
	[id(101)] HRESULT OnCompletePage();
};

[coclass, uuid("6865136D-52E8-463f-946B-B9CE7CE9361F"), threading(apartment), event_source(com), vi_progid("AVSDocumentRenderer"), progid("AVSDocumentRenderer.1"), version(1.0)]
class ATL_NO_VTABLE CAVSDocumentRenderer :	public IAVSDocumentRenderer, public IAVSRenderer2
{
private:
	class CPageInfo
	{
...
private:
	...
	LONG m_lPagesCount;
	CAtlArray<IAVSMetafile*> m_arPages;
	...
	NSDocumentViewer::CWatermark m_oWatermark;

public:

	__event __interface _IAVSDocumentRendererEvents;

	CAVSDocumentRenderer() : m_arPages()
	{
		m_fCurrentX = 0;
		m_fCurrentY = 0;
...
public:
	...
	STDMETHOD(NewPage)()
	{
		...
		IAVSMetafile* pPage = NULL;
		CoCreateInstance(__uuidof(CAVSMetafile), NULL, CLSCTX_INPROC, __uuidof(IAVSMetafile), (void**)&pPage);
...
// additiaonal params ----------------------------------------------------------------------
	...
	STDMETHOD(DrawPage)(LONG lPageNumber, IUnknown* punkRenderer)
	{
		...
		if (m_oWatermark.IsInit())
		{
			DrawWatermark(lPageNumber, punkRenderer);
		}
	...
	STDMETHOD(SetAdditionalCommands)(BSTR bsXML)
	{
		...
		{
			m_oWatermark.LoadFromXML(bsXML);
		}
	...
	STDMETHOD(CommandLong)(LONG lCommand)
	{
		...
		IAVSRenderer2* pRend2 = NULL;
		m_arPages[lPageIndex]->QueryInterface(__uuidof(IAVSRenderer2), (void**)&pRend2);
...
protected:
	...
	void DrawWatermark(LONG lPageIndex, IUnknown* punkRenderer);
};