...
namespace agg
{
    ...
    public:
		...
		template<class Glyph> void render(const Glyph* pGlyph, int x, int y)
		{
			int width = pGlyph->nWidth;
			for (int j = 0; j < pGlyph->nHeight; ++j)
			{
				m_ren->blend_solid_hspan(x, y + j, width, m_color, pGlyph->pData + j * width);
			}
		}

		__forceinline void render(int lWidth, int lHeight, unsigned char* pData, int x, int y)
		{
			BYTE* pMemory = pData;
