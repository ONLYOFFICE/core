...
//------------------------------------------------------------------------
void Agg2D::rotate(double angle)
{
    m_affine.premultiply(agg::trans_affine_rotation(angle));
    updateTransformations();
}
//------------------------------------------------------------------------
void Agg2D::rotate(double angle, double cx, double cy)
{
...
//------------------------------------------------------------------------
void Agg2D::skew(double sx, double sy)
{
    m_affine.premultiply(agg::trans_affine_skewing(sx, sy));
    updateTransformations();
}
//------------------------------------------------------------------------
void Agg2D::translate(double x, double y)
{
...
//------------------------------------------------------------------------
double Agg2D::textWidth(const char* str, unsigned int len)
{
#if defined( UNDER_CE )
  return 0;
#else
...
//------------------------------------------------------------------------
double Agg2D::textWidth(const wchar_t* str, unsigned int len)
{
#if defined( UNDER_CE )
  return 0;
#else
...
/*void Agg2D::drawSVG(SVGImage* svg_im)
{
	agg::scanline_p8 sl;
	agg::trans_affine mtx;

	m_rasterizer.reset();
	m_rasterizer.gamma(agg::gamma_power(1));
	mtx *= agg::trans_affine_translation((svg_im->m_min_x + svg_im->m_max_x) * -0.5, (svg_im->m_min_y + svg_im->m_max_y) * -0.5);
	mtx *= agg::trans_affine_scaling(0.5,0.1);
	//mtx *= agg::trans_affine_rotation(agg::deg2rad(5));
	mtx *= agg::trans_affine_translation((svg_im->m_min_x + svg_im->m_max_x) * 0.5, (svg_im->m_min_y + svg_im->m_max_y) * 0.5 + 30);

	svg_im->m_path.expand(5);

	if(m_blendMode == BlendAlpha)
	{
		svg_im->m_path.render(m_rasterizer, sl, m_renSolid, mtx, m_renBase.clip_box(), 1.0);
	}
	else
	{
		svg_im->m_path.render(m_rasterizer, sl, m_renSolidComp, mtx, m_renBaseComp.clip_box(), 1.0);
	}
