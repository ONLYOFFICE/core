...
void PNGAPI CxImagePNG::user_error_fn(png_structp png_ptr,png_const_charp error_msg)
{
	strncpy((char*)png_ptr->error_ptr,error_msg,255);
	longjmp(png_ptr->jmp_buf_local, 1);
}
void PNGAPI CxImagePNG::user_flush_data(png_structp png_ptr)
{
	CxFile* hFile = (CxFile*)png_get_io_ptr(png_ptr);
	if (hFile == NULL || !hFile->Flush()) png_error(png_ptr, "Flush Error");
}
void PNGAPI CxImagePNG::user_read_data(png_structp png_ptr, png_bytep data, png_size_t length)
{
	CxFile* hFile = (CxFile*)png_get_io_ptr(png_ptr);
	if (hFile == NULL || hFile->Read(data,1,length) != length) png_error(png_ptr, "Read Error");
}

void PNGAPI CxImagePNG::user_write_data(png_structp png_ptr, png_bytep data, png_size_t length)
{
	CxFile* hFile = (CxFile*)png_get_io_ptr(png_ptr);
...
bool CxImagePNG::Decode(CxFile *hFile)
{
	...
	bool start_read_end_data = false;
  ...
  cx_try
  {
    ...
    * set up your own error handlers in the png_create_read_struct() earlier. */
	...
	int oneIndexAlpha = 0;												// NOTE! 17.10.2013	-	http://bugzserver/show_bug.cgi?id=20723

	if (info_ptr->num_trans!=0){ //palette transparency
		if (info_ptr->num_trans==1){
			if (info_ptr->color_type == PNG_COLOR_TYPE_PALETTE){
				info.nBkgndIndex = info_ptr->trans_color.index;				
				oneIndexAlpha = 1;										// NOTE! 17.10.2013	-	http://bugzserver/show_bug.cgi?id=20723
			} else{
	...
	row_pointers = new BYTE[info_ptr->rowbytes + 8+1024];
	...
	start_read_end_data =true;
	/* read the rest of the file, getting any additional chunks in info_ptr */

	png_read_end(png_ptr, info_ptr);

	start_read_end_data = false;
	/* clean up after the read, and free any memory allocated - REQUIRED */
	png_destroy_read_struct(&png_ptr, &info_ptr, (png_infopp)NULL);
	
#if CXIMAGE_SUPPORT_ALPHA														// NOTE<! 17.10.2013	-	http://bugzserver/show_bug.cgi?id=20723

	// только если индексная палитра и нету альфы

	if ((1 == oneIndexAlpha) && (0 == alpha_present) && (8 == GetBpp()))
	{
		info.bAlphaPaletteEnabled	=	true;								
		info.nAlphaMax				=	255;
		...
		if (0 == alpha_present)
		{
			if (AlphaCreate())
			{
				...
				DWORD stride = GetEffWidth();

				BYTE* alpha	= AlphaGetPointer();
				BYTE* src = GetBits();
	...
	}																			// NOTE!--> 17.10.2013	-	http://bugzserver/show_bug.cgi?id=20723

#endif //CXIMAGE_SUPPORT_ALPHA

  } cx_catch {
	if (strcmp(message,"")) strncpy(info.szLastError,message,255);
	if ((info.nEscape == -1 || start_read_end_data) && info.dwType == CXIMAGE_FORMAT_PNG) return true;
	return false;
  }
