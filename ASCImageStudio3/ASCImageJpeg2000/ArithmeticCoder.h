...
// Данный массив содержит все возможные состояние (см. Annex C Table C-2)
...
MQCoder* MQC_Create() 
{
	MQCoder *pCoder = (MQCoder*)Malloc(HEAP_ZERO_MEMORY, sizeof(MQCoder));
	return pCoder;
}
void     MQC_Destroy(MQCoder *pMQCoder) 
{
	RELEASEHEAP( pMQCoder );
}
...
{
	// TO DO: Данная функция непольностью протестирована
	pMQCoder->nCregister = 0;
	pMQCoder->nCt = 8;
