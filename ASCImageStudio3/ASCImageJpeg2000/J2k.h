...
static void J2k_ReadSIZ (J2kCodestream *pJ2k) 
{
	...
	pCodingParams->pTCP = (TileCodingParams *) Malloc(HEAP_ZERO_MEMORY,  pCodingParams->nXTilesCount * pCodingParams->nYTilesCount * sizeof( TileCodingParams ) );
	...
	pCodingParams->pTileIndex = (int *)Malloc(HEAP_ZERO_MEMORY,  pCodingParams->nXTilesCount * pCodingParams->nYTilesCount * sizeof(int) );
	if ( !pCodingParams->pTileIndex )
	{
		RELEASEHEAP( pImage->pComponents );
		RELEASEHEAP( pCodingParams->pTCP );
		pJ2k->pCodecInfo->nErrorCode = JP2_ERROR_NOT_ENOUGH_MEMORY;
		return;
	}
	...
	pJ2k->pDefaultTCP->pTCCP = (TileCompCodingParams *) Malloc(HEAP_ZERO_MEMORY,  sizeof(TileCompCodingParams) * pImage->nCsiz );
	if ( !pJ2k->pDefaultTCP->pTCCP )
	{
		RELEASEHEAP( pImage->pComponents );
		RELEASEHEAP( pCodingParams->pTCP );
		RELEASEHEAP( pCodingParams->pTileIndex );
		pJ2k->pCodecInfo->nErrorCode = JP2_ERROR_NOT_ENOUGH_MEMORY;
		return;
	}
	...
	for ( int nIndex = 0; nIndex < pCodingParams->nXTilesCount * pCodingParams->nYTilesCount; nIndex++ ) 
	{
		pCodingParams->pTCP[nIndex].pTCCP = (TileCompCodingParams *) Malloc(HEAP_ZERO_MEMORY,  sizeof(TileCompCodingParams) * pImage->nCsiz );
		if ( !pCodingParams->pTCP[nIndex].pTCCP )
		{
			for ( int nI = 0; nI < nIndex; nI++ )
			{
				RELEASEHEAP( pCodingParams->pTCP[nI].pTCCP );
				RELEASEHEAP( pImage->pComponents );
				RELEASEHEAP( pCodingParams->pTCP );
				RELEASEHEAP( pCodingParams->pTileIndex );
				RELEASEHEAP( pJ2k->pDefaultTCP->pTCCP );
				pJ2k->pCodecInfo->nErrorCode = JP2_ERROR_NOT_ENOUGH_MEMORY;
				return;
			}
		}
	}
	pJ2k->ppTileData = (unsigned char **) Malloc(HEAP_ZERO_MEMORY,  pCodingParams->nXTilesCount * pCodingParams->nYTilesCount * sizeof(unsigned char *) );
	if ( !pJ2k->ppTileData )
	{
		for ( int nIndex = 0; nIndex < pCodingParams->nXTilesCount * pCodingParams->nYTilesCount; nIndex++ ) 
		{
			RELEASEHEAP( pCodingParams->pTCP[nIndex].pTCCP );
		}
		RELEASEHEAP( pImage->pComponents );
		RELEASEHEAP( pCodingParams->pTCP );
		RELEASEHEAP( pCodingParams->pTileIndex );
		RELEASEHEAP( pJ2k->pDefaultTCP->pTCCP );
		RELEASEHEAP( pJ2k->pDefaultTCP);
		pJ2k->pCodecInfo->nErrorCode = JP2_ERROR_NOT_ENOUGH_MEMORY;
		return;
	}

	pJ2k->pTileLen   = (int *) Malloc(HEAP_ZERO_MEMORY,  pCodingParams->nXTilesCount * pCodingParams->nYTilesCount * sizeof(int) );
	if ( !pJ2k->pTileLen )
	{
		for ( int nIndex = 0; nIndex < pCodingParams->nXTilesCount * pCodingParams->nYTilesCount; nIndex++ ) 
		{
...
static void J2k_ReadPPM (J2kCodestream *pJ2k) 
{
	...
	while ( nLen > 0 ) 
	{
		...
		{
			pCodingParams->pPPMData      = (unsigned char *) Malloc(HEAP_ZERO_MEMORY,  nNppm * sizeof(unsigned char) );
		...
		{
			BYTE *pPPMData_new = (BYTE *)Malloc(0, ( nNppm + pCodingParams->nPPMStore ) * sizeof(unsigned char));
			if (!pPPMData_new)
			{
				pJ2k->pCodecInfo->nErrorCode = JP2_ERROR_NOT_ENOUGH_MEMORY;
				return;
			...
			RELEASEHEAP(pCodingParams->pPPMData);
...
static void J2k_ReadPPT (J2kCodestream *pJ2k) 
{
	...
	{
		pTCP->pPPTData      = (unsigned char *) Malloc(HEAP_ZERO_MEMORY,  (nLen - 3) * sizeof(unsigned char) );
		if (!pTCP->pPPTData)
		{
			pJ2k->pCodecInfo->nErrorCode = JP2_ERROR_NOT_ENOUGH_MEMORY;
			return;
	...
	{
		BYTE *pPPTData_new =  (unsigned char *) Malloc(HEAP_ZERO_MEMORY, (nLen - 3 + pTCP->nPPTStore) * sizeof(unsigned char));
		if (!pPPTData_new)
		{
			pJ2k->pCodecInfo->nErrorCode = JP2_ERROR_NOT_ENOUGH_MEMORY;
			return;
...
static void J2k_ReadSOD (J2kCodestream *pJ2k) 
{
	...
	unsigned char *pData = (unsigned char *) Malloc(HEAP_ZERO_MEMORY, ( pJ2k->pTileLen[nCurTileIndex] + nLen ) * sizeof(unsigned char));
	if ( !pData )
	{
		pJ2k->pCodecInfo->nErrorCode = JP2_ERROR_NOT_ENOUGH_MEMORY;
		return;
	...
	RELEASEHEAP( pJ2k->ppTileData[nCurTileIndex] );
...
static void J2k_ReadEOC (J2kCodestream *pJ2k) 
{
	...
	if ( pTCD->pCodecInfo->nErrorCode == JP2_ERROR_NO_ERROR )
	{
		for ( int nIndex = 0; nIndex < pJ2k->pCodingParams->nTileIndexSize; nIndex++ ) 
		{
			...
			RELEASEHEAP( pJ2k->ppTileData[nTileIndex] );
...
{
	J2kCodestream *pJ2k = (J2kCodestream*)Malloc(HEAP_ZERO_MEMORY,  sizeof(J2kCodestream) );
	if( pJ2k ) 
	{
		pJ2k->pCodecInfo  = pCodecInfo;
		pJ2k->pDefaultTCP = (TileCodingParams*)Malloc(HEAP_ZERO_MEMORY, sizeof(TileCodingParams));
		if( !pJ2k->pDefaultTCP ) 
		{
...
void  J2k_DestroyDecompress(J2kCodestream *pJ2k) 
{
	...
	if( pJ2k->pDefaultTCP != NULL ) 
	{
		...
		RELEASEHEAP( pDefaultTCP->pPPTDataFirst );
	...
	if( pJ2k->pCodingParams != NULL ) 
	{
		...
		if( pCodingParams->pTCP != NULL ) 
		{
			for( int nIndex = 0; nIndex < pCodingParams->nXTilesCount * pCodingParams->nYTilesCount; nIndex++ ) 
			{
				RELEASEHEAP( pCodingParams->pTCP[nIndex].pPPTDataFirst );
				RELEASEHEAP( pCodingParams->pTCP[nIndex].pTCCP );
			}
			RELEASEHEAP( pCodingParams->pTCP );
		}
		RELEASEHEAP( pCodingParams->pPPMDataFirst );
		RELEASEHEAP( pCodingParams->pTileIndex );  
		RELEASEHEAP( pCodingParams->sComment );

		RELEASEHEAP( pCodingParams );
	}
...
void           J2k_SetupDecoder(J2kCodestream *pJ2k, DecoderParams *pParameters) 
{
	if( pJ2k && pParameters ) 
	{
		...
		CodingParams *pCodingParams = (CodingParams*)Malloc(HEAP_ZERO_MEMORY, sizeof(CodingParams));
		if ( !pCodingParams )
		{
			pJ2k->pCodecInfo->nErrorCode = JP2_ERROR_NOT_ENOUGH_MEMORY;
			return;
	...
	if( pJ2k->pImageInfo != NULL ) 
	{
		...
		if ( pImageInfo->nIndexOn && pJ2k->pCodingParams ) 
		{
			...
			for ( int nTileIndex = 0; nTileIndex < pCodingParams->nXTilesCount * pCodingParams->nYTilesCount; nTileIndex++ ) 
			{
				...
				RELEASEHEAP( pTileInfo->pdThreshold );
				RELEASEHEAP( pTileInfo->pPacket );
			}
			RELEASEHEAP( pImageInfo->pTile );
		}
		RELEASEHEAP( pImageInfo );
	}
	if( pJ2k->pCodingParams != NULL ) 
	{
		CodingParams *pCodingParams = pJ2k->pCodingParams;
		...
		RELEASEHEAP( pCodingParams->sComment );
		RELEASEHEAP( pCodingParams->pMatrix );
		
		for ( int nTileIndex = 0; nTileIndex < pCodingParams->nXTilesCount * pCodingParams->nYTilesCount; nTileIndex++ ) 
		{
			RELEASEHEAP( pCodingParams->pTCP[nTileIndex].pTCCP );
		}
		RELEASEHEAP( pCodingParams->pTCP );
		RELEASEHEAP( pCodingParams );
	}
...
void           J2k_SetupEncoder(J2kCodestream *pJ2k, EncoderParams *pParameters, Image *pImage) 
{
	...
	if( pParameters->pMatrix ) 
	{
		...
		pCodingParams->pMatrix = (int *) Malloc(HEAP_ZERO_MEMORY, array_size);
	...
	{
		pJ2k->pImageInfo = (ImageInfo*)Malloc(HEAP_ZERO_MEMORY, sizeof(ImageInfo));
		if ( !pJ2k->pImageInfo )
		{
			if ( pCodingParams->pMatrix )
				RELEASEHEAP( pCodingParams->pMatrix );
	...
	{
		pCodingParams->sComment = (char*)Malloc(HEAP_ZERO_MEMORY,  strlen(pParameters->sComment) + 1 );
	...
	for ( int nTileIndex = 0; nTileIndex < pCodingParams->nXTilesCount * pCodingParams->nYTilesCount; nTileIndex++ ) 
	{
		...
		pTCP->pTCCP = (TileCompCodingParams *) Malloc(HEAP_ZERO_MEMORY,  pImage->nCsiz * sizeof(TileCompCodingParams) );
		if ( !pTCP->pTCCP )
		{
			for ( int nI = 0; nI < nTileIndex; nI++ )
			{
				TileCodingParams *pDeleteTCP = &pCodingParams->pTCP[nI];
				RELEASEHEAP( pDeleteTCP->pTCCP );
			}
			RELEASEHEAP( pCodingParams->pMatrix );
			RELEASEHEAP( pJ2k->pImageInfo );
...
bool           J2k_Encode(J2kCodestream *pJ2k, CReader *pStream, Image *pImage, char *sIndex) 
{
	...
	if ( pImageInfo && pCodingParams->nIndexOn ) 
	{
		...
		pImageInfo->pTile        = (TileInfo *) Malloc(HEAP_ZERO_MEMORY,  pCodingParams->nXTilesCount * pCodingParams->nYTilesCount * sizeof(TileInfo) );
