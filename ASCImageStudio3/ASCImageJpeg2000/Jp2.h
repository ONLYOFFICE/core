...
static bool Jp2_ReadIHDR (Jp2Stream *pJp2, CReader *pStream) 
{
	...
	pJp2->pComponents      = (Jp2Component*) Malloc(HEAP_ZERO_MEMORY, pJp2->nComponentsCount * sizeof(Jp2Component));
	if ( !pJp2->pComponents )
	{
...
static bool Jp2_ReadCOLR (Jp2Stream *pJp2, CReader *pStream) 
{
	...
	{
		AtlTrace("A conforming JP2 reader shall ignore all Colour Specification boxes after the first, so we ignore this one.\n");
		return true;
	}
	...
	if ( pJp2->nMethod == 1 ) 
	{
		...
		pJp2->oColor.icc_profile_buf = (BYTE*) Malloc(HEAP_ZERO_MEMORY,icc_len);
...
static bool Jp2_ReadFTYP (Jp2Stream *pJp2, CReader *pStream) 
{
	...
	pJp2->pCompList = (unsigned int *) Malloc(HEAP_ZERO_MEMORY,  pJp2->nCompListLength * sizeof(unsigned int));
	if ( !pJp2->pCompList )
	{
		pJp2->pCodecInfo->nErrorCode = JP2_ERROR_NOT_ENOUGH_MEMORY;
		return false;
...
void       Jp2_DestroyDecompress(Jp2Stream *pJp2) 
{
	if( pJp2 ) 
	{
		...
		RELEASEHEAP( pJp2->oColor.icc_profile_buf);
		RELEASEHEAP( pJp2->pComponents );
		RELEASEHEAP( pJp2->pCompList );		
		RELEASEHEAP( pJp2 );
	}
}
...
{
	Jp2Stream *pJp2 = (Jp2Stream*)Malloc(HEAP_ZERO_MEMORY,  sizeof(Jp2Stream) );
	if( pJp2 ) 
	{
