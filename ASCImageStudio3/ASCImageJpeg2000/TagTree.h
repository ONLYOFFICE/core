...
{
	TagTree *pTree = (TagTree *) Malloc(HEAP_ZERO_MEMORY,  sizeof(TagTree) );
	if( !pTree ) 
		return NULL;
	...
	pTree->pNodes = (TagNode *) Malloc(HEAP_ZERO_MEMORY,  pTree->nNodesCount * sizeof(TagNode) );
	if( !pTree->pNodes ) 
	{
		RELEASEHEAP( pTree );
		return NULL;
