...
BOOL ImagePaint3::ApplyColor(BYTE* pPixels, int nSize, int nColor, int nAlpha)
{
	// variables
...
BOOL ImagePaint3::WipeDoor(double Frame, int Type, int Color, int Alpha)
{
	// check for simpliest case
	if (Frame < c_dFrameMin)
		return TRUE;
	else if (Frame > c_dFrameMax)
		return ApplyColor(m_pixels, m_nSize, Color, Alpha);
	...
	// compute tile size
	int nSizeX = (m_nSizeX < 2) ? m_width : m_nSizeX;
	int nSizeY = (m_nSizeY < 2) ? m_height : m_nSizeY;
	...
	for (nY = 0; nY < m_height; ++nY)
	{
		...
		for (nX = 0; nX < m_width; ++nX, nIndex += 4)
		{
			...
			{
				m_pixels[nIndex + 0] = (BYTE)(dAlpha2*m_pBackup[nIndex + 0] + dBlue);
				m_pixels[nIndex + 1] = (BYTE)(dAlpha2*m_pBackup[nIndex + 1] + dGreen);
				m_pixels[nIndex + 2] = (BYTE)(dAlpha2*m_pBackup[nIndex + 2] + dRed);
			}
		}
	}
...
BOOL ImagePaint3::WipeSide(double Frame, int Type, int Color, int Alpha)
{
	// check for simpliest case
	if (Frame < c_dFrameMin)
		return TRUE;
...
BOOL ImagePaint3::WipeCenter(double Frame, int Type, int Color, int Alpha)
{
	// check for simpliest case
	if (Frame < c_dFrameMin)
		return TRUE;
...
BOOL ImagePaint3::WipeStrips(double Frame, int Type, int Color, int Alpha)
{
	// check for simpliest case
	if (Frame < c_dFrameMin)
		return TRUE;
...
BOOL ImagePaint3::PushDoor(double Frame, int Type, int Color, int Alpha)
{
	// check for simpliest case
	if (Frame < c_dFrameMin)
		return TRUE;
	...
	for (nY = 0; nY < m_height; ++nY)
	{
		...
		for (nX = 0; nX < m_width; ++nX, nIndex += 4)
		{
			...
			// combine colors
			if (nSourceX < 0 || nSourceX >= m_width || nSourceY < 0 || nSourceY >= m_height)
			{
				m_pixels[nIndex + 0] = m_pBackup[nIndex + 0];
				m_pixels[nIndex + 1] = m_pBackup[nIndex + 1];
				m_pixels[nIndex + 2] = m_pBackup[nIndex + 2];
			}
			else
			{
				// compute source pixel index
				nSourceIndex = 4*(nSourceX + nSourceY*m_width);

				if (nPatternValue == 0)
				{
					m_pixels[nIndex + 0] = m_pBackup[nSourceIndex + 0];
					m_pixels[nIndex + 1] = m_pBackup[nSourceIndex + 1];
					m_pixels[nIndex + 2] = m_pBackup[nSourceIndex + 2];
				}
				else
				{
					m_pixels[nIndex + 0] = (BYTE)(dAlpha2*m_pBackup[nSourceIndex + 0] + dBlue);
					m_pixels[nIndex + 1] = (BYTE)(dAlpha2*m_pBackup[nSourceIndex + 1] + dGreen);
					m_pixels[nIndex + 2] = (BYTE)(dAlpha2*m_pBackup[nSourceIndex + 2] + dRed);
				}
			}
		}
	}
...
BOOL ImagePaint3::PushSide(double Frame, int Type, int Color, int Alpha)
{
	// check for simpliest case
	if (Frame < c_dFrameMin)
		return TRUE;
...
BOOL ImagePaint3::PushStrips(double Frame, int Type, int Color, int Alpha)
{
	// check for simpliest case
	if (Frame < c_dFrameMin)
		return TRUE;
...
BOOL ImagePaint3::FxMosaic(double Frame, int Type, int Color, int Alpha)
{
	// check for simpliest case
	if (Frame < c_dFrameMin)
		return TRUE;
...
BOOL ImagePaint3::ApplyCompose(double Frame, long Type, int Color, int Alpha)
{
