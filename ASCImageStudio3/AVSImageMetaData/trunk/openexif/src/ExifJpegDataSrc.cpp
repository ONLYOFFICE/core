...
METHODDEF (void)
init_ref_source (j_decompress_ptr cinfo)
{
...
#ifdef macintosh
METHODDEF (boolean)
fill_ref_input_buffer (j_decompress_ptr cinfo)
{
  nif_ref_src_ptr src = (nif_ref_src_ptr) cinfo->src;
  size_t nbytes;
...
#ifdef macintosh
METHODDEF(void)
skip_ref_input_data (j_decompress_ptr cinfo, int num_bytes)
{
...
GLOBAL(void)
openexif_jpeg_mem_src (j_decompress_ptr cinfo, JOCTET *UserBuffer, int size)
{
  nif_src_ptr src;
...
#ifdef macintosh
GLOBAL (void)
jpeg_ref_src (j_decompress_ptr cinfo, short refNum )
{
  nif_ref_src_ptr src;
