...
#define SET_TEXT2(VAL)		tmpNode2.SetText(_bstr_t(VAL))
#define SET_TEXT(VAL)		tmpNode.SetText(_bstr_t(VAL))
...
//	XML::XMLDocumentWrapper xmlDoc;
	...
	SetAttrib(tmpNode,OLESTR("mode"),1);
	...
	_NODE(OLESTR("AutoWhiteBalance"),UseCameraWhiteBalance);
	...
	_SUBNODE(OLESTR("R"));
	SET_MIN_ATTR2(CorrectChromaticAberration.max_r);
...
HRESULT ImageRaw3SimpleModeOptions::ParseXMLDoc(XML::XMLDocumentWrapper & xmlDoc)
{
	...
	if (GetUseByName(tmpNode,OLESTR("Temperature"),UseTemperature))
	{
		...
		bool tmpB;
		GetParamsByNameI(tmpNode2,OLESTR("Temperature"),tmpB,Temperature.temperature,Temperature.min_temperature,Temperature.max_temperature);
