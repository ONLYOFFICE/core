...
/*! \file	dcraw_api.c
*	\brief	API for DCRaw (Адаптация Unidentified Flying Raw converter для COM-компоненты AVSImageRaw3)
*
*	За основу взят файл dcraw_api.cc из пакета ufraw-0.14.1.tar.gz
...
extern "C" {
	...
	*/
		...
		dcraw_data *dark, int scale)
	{
		...
		int black = dark ? DCRAW_MAX(hh->black - dark->black, 0) : hh->black;
				...
				for(c=0; c<w; c++) {
					...
					for (ri=0; ri<scale; ri++)
						...
						for (cl=0; cl<hh->raw.colors; cl++)
							f->image[r*w+c][cl] =
							DCRAW_MAX(sum[cl]/count[cl] - black,0);
		...
		dcraw_data *h, float threshold)
	{
		...
		d->lastStatus = DCRAW_SUCCESS;
		if (threshold)
			wavelet_denoise_INDI(f->image, 0, f->height, f->width, 0, 0, 4, 0,	NULL, threshold, 0, d);
		h->message = d->messageBuffer;
		return d->lastStatus;
	}
		...
		dcraw_data *dark, int interpolation, int smoothing, int rgbWB[4])
	{
		...
		int black = dark ? DCRAW_MAX(h->black - dark->black, 0) : h->black;
