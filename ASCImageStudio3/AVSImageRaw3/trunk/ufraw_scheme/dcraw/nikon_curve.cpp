/*!
*	\file	nikon_curve.cpp
*	\brief	read Nikon NTC/NCV files (Адаптация Unidentified Flying Raw converter для COM-компоненты AVSImageRaw3)
*
...
*
*	За основу взят файл ufraw.h из пакета ufraw-0.14.1.tar.gz
...
/*!
*	\brief	The Nikon Curve message handler. Udi Fuchs created this
...
#ifdef _DEBUG
#define DEBUG_PRINT(...) do {AtlTrace("[nikon curve]: ");AtlTrace(__VA_ARGS__);} while(0);
#else
#define DEBUG_PRINT(...)
#endif

/*!
 *	\brief	Handle memory allocaltion errors
 *	\todo	Сделать общую обработку ошибок
 */
void nc_merror(void *ptr, char *where_)
{
	if (ptr) return;
...
*/
#define STATIC_ASSERT(cond)    { (void)((int (*)(char failed_static_assertion[(cond)?1:-1]))0); }

/*!
*	\brief	Determines if the machine we are running on is big endian or not.
*/
int isBigEndian()
{
...
/*!
*	\brief	Convert short int (16 bit) from little endian to machine endianess.
*/
...
/*!
*	\brief	Convert long int (32 bit) from little endian to machine endianess.
*/
int LongVal(int i)
{
...
/*!
*	\brief	Convert float from little endian to machine endianess.
*/
...
/*!
*	\brief	Convert double from little endian to machine endianess.
*/
...
/*!
 *	\brief	factors and solves a D3 system.
 *	\ditails	Discussion:
 ...
 *	\param[in]		n	the order of the linear system.
 *	\param[in,out]	a	[3*N]:	On input, the nonzero diagonals of the linear system.
 *						On output, the data in these vectors has been overwritten
 *						by factorization information.
 *	
 *	\param[in]		B	[N], the right hand side.
 *	
 *	\return			double D3_NP_FS[N], the solution of the linear system.
...
/*!	
 *	\brief		Computes the second derivatives of a piecewise cubic spline.
 *	\details	Discussion:
 ...
 *	
 *	
 *	\param[in]	n		The number of data points.  N must be at least 2.
 *						In the special case where N = 2 and IBCBEG = IBCEND = 0, the
 *						spline will actually be linear.
 *	
 *	\param[in]	t		[N], the knot values, that is, the points were data is
 *						specified.  The knot values should be distinct, and increasing.
 *	
 *	\param[in]	y		[N], the data values to be interpolated.
 *	
 *	\param[in]	ibcbeg	Left boundary condition flag:										\n
 *						0: the cubic spline should be a quadratic over the first interval;	\n
 *						1: the first derivative at the left endpoint should be YBCBEG;		\n
 *						2: the second derivative at the left endpoint should be YBCBEG.		
 *	
 *	\parma[in]	ybcbeg	The values to be used in the boundary
 *						conditions if IBCBEG is equal to 1 or 2.
 *	
 *	\param[in]	ibcend	Right boundary condition flag:										\n
 *						0: the cubic spline should be a quadratic over the last interval;	\n
 *						1: the first derivative at the right endpoint should be YBCEND;		\n
 *						2: the second derivative at the right endpoint should be YBCEND.	
 *	
 *	\parama[in]	ybcend	The values to be used in the boundary
 *						conditions if IBCEND is equal to 1 or 2.
 *	
 *	\return		SPLINE_CUBIC_SET[N], the second derivatives of the cubic spline.
 */
...
/*!
 *	\brief		Evaluates a piecewise cubic spline at a point.
 *	
 *	\details	SPLINE_CUBIC_SET must have already been called to define the values of YPP.
 *	
 *				For any point T in the interval T(IVAL), T(IVAL+1), the form of
 *				the spline is
 *				
 ...
 *	
 *	\param[in]	y			[N], the data values at the knots.
 *	
 *	\param[in]	t			[N], the knot values.
 *	
 *	\param[in]	tval		a point, typically between T[0] and T[N-1], at
 *							which the spline is to be evalulated.  If TVAL lies outside
 *							this range, extrapolation is used.
 *	
 *	\param[in]	y			[N], the data values at the knots.
 *	
 *	\param[in]	ypp			[N], the second derivatives of the spline at
 *	the knots.
 *	
 *	\param[out]	ypval		the derivative of the spline at TVAL.
 *	
 *	\param[out]	yppval		the second derivative of the spline at TVAL.
 *	
 *	\return		spline_val	the value of the spline at TVAL.
 *	
 */
...
/*!
 *	Gets the nikon file type by comparing file headers.
 *	
...
/*!
 *	\brief	Loads all curves from a Nikon ntc or ncv file.
 *	
 *	\param	fileName	The filename.
 *	\param	curve		Pointer to curve struct to hold the data.
 *	\param	resolution	How many data points to sample from the curve.
 */
...
/*!
 *	\brief	Samples from a spline curve constructed from
 *			the Nikon data.
 *	
 *	\param	curve	Pointer to curve struct to hold the data.
 *	\param	sample	Pointer to sample struct to hold the data.
 */
int CurveDataSample(CurveData *curve, CurveSample *sample)
{
	...
	for(i = 0; i < (int)sample->m_samplingRes; i++)
	{
		...
		if (i < firstPointX) {
			...
			sample->m_Samples[i] = DCRAW_MIN(DCRAW_MAX(val,minY),maxY);
		}
	}
...
/*!
 *	\brief	Change the position of point to the new (x,y) coordinate.
...
/*!
 *	\fn	int SampleToCameraCurve(CurveData *curve, CurveSample *sample)
 *	
 *	\brief	EXPERIMENTAL!!!!!
 ...
 *			the camera.
 *	
 *	\details	This is a special function. While the function places
 ...
 *				accordingly.
 *	
 *	\param	curve	The Nikon curve to sample and transform.
 */
...
/*!
 *	\brief	Saves a curve to a Nikon ntc or ncv file.
 *	
 *	\param	data		A NikonData structure containing info of all the curves.
 *	\param	fileName	The filename.
 *	\param	filetype	Indicator for an NCV or NTC file.
 *	\param	version		The version of the Nikon file to write
 */
...
/*!
 *	\brief	Saves out curves to a Nikon ntc or ncv file. This function
 *			takes a single curve and uses defaults for the other curves.
 *			Typically, the curve used is the tone curve.
 *	
 *	\param	curve		A CurveData structure. This is usually the tone curve
 *	\param	curve_type	The curve type (TONE_CURVE, RED_CURVE, etc.)
 *	\param	fileName	The filename.
 *	\param	filetype	Indicator for an NCV or NTC file.
 *	\param	version		The version of the Nikon file to write
 *	
 *	\note	The only version tested is Nikon 4.1 anything
 *			other than this may result in unpredictable behavior.
 *			For now, the version passed in is ignored and is forced to 4.1.
 *	
 *	\details	This function is just a helper function that allows the user
...
/*!
 *	\brief	Saves a sampling from a curve to text file to
 *			be processed by UFRaw.
 *
 *	\param	sample		Pointer to sampled curve struct to hold the data.
 *	\param	fileName	The filename.
**********************************************/
...
/*!
 *	\brief	Frees memory allocated for this curve sample.
 */
...
/*!
 *	\brief	The main driver. Takes a filename and
...
/*!
 *	\brief	Moves the file pointer to the location
 *			indicated by the TAG-TYPE pairing. This is meant just
 *			as a helper function for this code. Uses elsewhere
 *			may be limited.
 *	
 *	\param	file		Nikon File ptr
 *	\param	num_entries	Number of entries to search through
 *	\param	tiff_tag	The tiff tag to match.
 *	\param	tiff_type	The tiff type to match.
 */
...
/*!
 *	\brief	Gets Nikon NEF data. For now, this is just the tone
 *			curve data.
 *	
 *	\param	infile		The input file
 *	\param	curve		data structure to hold data in.
 *	\param	sample_p	pointer to the curve sample reference.
...
/*!
 *	\brief	The actual retriever for the curve data from the NEF file.
 *	
 *	\param	file		The input file.
 *	\param	infile		Offset to retrieve the data
 *	\param	curve		data structure to hold curve in.
 *	\param	sample_p	pointer to the curve sample reference.
 *						can be NULL if curve sample is not needed.
********************************************************/
