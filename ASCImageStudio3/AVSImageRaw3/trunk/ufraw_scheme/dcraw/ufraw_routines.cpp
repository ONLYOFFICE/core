/*!
*	\file ufraw_routines.cpp
*	\brief	General routines (Адаптация Unidentified Flying Raw converter для COM-компоненты AVSImageRaw3)
*	За основу взят файл ufraw_routines.c из пакета ufraw-0.14.1.tar.gz
*	Новые версии доступны тут: http://ufraw.sourceforge.net/
* 
*	Базовый файл, относительно которого вносились правки:
*		./base/ufraw_routines.cb
*
...
static void curve_parse_start(GMarkupParseContext *context,
							  ...
							  gpointer user, /*GError*/ void **error)
{
	CurveData *c = user;
	int int_value;
...
int ptr_array_insert_sorted (GPtrArray *array_, const void *item, GCompareFunc compare)
{
	int length = array_->len;
	g_ptr_array_set_size (array_, length + 1);
	const void **root = (const void **)array_->pdata;
...
int ptr_array_find_sorted (const GPtrArray *array_, const void *item, GCompareFunc compare)
{
	int length = array_->len;
	void **root = array_->pdata;
