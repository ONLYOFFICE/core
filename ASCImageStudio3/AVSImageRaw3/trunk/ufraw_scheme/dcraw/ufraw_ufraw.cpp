/*!
*	\file ufraw_ufraw.cpp
*	\brief	Program interface to all the components (Адаптация Unidentified Flying Raw converter для COM-компоненты AVSImageRaw3)
*	За основу взят файл ufraw_ufraw.c из пакета ufraw-0.14.1.tar.gz
...
static int make_temporary(char *basefilename, char **tmpfilename)
{
	...
	char *template_ = g_strconcat(basename, ".tmp.XXXXXX", NULL);
	fd = g_file_open_tmp(template_, tmpfilename, NULL);
	g_free(template_);
	g_free(basename);
	return fd;
#endif
...
ufraw_data *ufraw_open(char *filename)
{
	...
	fname = strdup(filename); //g_filename_from_uri(filename, &hostname, NULL);
	if (fname!=NULL) {
		if (hostname!=NULL) {
	...
	if ( _strcasecmp(filename + strlen(filename) - 6, ".ufraw")==0 ) {
		...
		* This allows moving raw and ID files together between folders. */
		...
		if ( strcmp(inPath, outPath)==0 ) {
			...
			if ( /*! \todo	Исправить условие
				 g_file_test(inFile, G_FILE_TEST_EXISTS)*/1 ) {
	...
	if (!_strcasecmp(filename + strlen(filename) - 3, ".gz"))
		filename = decompress_gz(filename);
	else if (!_strcasecmp(filename + strlen(filename) - 4, ".bz2"))
...
int ufraw_load_darkframe(ufraw_data *uf)
{
	...
	dcraw_data *darkRaw = (dcraw_data *)(dark->raw);
...
int ufraw_load_raw(ufraw_data *uf)
{
	...
	if ( strcmp(uf->conf->make, "Canon")==0 &&
		strncmp(uf->conf->model, "EOS", 3)==0 ) {
			...
			for (c=1; c<raw->colors; c++) max = DCRAW_MAX(raw->cam_mul[c], max);
			...
			if ( max < 100 ) {
				...
				* ExposureNorm */
				if (uf->LoadingID && uf->conf->ExposureNorm==0)
					uf->conf->exposure -= log((double)1.0*raw->rgbMax/max)/log((double)2);
...
int ufraw_convert_image_first_phase(ufraw_data *uf, gboolean lensfix)
{
	...
	dcraw_image_data final;
	final.image = uf->image.image;
	dcraw_data *dark = (uf->conf->darkframe ? ((dcraw_data *)(uf->conf->darkframe->raw)) : (dcraw_data *)NULL);
	...
	if (uf->conf->size==0 && uf->conf->shrink>1) {
		dcraw_image_resize(&final,
			uf->ConvertShrink*DCRAW_MAX(final.height, final.width)/uf->conf->shrink);
	...
	ufraw_data *uf, int saidx, UFRawPhase phase)
{
	...
	ufraw_image_data *in = (phase > ufraw_first_phase) ? ufraw_convert_image_area (uf, saidx, (UFRawPhase)(phase - 1)) :
	...
	case ufraw_develop_phase:
		{
			...
			for (yy = y; yy < y + h; yy++)
			{
				...
				develope (
					dest, (guint16*)( (void *)(in->buffer + (yy * in->width + x) * in->depth)),
...
static void ufraw_build_raw_histogram(ufraw_data *uf)
{
	...
	double maxChan = 0;
	for (c=0; c<uf->colors; c++) maxChan = DCRAW_MAX(uf->conf->chanMul[c], maxChan);
	for (c=0; c<uf->colors; c++) {
	...
	for (i=0; i<count; i++)
		for (c=0; c<raw->raw.colors; c++)
			...
			(gint64)DCRAW_MAX(raw->raw.image[i][c]-raw->black, 0) *
...
void ufraw_auto_expose(ufraw_data *uf)
{
	...
	for (pMax=uf->rgbMax, pMin=0, p=(pMax+pMin)/2; pMin<pMax-1; p=(pMax+pMin)/2)
	{
		...
		for (c=0, wp=0; c<3; c++) wp = DCRAW_MAX(wp, p16[c]);
	...
	* we need to cancel its effect here. */
	if ( uf->conf->ExposureNorm>0 )
		uf->conf->exposure -=
		log((double)(1.0*uf->rgbMax/uf->conf->ExposureNorm))/log((double)2);
...
void ufraw_auto_black(ufraw_data *uf)
{
	...
	develope(p16, pix, uf->AutoDeveloper, 16, pixtmp, 1);
	for (c=0, bp=0; c<3; c++) bp = DCRAW_MAX(bp, p16[c]);
...
void ufraw_auto_curve(ufraw_data *uf)
{
	...
	for (bp=0, sum=0, p=0, i=j=0; i<steps && bp<uf->rgbMax && p<0xFFFF; i++) {
		...
		for (c=0; c<uf->colors; c++)
			pix[c] = DCRAW_MIN (bp * maxChan/uf->conf->chanMul[c], uf->rgbMax);
		develope(p16, pix, uf->AutoDeveloper, 16, pixtmp, 1);
		for (c=0, p=0; c<3; c++) p = DCRAW_MAX(p, p16[c]);
