...
[
	object,
	uuid("75733638-7311-4be0-A52D-3FF1CFF8C6DF"),
	dual,
	pointer_default(unique)
]
__interface IImageRaw3Thumbnail : IDispatch
{
	[id(1)]	HRESULT LoadThumbnail([in] BSTR fileName, [out] IUnknown** piUncFrame);
};
...
public:
	...
	CImageRaw3()
	{
		m_OpenFileWithoutLoadData = FALSE;
	}
...
public:
	...
	// IImageRaw3Thumbnail
	STDMETHOD(LoadThumbnail)(BSTR fileName, IUnknown** piUncFrame);

protected:

	HRESULT LoadGdiplusThumbnail(BYTE * thumb_data, size_t thumb_size, IUnknown** piUnkFrame);
	HRESULT LoadPPMThumbnail(BYTE * thumb_data, size_t thumb_size, IUnknown** piUnkFrame);
    HRESULT LoadPPMThumbnailWithoutHeader(size_t _Width, size_t _Height, unsigned int _Colors, BYTE * thumb_data, size_t thumb_size, IUnknown** piUnkFrame, int _Flip = 0);

	DCRaw::_ifd_entrys_t _ifd_entrys;
	DWORD m_dwMetaDataFlag;
	...
	VARIANT_BOOL m_OpenFileWithoutLoadData;
