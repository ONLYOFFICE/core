...
namespace DocFileFormat
{
	...
	long Converter::LoadAndConvert(const CString& strSrcFile, const CString& strDstFile, const ProgressCallback* progress)
	{
		...
		WordprocessingDocument docx(strDstFile, &doc);
		...
		if (SUCCEEDED(result) && (result != S_FALSE))
		{
			...
			if (SUCCEEDED(result) && (result != S_FALSE))
			{
				...
				progress->OnProgress(progress->caller, DOC_ONPROGRESSEVENT_ID, 1000000);

				short cancel = 0;
				progress->OnProgressEx(progress->caller, DOC_ONPROGRESSEVENT_ID, 1000000, &cancel);
