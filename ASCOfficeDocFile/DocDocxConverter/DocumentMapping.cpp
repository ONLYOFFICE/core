...
namespace DocFileFormat
{
	...
	int DocumentMapping::writeParagraph (int initialCp, int cpEnd, bool sectionEnd)
	{
		...
		if ( ( chpxs != NULL ) && ( chpxFcs != NULL ) && !chpxFcs->empty() )
		{
			...
			for (std::list<CharacterPropertyExceptions*>::iterator cpeIter = chpxs->begin(); cpeIter != chpxs->end(); ++cpeIter)
			{
				...
				if (bookmarks.size())
				{
					std::list<vector<WCHAR>>* runs = splitCharList(chpxChars, &bookmarks);

					for (std::list<vector<WCHAR> >::iterator iter = runs->begin(); iter != runs->end(); ++iter)
					{
	...
	int DocumentMapping::writeRun (std::vector<WCHAR>* chars, CharacterPropertyExceptions* chpx, int initialCp)
	{
		int cp = initialCp;
	...
	void DocumentMapping::writeText(std::vector<WCHAR>* chars, int initialCp, CharacterPropertyExceptions* chpx, bool writeDeletedText)
	{
		int cp = initialCp;
		...
		if (0)
			...
			if( writeDeletedText )
			{
				textType = wstring( _T( "delText" ) );
			}
			else if( this->_writeInstrText )
			{
				textType = wstring( _T( "instrText" ) );
			}
			...
			for ( unsigned int i = 0; i < chars->size(); i++ )
			{
				...
				if ( c == TextMark::Tab )
				{
					...
					XMLTools::XMLElement<WCHAR> elem( _T( "w:tab" ) );
				...
				else if ( c == TextMark::HardLineBreak )
				{
					...
					XMLTools::XMLElement<WCHAR> elem( _T( "w:br" ) );
				...
				else if ( c == TextMark::FieldBeginMark )
				{
					...
					wstring f( ( m_document->Text->begin() + cpFieldStart ), ( m_document->Text->begin() + cpFieldEnd + 1 ) );
					wstring embed( _T( " EMBED" ) );
					wstring link( _T( " LINK" ) );
					wstring form( _T( " FORM" ) );
				...
				else if ( c == TextMark::FieldSeparator )
				{
					if ( this->_fldCharCounter > 0 )
					{
						XMLTools::XMLElement<WCHAR> elem( _T( "w:fldChar" ) );
	...
	void DocumentMapping::writeTextElement(const wstring& text, const wstring& textType)
	{
		if ( !text.empty() )
		{
			writeTextStart( textType );
	...
	void DocumentMapping::writeTextStart(const wstring& textType)
	{
		wstring str = ( wstring( _T( "w:" ) ) + textType );
	...
	void DocumentMapping::writeTextEnd(const wstring& textType)
	{
		wstring str = ( wstring( _T( "w:" ) ) + textType );
	...
	std::vector<int> DocumentMapping::searchBookmarks(std::vector<WCHAR>* chars, int initialCp)
	{
		vector<int> ret;
	...
	std::list<vector<WCHAR> >* DocumentMapping::splitCharList(std::vector<WCHAR>* chars, std::vector<int>* splitIndices)
	{
		list<vector<WCHAR> >* ret = new list<vector<WCHAR> >();
		vector<WCHAR> wcharVector;
		...
		//add the last part
		wcharVector = vector<WCHAR>( chars->size() - startIndex );
	...
	bool DocumentMapping::writeBookmarkStart(short id)
	{
		...
		{
			XMLTools::XMLElement<WCHAR> bookmarkElem(_T("w:bookmarkStart"));
	...
	bool DocumentMapping::writeBookmarkEnd(short id)
	{
		...
		if ( ( bookmarkName != NULL ) && ( *bookmarkName != _T( "_PictureBullets" ) ) )
		{
			XMLTools::XMLElement<WCHAR> bookmarkElem( _T( "w:bookmarkEnd" ) );
	...
	int DocumentMapping::searchNextTextMark (std::vector<WCHAR>* chars, int initialCp, WCHAR mark)
	{
		int ret			=	initialCp;
		size_t count	=	chars->size();
