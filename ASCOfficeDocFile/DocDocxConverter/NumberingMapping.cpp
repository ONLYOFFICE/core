...
namespace DocFileFormat
{
	...
	bool NumberingMapping::IsPlaceholder(WCHAR symbol)
	{
		if ((symbol == (WCHAR)0x0000) || (symbol == (WCHAR)0x0001) || (symbol == (WCHAR)0x0002) || 
			(symbol == (WCHAR)0x0003) || (symbol == (WCHAR)0x0004) || (symbol == (WCHAR)0x0005) || 
			(symbol == (WCHAR)0x0006) || (symbol == (WCHAR)0x0007) || (symbol == (WCHAR)0x0008))
		{
			return true;  
		}
	...
	void NumberingMapping::PictureBulletsMapping()
	{
		for (map<int, int>::const_iterator iter = m_document->PictureBulletsCPsMap.begin(); iter != m_document->PictureBulletsCPsMap.end(); ++iter)
		{
			...
			if ((chpxs != NULL) && (!chpxs->empty()))
			{
				...
				if ((pict.mfp.mm > 98) && (pict.shapeContainer != NULL) && (pict.blipStoreEntry != NULL))
				{
