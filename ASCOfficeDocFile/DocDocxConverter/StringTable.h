...
namespace DocFileFormat
{
  template<class T> class StringTable: public IVisitable
  {
    ...
    public:
	  ...
	  StringTable( IStream* tableStream, unsigned int fc, unsigned int lcb ):
	  _enc(ENCODING_INVALID_VALUE), fExtend(false), cData(0), cbExtra(0)
      {
        if ( lcb > 0 )
        {
	  ...
	  void parse( VirtualStreamReader *reader, unsigned int fc )
	  {
	    if ( reader != NULL )
		{
          ...
          else
          {
		    ...
		    reader->Seek( (int)fc, STREAM_SEEK_SET );
	      }
          ...
          else
          {
		    ...
		    reader->Seek( (int)( fc + cDataStart ), STREAM_SEEK_SET );
	      ...
	      {   
		    ...
		    reader->Seek( (int)( posBeforeType + cbData ), STREAM_SEEK_SET );
