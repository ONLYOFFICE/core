...
namespace DocFileFormat
{
	...
	void WordDocument::Clear()
	{
		...
		RELEASEINTERFACE(DataStream);
		RELEASEINTERFACE(TableStream);
		RELEASEINTERFACE(WordDocumentStream);
	}
	...
	long WordDocument::Parse(int fibFC)
	{
		...
		if (m_pStorage)
		{
			hr		=	m_pStorage->SetFile (m_sFileName);
		...
		hr			=	m_pStorage->GetStream (_T( "WordDocument" ), &WordDocumentStream);
		...
		{
			hr	=	m_pStorage->GetStream (L"1Table", &TableStream);
		}
		else
		{
			hr	=	m_pStorage->GetStream (L"0Table", &TableStream);
		}
		...
		{
			ATLTRACE (L"\n=========================================================\n");
			for (size_t j = 0; j < Text->size(); ++j) ATLTRACE (L"%c", Text->operator [](j));
			ATLTRACE (L"\n=======================================================\n");
		}
#endif
		if (BookmarkNames)
		...
		if ( !SectionPlex->Elements.empty() )
		{
			...
			for (unsigned int i = 0; i < SectionPlex->Elements.size(); ++i)
			{
				...
				byte* bytes		=	wordReader.ReadBytes( ( cbSepx /*- 2*/ ), true );
