...
namespace ASCDocFileFormat
{
	...
	public:
		...
		inline void SetWrap(const nullable_property<OOX::Logic::Wrap>& oWrap)
		{
			if (oWrap.is_init())
			{
				SetWrap (oWrap->Type);
		...
		inline void SetImageWrap(const nullable_property<OOX::Logic::DrawingWrap>& oWrap)
		{
			if (oWrap.is_init())
			{				
		...
		inline void SetImageUseBehind(const nullable_property<bool>& oBehindDoc)
		{
	...
	private:
		...
		inline void Init (unsigned int nSSpa)
		{
			...
			m_arProperties.push_back (Prl ((short)DocFileFormat::sprmCPicLocation, (byte*)(&nSSpa)));	//	offset spa in storage from 0
		}
