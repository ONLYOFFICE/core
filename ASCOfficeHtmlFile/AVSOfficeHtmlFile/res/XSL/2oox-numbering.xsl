...
'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'NONE')">
          ...
          <!--<xsl:variable name="StartString1" select="substring-before($start1, 'padding:')"/>
          <xsl:variable name="StartString3" select="substring-after($start1, $StartString1)"/>
          <xsl:variable name="StartString4" select="substring-before($StartString3, 'px;')"/>
          <xsl:variable name="StartString5" select="substring-after($StartString4, 'padding:')"/>-->

          <xsl:variable name="currentPadding">
            <xsl:call-template name="GetPropertyFromStyle">
          ...
          <xsl:choose>
            <xsl:when test="$currentPadding = '0' or $currentPadding = '0px'">
              <xsl:attribute name="w:left">
                <xsl:value-of select="(number($level) + 1) * 720 + 360"/>
              </xsl:attribute>
            ...
            <xsl:otherwise>
              <xsl:attribute name="w:left">
                <xsl:value-of select="(number($level) + 1) * 720 + 720"/>
              </xsl:attribute>
              <xsl:attribute name="w:hanging">
                <xsl:text>360</xsl:text>
              </xsl:attribute>
