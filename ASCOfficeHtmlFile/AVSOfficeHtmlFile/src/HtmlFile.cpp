...
bool CHtmlFile::HtmlToXHtml_htmlayout( const wchar_t *wsHtmlPath, const wchar_t *wsXHtmlPath, const MhtFile* pMhtFile, BOOL bDownloadImages )
{	
    ...
    if ( xml_doc->load( sFileName ) == VARIANT_TRUE )
    {
        ...
        while( MSXML2::IXMLDOMNodePtr child = ins_elems->nextNode() )
        {
            ...
            if ( src_attr && !skip )
            {
				...
				if ( !HtmlToXhtml::FileExist( &sPath ) )
   ...
   while( MSXML2::IXMLDOMNodePtr child = del_elems->nextNode() )
    {
		...
		bool check;
		if(_bstr_t( L"w:pageBreakBefore" ) == _bstr_t( bstrText) && _bstr_t( nextNode ) != _bstr_t( bstrText) && _bstr_t( predNode ) != _bstr_t( bstrText))
			check = true;
		else
			check = false;
...
{	
    ...
    { 
		...
		xml_doc->save( sDstFileName );

		std::wstring path(sDstFileName);
		TxtFile file(path);
		std::list<std::string> content;
		content = file.readUtf8withoutPref();
...
int CHtmlFile::getSize(MSXML2::IXMLDOMNamedNodeMapPtr attributs, const std::wstring& nameSize)
{
	if (attributs->getNamedItem(nameSize.c_str()) != 0)
	{
		return strings::toInt(attributs->getNamedItem(nameSize.c_str())->nodeValue.bstrVal);
	}
	else
	{
		std::wstring size = attributs->getNamedItem(L"style")->nodeValue.bstrVal;
		wchar_t  name [ 1024 ] = { 0 };
		swprintf( name, L"%s:", nameSize.c_str());
		return strings::toInt(strings::substring_between(size, name, L"px;"));
	}
}
...
HRESULT CHtmlFile::LoadHtmlFile( BSTR sSrcFileName, BSTR sDstPath, BSTR sXMLOptions )
{
	...
	result = HtmlToXHtml_htmlayout( sSrcFileName, xhtml_file, NULL, m_bDownloadImages );
