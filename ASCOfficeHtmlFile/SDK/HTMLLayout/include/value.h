...
EXTERN_C UINT VALAPI ValueInvoke( VALUE* pval, VALUE* pthis, UINT argc, const VALUE* argv, VALUE* pretval, LPCWSTR url);
  ...
  namespace json
  {
    class string
    {
      struct data { mutable volatile unsigned rc; unsigned length; wchar_t chars[1]; };
      data* _data;

      void init( const wchar_t* s, unsigned n )
      {
        assert(_data == 0); if(!s || !n) return;
        _data = (data*)malloc(sizeof(data) + sizeof(wchar_t) * n);
        if(_data) {  wcsncpy(_data->chars,s,n); _data->rc = 1; _data->length = n; _data->chars[n] = 0; }
      }
      void init( data* pd ) {  _data = pd; if(_data) ++(_data->rc); }
      
    public: 
      string(): _data(0) {}
      string(const wchar_t* s)  : _data(0) { init(s, (unsigned int)(s? wcslen(s):0)); }
      string(aux::wchars wc)    : _data(0) { init(wc.start,wc.length); }
      string(const string& s)   : _data(0) { init(s._data); }
      string& operator = ( const string& s ) { clear(); init(s._data); return *this; }
      string& operator = ( const wchar_t* s ) { clear(); init(s, (unsigned int)(s? wcslen(s):0)); return *this; }
      ~string() { clear(); }

      const wchar_t* c_str() const { static wchar_t z = 0; return _data?_data->chars:&z; }
            unsigned length() const { return _data?_data->length: 0; }
                void clear() { if(_data && (--(_data->rc) == 0)) { free(_data); _data = 0; } }
      
      operator const wchar_t*() const { return c_str(); }
      operator const aux::wchars() const { return aux::wchars(c_str(), length()); }
      bool operator == (aux::wchars rs) const { return aux::wchars(c_str(), length()) == rs; }
      bool operator != (aux::wchars rs) const { return aux::wchars(c_str(), length()) != rs; }
    };
    ...
    public:
     ...
     ~value()                 { ValueClear(this); }
      ...
      value( const value* arr, unsigned n )  { ValueInit(this); for( unsigned i = 0; i < n; ++i ) set(i,arr[i]); }
