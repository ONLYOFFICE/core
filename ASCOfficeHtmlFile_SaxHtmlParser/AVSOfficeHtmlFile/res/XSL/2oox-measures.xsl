...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  -->
	...
	<xsl:template name="GetUnit">
		...
		<!-- (string) The length including the unit to be returned 
	...
	<xsl:template name="ConvertToPixels">
		...
		<xsl:variable name="newlength">
			...
			<xsl:choose>
				...
				<xsl:when test="contains($length, 'mm')">
					<xsl:value-of select="format-number(substring-before($length, 'mm') div 10 div 0.0264,'#.###')"/>
				</xsl:when>
				<xsl:when test="contains($length, 'in')">
					<xsl:value-of select="format-number(substring-before($length, 'in') * 2.54 div 0.0264,'#.###')"/>
				</xsl:when>
				<xsl:when test="contains($length, 'pt')">
					<xsl:value-of
					  select="format-number(substring-before($length, 'pt') * 2.54 div 72 div 0.0264,'#.###')"/>
				</xsl:when>
				<xsl:when test="contains($length, 'twip')">
					<xsl:value-of
					  select="format-number(substring-before($length, 'twip') * 2.54 div 1440 div 0.0264,'#.###')"/>
				</xsl:when>
				<xsl:when test="contains($length, 'pica')">
					<xsl:value-of
					  select="format-number(substring-before($length, 'pica') * 2.54 div 6 div 0.0264,'#.###')"/>
				</xsl:when>
				<xsl:when test="contains($length, 'dpt')">
					<xsl:value-of
					  select="format-number(substring-before($length, 'dpt') * 2.54 div 72 div 0.0264,'#.###')"/>
				</xsl:when>
				<xsl:when test="contains($length, 'cm')">
					<xsl:value-of select="format-number(substring-before($length, 'cm') div 0.0264,'#.###')"/>
				</xsl:when>
		...
		<xsl:value-of select="concat($roundLength, 'px')"/>
	</xsl:template>
    ...
    -->
		...
		<!-- (string) The length including the unit -->
		<xsl:param name="unit"/>
		<!-- (string) The unit to be converted to -->
	...
	<xsl:template name="DegreesAngle">
		...
		<!-- (number) The angle to be converted (in radiant) -->
		<xsl:param name="select" select="'false'"/>
		<!-- (string) If set to 'true', only converts the values pi, pi/2, -pi, -pi/2 -->
		<xsl:param name="revert" select="'false'"/>
		<!-- (string) If set to 'true', returns a negated result -->

		<xsl:variable name="val">
			<!-- Contains the convertion's result  -->
			<xsl:choose>
	...
	<xsl:template name="twips-measure">
		...
		<xsl:choose>
			...
			<xsl:when test="contains($length, 'px')">
				<xsl:value-of select="round(number(substring-before($length, 'px')) * 1440 div 96)"/>
			</xsl:when>
			<xsl:when test="not($length) or $length='' ">0</xsl:when>
			<xsl:otherwise>
				<!--<xsl:value-of select="$length"/>-->
	...
	<xsl:template name="eightspoint-measure">
		...
		<xsl:choose>
			...
			<xsl:when test="contains($length, 'px')">
				<xsl:value-of select="round(number(substring-before($length, 'px')) * 576 div 96)"/>
			</xsl:when>
			<xsl:when test="not($length) or $length='' ">0</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="$length"/>
	...
	<xsl:template name="point-measure">
		...
		<xsl:variable name="newlength">
			<xsl:choose>
				...
				<xsl:when test="contains($length, 'px')">
					<xsl:value-of select="number(substring-before($length, 'px')) * 72 div 96"/>
				</xsl:when>
				<xsl:when test="not($length) or $length='' ">0</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="$length"/>
