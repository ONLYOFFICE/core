...
//default
	...
	//speed & onClick выставл€ютс€ ранее
}
...
void pptx_slide_context::process_images()
{
    ...
    BOOST_FOREACH(drawing_object_description & pic, impl_->images_)
    {
		...
		if ((pos_replaicement <0 && pos_preview <0) || pic.use_image_replace_)//оригинал, а не замен€емый объект (при наличии объекта)
		{
			...
			if (sTextContent)//в ms office на картинке нельз€ сделать надпись - мен€ем тип на рект с заливкой картинкой
			{
...
void pptx_slide_context::process_common_properties(drawing_object_description & pic,_pptx_drawing & drawing)
{
	if (pic.svg_rect_)
	{
		//todooo непон€тки с отрицательными значени€ми
...
void pptx_slide_context::serialize_objects(std::wostream & strm)
{
    ...
    CP_XML_WRITER(strm)
    {
// дефолтную об€зательную noGroup пока воткнем сюда
