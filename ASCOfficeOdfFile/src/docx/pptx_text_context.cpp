...
public:
	...
	void set_local_styles_container(odf_reader::styles_container*  local_styles_);//это если стили объектов содержатс€ в другом документе
...
private:
	...
	std::wstringstream text_; //приход€щий текст
...
/////////////lists////////////
    ...
    // счетчик дл€ нумерации имен созданных в процессе конвертации стилей
...
void pptx_text_context::Impl::write_pPr(std::wostream & strm)
{
	...
	ApplyListProperties (paragraph_properties_,level);//выравнивани€ листа накатим на свойства параграфа
...
{
	in_comment = true;//отдельна€ ветка - так как без форматировани€
}
