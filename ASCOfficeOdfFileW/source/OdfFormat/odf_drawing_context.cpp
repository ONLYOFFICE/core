...
public:
    ...
    {	
		...
		is_footer_header_ = false;  //некоторые свойства дл€ объектов графики не поддерживаюис€ в редакторах Liber && OpenOffice.net
	...
	std::vector<odf_drawing_state>		drawing_list_;	//все элементы(кроме групп) .. дл€ удобства разделение по "топам"
...
void odf_drawing_context::end_drawing()
{
	...
	if (draw)
	{
		...
		if (impl_->current_drawing_state_.rotateAngle)
		{
			...
			//так как вращени€ все в мс относительно центра фигуры, а не от начала координат - убираем смещение
...
///////////////////////////////////////////////////////
	...
	{
		//не поддерживаетс€ :( - нужно считать искажени€ на простейшие фигуры - линии, ректы, эллипсы 
	}
...
void odf_drawing_context::end_shape()
{
	...
	//вторичные, вычисл€емые свойства шейпов
		...
		if (enhanced)
		{
			...
			else
			{
				...
				if (shape_define)
				{
					...
					else // об€зательно нужны дефолтовые
...
void odf_drawing_context::set_solid_fill(std::wstring hexColor)
{
	...
	case Area:
		...
		//последнее нужно - что если будут вводить текст - под текстом будет цвет фона (или он помен€етс€ в пол€х текста)
		break;
	case Line:
...
{
	//хороший тон сохранить все размеры в см (хот€ можно и в другой системе)
	if (x_pt)
	{
		impl_->x = *x_pt;
...
{
	if (impl_->current_drawing_state_.elements_.size() < 1) return;
	//добавить в стиль ссыль на базовый стиль Frame - зачемто нужно дл€ таблиц которые не инлайн 
...
void odf_drawing_context::finalize(office_element_ptr & root_elm)//дл€ прив€зки 
{
