...
unsigned long CIDFontDefLoadFontData(FontDef pFontDef, std::wstring  wsFilePath, CIDFontFileType eCIDType)
{
	...
	if ( wsFilePath.length() < 1 )return S_OK;
	
	CFile oFile;
	if (oFile.OpenFile(std_string2string(wsFilePath)) != S_OK) 
	...
	StreamWrite( pAttr->pFontData, fileContent, fileSize, true );
...
FontDict Type0FontNew  (MMgr oMMgr, FontDef pFontDef, EncoderRecPtr pEncoder, Xref pXref, PToUnicode pToUnicode/* = NULL*/)
{
	...
	if ( FontDefCID == pFontDef->eType ) 
	{
        pAttr->pDescendantFont = CIDFontType0New( pFont, pXref );
