...
static void FileWrite(StreamRecPtr pStream, char *sData, int nLen) 
{
...
static unsigned short ReadULong         (BYTE *pBuffer, unsigned int unLen = 4)
{
    ...
    unLen = (std::max)( (unsigned int)0, (std::min)( (unsigned int)4, unLen ) );
	if ( 4 == unLen )
	{
		ulValue = pBuffer[0];
...
static unsigned long GetUINT32          (StreamRecPtr  pStream, unsigned int   *pnValue)
{
...
static unsigned long GetUINT16          (StreamRecPtr  pStream, unsigned short *pnValue)
{
...
static unsigned long GetINT16           (StreamRecPtr  pStream, short          *pnValue)
{
...
static unsigned long WriteUINT32        (StreamRecPtr  pStream, unsigned int   nValue)
{
...
static unsigned long WriteUINT16        (StreamRecPtr  pStream, unsigned short nValue)
{
...
static unsigned long WriteINT16         (StreamRecPtr  pStream, short          nValue)
{
    short nTemp = nValue;
...
static unsigned long LoadTTFTable       (FontDef pFontDef)
{
	...
	CFile oFile;
	if (oFile.OpenFile(pAttr->wsFilePath) != S_OK) 
		return AVS_OFFICEPDFWRITER_ERROR_FILE_OPEN_ERROR;		

	ULONG64  fileSize	 = oFile.GetFileSize();
	...
	oFile.ReadFile(fileContent, fileSize);
	oFile.CloseFile();
	
	BYTE *pPointer = fileContent;
    ...
    if ( 0 == UtilsMemCmp( anTag, (BYTE*)"ttcf", 4 ) )
	{
		...
		pPointer = fileContent + ulOffset;
	}

	pAttr->oOffsetTable.nSFNTVersion   = ReadULong( pPointer );  pPointer += 4;
	pAttr->oOffsetTable.nNumTables     = ReadUShort( pPointer ); pPointer += 2;
	pAttr->oOffsetTable.nSearchRange   = ReadUShort( pPointer ); pPointer += 2;
	pAttr->oOffsetTable.nEntrySelector = ReadUShort( pPointer ); pPointer += 2;
	pAttr->oOffsetTable.nRangeShift    = ReadUShort( pPointer ); pPointer += 2;
	...
	{
		pTable->anTag[0]  = (char)pPointer[0]; pPointer++;
		pTable->anTag[1]  = (char)pPointer[0]; pPointer++;
		pTable->anTag[2]  = (char)pPointer[0]; pPointer++;
		pTable->anTag[3]  = (char)pPointer[0]; pPointer++;
		pTable->nCheckSum = ReadULong( pPointer ); pPointer += 4;
		pTable->nOffset   = ReadULong( pPointer ); pPointer += 4;
		pTable->nLength   = ReadULong( pPointer ); pPointer += 4;
		pTable++;
	}
...
static unsigned long GetTTFTable        (FontDef pFontDef, FT_Face pFace, const char *sTag, BYTE **ppPointer, unsigned long *ulLength)
{
	...
	if ( NULL == pBuffer )
		return SetError( pFontDef->oError, AVS_OFFICEPDFWRITER_ERROR_NOT_ENOUGH_MEMORY, 5);
...
static unsigned long RecreateGLYF       (FontDef pFontDef, unsigned int *pnNewOffsets, StreamRecPtr pStream)
{
...
static unsigned long RecreateName       (FontDef pFontDef, StreamRecPtr pStream)
{
    TTFontDefAttrRecPtr pAttr = (TTFontDefAttrRecPtr)pFontDef->pAttr;
    TTFTable *pTable = FindTable( pFontDef, "name");
    unsigned long nRet = OK;

	StreamRecPtr pTempStream = MemStreamNew( pFontDef->oMMgr, STREAM_BUF_SIZ);
...
static unsigned long RecreateCMap       (FontDef pFontDef, StreamRecPtr pStream, PToUnicode pToUnicode)
{
	return OK;
    //TTFontDefAttrRecPtr pAttr = (TTFontDefAttrRecPtr)pFontDef->pAttr;
...
static unsigned long RecreateFPGM       (FontDef pFontDef, StreamRecPtr pStream) 
{
	// Ничего не делаем
    return OK;
}
static unsigned long WriteHeader        (FontDef pFontDef, StreamRecPtr pStream, unsigned int *pnCheckSum)
{
    TTFontDefAttrRecPtr pAttr = (TTFontDefAttrRecPtr)pFontDef->pAttr;
	unsigned long nRet = OK;
...
static unsigned long CreateDescriptor  (FontDict pFont)
{
    ...
    TTFontDefAttrRecPtr pDefAttr = (TTFontDefAttrRecPtr)pDef->pAttr;
...
static unsigned long TTFOnWrite        (Dict pObj, StreamRecPtr pStream)
{
...
FontDef        TTFontDefNew         (MMgr oMMgr)
{
    ...
    TTFontDefAttrRecPtr pFontDefAttr = (TTFontDefAttrRecPtr)GetMem( oMMgr, sizeof(TTFontDefAttrRec) );
    if ( !pFontDefAttr ) 
	{
...
FontDef        TTFontDefLoad        (MMgr oMMgr, std::wstring & wsFilePath, bool bEmbedding, const char *sEncodingName/* = NULL*/, bool bEncoding/* = false*/, bool bSymbolic/* = false*/, bool bUsesMacRomanEncoding/* = false*/, bool bFastParse /*= false*/)
{
    ...
    memcpy(pAttr->wsFilePath, wsFilePath.c_str(), wsFilePath.length() * sizeof (wchar_t));
	pAttr->unFontIndex           = 0;
...
#if defined(_WIN32) || defined (_WIN64)
	USES_CONVERSION;
	sPath = W2A( pAttr->wsFilePath  );
#else
    std::wstring unicodeStr(pAttr->wsFilePath);
...
FontDict TTFontNew  (MMgr oMMgr, FontDef pFontDef, EncoderRecPtr pEncoder, Xref pXref, PToUnicode pToUnicode /*= NULL*/)
{
    ...
    TTFontDefAttrRecPtr pFontDefAttr = (TTFontDefAttrRecPtr)pFontDef->pAttr;
