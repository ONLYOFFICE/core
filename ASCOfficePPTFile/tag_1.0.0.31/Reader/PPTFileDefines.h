...
#ifdef _DEBUG
#define		PPT_DUMP_LOG		TRUE		// TRUE - чтоб в дебаге скидывались xml с инфой
#else
#define     PPT_DUMP_LOG		FALSE		// вот это не меняем никогда
#endif

#define		PPT_DEFAULT_SLIDE_DURATION						30000
/////////////////////////////////////////////////////////////////////////
...
namespace NSPPT
{
	...
	void AddToSimpleMap(CSimpleMap<TKey, TVal>* pMap, TKey key, TVal val)
	{
		if (NULL == pMap)
			return;

		int nIndexKey = pMap->FindKey(key);
		...
		{
			pMap->SetAtIndex(nIndexKey, key, val);
		}
	}

	template <typename TKey, typename TVal>
	void AppendSimpleMap(CSimpleMap<TKey, TVal>* pMapDst, CSimpleMap<TKey, TVal>* pMapSrc)
	{
		if ((NULL == pMapDst) || (NULL == pMapSrc))
			return;

		for (int nIndex = 0; nIndex < pMapSrc->GetSize(); ++nIndex)
		{
			TKey key = pMapSrc->GetKeyAt(nIndex);
			TVal val = pMapSrc->GetValueAt(nIndex);

			AddToSimpleMap(pMapDst, key, val);
		}
	}
}