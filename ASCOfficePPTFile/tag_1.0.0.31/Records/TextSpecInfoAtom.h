...
public:
	...
	void CRecordTextSpecInfoAtom::ReadFromStream(SRecordHeader oHeader, IStream* pStream)
	{
		...
		{
			m_arrSIs.Add();
			m_arrSIs[lCountItems].ReadFromStream(pStream);
	...
	{
		for (size_t nIndex = 0; nIndex < m_arrSIs.GetCount(); ++nIndex)
		{
			pTextAttributes->m_arSIs.Copy(m_arrSIs);
			break;
		}
	}
