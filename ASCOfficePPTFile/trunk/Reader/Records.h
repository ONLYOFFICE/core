...
{ 
	...
	void ToXmlWriter(XmlUtils::CXmlWriter* pWriter, IStream* pStream);
...
public:
	// читаем из файла
	virtual ~IRecord(){}
	virtual void ReadFromStream(SRecordHeader oHeader, IStream* pStream) = 0;
...
{
protected:
	CSimpleArray<IRecord*> m_arRecords;
...
public:
	...
	void Clear()
	{
		size_t nCount = m_arRecords.GetSize();
		while(0 != nCount) 
		{ 
			if (NULL != m_arRecords[0])
				delete m_arRecords[0]; 
			
			m_arRecords.RemoveAt(0);
			--nCount;
		} 
	}
	...
	template <typename T>
	void GetRecordsByType(CSimpleArray<T>* pArray, bool bIsChild, bool bOnlyFirst = false)
	{
		if (NULL == pArray)
			return;
