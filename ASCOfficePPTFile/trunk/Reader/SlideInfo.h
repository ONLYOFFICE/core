...
public:
	...
	CAtlMap<LONG, CElementInfo>		m_mapElements;
...
public:
	...
	{
		m_arTextPlaceHolders.Copy(oSrc.m_arTextPlaceHolders);
		...
		m_mapElements.RemoveAll();
		POSITION pos = m_mapElements.GetStartPosition();
		while (pos != NULL)
		{
			CAtlMap<LONG, CElementInfo>::CPair* pPair = m_mapElements.GetNext(pos);
			m_mapElements.SetAt(pPair->m_key, pPair->m_value);
		}
