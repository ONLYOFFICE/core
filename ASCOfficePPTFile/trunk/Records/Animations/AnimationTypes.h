...
inline int __cdecl sort (const long* a, const long* b) { return *a > *b ? 1 : -1; }

namespace debug
{
	inline static void tracearrf (CAtlArray<double>& arr)
	{
...
namespace Animations
{
	...
	public:
		...
		{
			for ( long i = 0; i < (long)rgParaBuildLevel.GetCount (); ++i )
		...
		virtual void ReadFromStream ( SRecordHeader oHeader, IStream* pStream )
		{
			...
			if ( SUCCEEDED ( pStream->Read ( &ReadHeader, sizeof ( SRecordHeader ), NULL ) ) )
				buildAtom.ReadFromStream ( ReadHeader, pStream );

			if ( SUCCEEDED ( pStream->Read ( &ReadHeader, sizeof ( SRecordHeader ), NULL ) ) )
				paraBuildAtom.ReadFromStream ( ReadHeader, pStream );
	...
	public:
		...
		CAtlArray <ParaBuildLevel*>	rgParaBuildLevel;
	};
	...
	public:
		...
		{
			for ( long i = 0; i < (long)rgChildRec.GetCount (); ++i )
		...
		virtual void ReadFromStream ( SRecordHeader oHeader, IStream* pStream )
		{
			...
			while ( lCurLen < m_oHeader.RecLen )
			{
				if ( FAILED ( pStream->Read ( &header, sizeof ( SRecordHeader ), &lReadLen ) ) )
					break;
	...
	public:
		...
		CAtlArray <ParaBuildContainer*>	rgChildRec;
...
namespace Animations
{
	...
	struct TimeVariantString : public TimeVariant
	{
		...
		{	
			...
			CDirectory::WriteValueToNode ( _T("Type"),		Helpers::GetTimeVariantTypeEnum ( m_Type ), &oWriter );
			CDirectory::WriteValueToNode ( _T("StringValue"),	CString ( CW2A ( stringValue ) ), &oWriter );
	...
	struct TimeStringListContainer : public IRecord
	{
		...
		{	
			...
			{
				CDirectory::WriteValueToNode ( _T("Value"),	CString ( CW2A ( m_Values [i].stringValue ) ), &oWriter );
			}
...
namespace Animations
{
	...
	struct TimeEffectBehaviorContainer : public IRecord
	{
		...
		{	
			...
			if ( effectBehaviorAtom.m_bTypePropertyUsed )
				CDirectory::WriteValueToNode ( _T("TransitionEffect"),	CString ( ( CW2A ( m_varType.stringValue ) ) ), &oWriter );
			if ( effectBehaviorAtom.m_bProgressPropertyUsed )
				CDirectory::WriteValueToNode ( _T("VarProgres"),	Helpers::DoubleToString ( (DOUBLE)m_varProgres.m_Value ), &oWriter );
			if ( effectBehaviorAtom.m_bRuntimeContextObsolete )
				CDirectory::WriteValueToNode ( _T("VarRuntimeContext"),	CString ( CW2A ( m_varRuntimeContext.stringValue ) ), &oWriter );
...
namespace Animations
{
	...
	struct TimeMotionBehaviorContainer : public IRecord
	{
		...
		{	
			...
			CDirectory::WriteValueToNode ( _T("VarPath"), CString ( CW2A ( m_VarPath.stringValue ) ), &oWriter );
...
namespace Animations
{
	...
	struct TimeAnimationValueListContainer : public IRecord
	{
		...
		{	
			...
			for ( int i = 0; i < m_arrEntry.GetSize(); ++i )
			{
				CDirectory::WriteValueToNode ( _T("value"),	CString ( CW2A ( m_arrEntry[i].m_VarValue.stringValue ) ), &oWriter );
				CDirectory::WriteValueToNode ( _T("formula"), CString ( CW2A ( m_arrEntry[i].m_VarFormula.stringValue ) ), &oWriter );
			}
...
namespace Animations
{
	...
	public:
		virtual void ReadFromStream ( SRecordHeader oHeader, IStream* pStream )
		{
			...
			pStream->Read ( &colorBy, sizeof ( TimeAnimateColorBy ), NULL );
			pStream->Read ( &colorFrom, sizeof ( TimeAnimateColor ), NULL );
			pStream->Read ( &colorTo, sizeof ( TimeAnimateColor ), NULL );
		}
...
{	
	...
	public:
		...
		virtual ~ExtTimeNodeContainer()
		{
			...
			for ( long i = 0; i < (long)rgExtTimeNodeChildren.GetCount (); ++i )
				RELEASEOBJECT ( rgExtTimeNodeChildren[i] );

			for ( long i = 0; i < (long)timeCondition.GetCount (); ++i )
				RELEASEOBJECT ( timeCondition[i] );

			for ( long i = 0; i < (long)rgTimeModifierAtom.GetCount (); ++i )
			...
			while ( lCurLen < m_oHeader.RecLen )
			{
				...
				if ( RT_TimeExtTimeNodeContainer	==	nRecord )
				{
					...
					if (pContainer)
					{
						...
						rgExtTimeNodeChildren.Add ( pContainer );
	...
	public:
		...
		CAtlArray <IRecord*>					timeCondition;
		//	rgBeginTimeCondition
		//	rgEndTimeCondition
		//	timeEndSyncTimeCondition
		CAtlArray<TimeModifierAtom*>			rgTimeModifierAtom;
		SlaveContainer*							rgSlave;						//	OPTIONAL
		CAtlArray <ExtTimeNodeContainer*>		rgExtTimeNodeChildren;
...
namespace Animations
{
	...
	typedef CAtlArray <Effect> Effects;
...
{	
	...
	public:
		...
		inline void Push ( double dTime, int nType )
		{
			...
			m_arrEffects.Add ( m_oTop );

			m_nDuration			=	__max ( m_nDuration, m_oTop.dTime + m_oTop.dDuration );
		...
		inline void Pop ()
		{
			...
			ATLTRACE ( _T("Pop \n") );	
#endif
			if ( (int)m_arrEffects.GetCount () )
				m_arrEffects.RemoveAt ( m_arrEffects.GetCount () - 1 );						

			if ( (int)m_arrEffects.GetCount () )
			{
				m_oTop				=	m_arrEffects.GetAt ( m_arrEffects.GetCount () - 1 );
			}
			else
			{
	...
	private:
		...
		double					m_dTimeLine;
		double					m_dTimeDelay;
		CAtlArray <EffectTime>	m_arrEffects;
	...
	public:
		...
		{
			for ( long i = 0; i < (long)m_arrBuilds.GetCount(); ++i )
			{
	...
	private:
		...
		inline BOOL FindBuildList ( ExtTimeNodeContainer* pContainer )
		{
			...
			{
				for ( long i = 0; i < (long)pContainer->buildList->rgChildRec.GetCount(); ++i )
					m_arrBuilds.Add(EffectBuild(pContainer->buildList->rgChildRec[i]->buildAtom.shapeIdRef,	pContainer->buildList->rgChildRec[i]->paraBuildAtom.buildLevel) );

				return TRUE;
			}

			for ( int i = 0; i < (int)pContainer->rgExtTimeNodeChildren.GetCount (); ++i )
			{
	...
	private:
		...
		CAtlArray<EffectBuild>	m_arrBuilds;
	};
...
#define MIN_SLIDE_TIME	5000.0
	...
	private:
		...
		{	
			...
			if (nNodeType)
			{
				if (CNodeTiming::MainSequenceNode != nNodeType && CNodeTiming::TimingRootNode != nNodeType)
				{
					...
					if ( pContainer->timeCondition.GetCount () )
					{
					...
					if (pContainer->rgTimeModifierAtom.GetCount())
					{
						if (pContainer->GetModifier(3))
							m_oTopEffect.m_dTimeAccel			=	max(min(pContainer->GetModifier(3)->value,1.0),0.0);

						if (pContainer->GetModifier(4))
							m_oTopEffect.m_dTimeDecel			=	max(min(pContainer->GetModifier(4)->value,1.0),0.0);
					}
			...
			if ( -1 != nID )
			{
				...
				// ATLTRACE ( _T("dbg - NodeType : %d, count : %d\n"), nNodeType, (int)pNode->rgExtTimeNodeChildren.GetCount () );
#endif
			}				
		...
		inline void CalculateTimeEffect (ExtTimeNodeContainer* pContainer)
		{
			...
			{
				if ((long)pContainer->rgExtTimeNodeChildren.GetCount())
				{
			...
			if ((int)m_arDurations.GetCount () > 1)
			{
			...
			{
				if ((int)m_arDurations.GetCount () > 5)
					m_dEffectDuration	=	 m_arDurations[1]	+ m_arDurations[5];
			}

			if (26 == m_oTopEffect.m_nEffectID)
			{
				if ((long)m_arDurations.GetCount () && (long)m_arBeginTimes.GetCount () )
					m_dEffectDuration	=	m_arDurations.GetAt(m_arDurations.GetCount ()-1) + m_arBeginTimes.GetAt(m_arBeginTimes.GetCount ()-1);
			}

			if ( 32 == m_oTopEffect.m_nEffectID && 3 == m_oTopEffect.m_nEffectType )		//	Teeter Effect
			{
				if ((int)m_arDurations.GetCount ())
			...
			{
				if ((int)m_arDurations.GetCount () >= 2)
					m_dEffectDuration	=	 m_arDurations[0];
			}
		...
		inline BOOL MediumEffectDuration (ExtTimeNodeContainer* pContainer)
		{
			...
			{
				m_arDurations.Add ( pContainer->timeNodeAtom.m_nDuration );
		...
		inline BOOL MediumEffectTimeDelay (ExtTimeNodeContainer* pContainer)
		{
			if ( pContainer->timeNodeAtom.m_bDurationProperty )
			{
				if ( pContainer->timeCondition.GetCount () )
				{
					m_arBeginTimes.Add ( static_cast<TimeConditionContainer*> ( pContainer->timeCondition[0] )->m_oTimeConditionAtom.m_nTimeDelay );	
				}
		...
		inline void ReadPropertyIgnoreShape (const TimeBehaviorContainer& container)
		{
			if (2 == container.clientVisualElement.m_oVisualShapeAtom.m_Type)										//	указывает на то что анимация применяется к тексту
			{
				...
				for (long i = 0; i < (long)m_arParIndexer.GetCount(); ++i)
				{
					if (m_oTopEffect.m_nTextSequence == m_arParIndexer.GetAt(i))
						return;
				}

				m_arParIndexer.Add (m_oTopEffect.m_nTextSequence);
			}
		}

		inline BOOL UpdateParagraph ()
		{
			if ((long)m_arParIndexer.GetCount())
			{
				//ATLTRACE ( _T("====================PARAGRAPH==========================================\n"));
				qsort ( m_arParIndexer.GetData(), m_arParIndexer.GetCount(), sizeof (long), (int(*)(const void*, const void*))sort );

				for (long j = 0; j < (long)m_oAnimation.GetSize(); ++j)
				{
					Effects* arEffects = m_oAnimation.GetValueAt(j);
					if (arEffects)
					{
						for (long i = 0; i < (long)arEffects->GetCount(); ++i)
						{
							Effect& oEffect = arEffects->GetAt(i);
							if (oEffect.m_nTextSequence >= 0)
							{
								for (long ind = 0; ind < (long)m_arParIndexer.GetCount(); ++ind)
								{
									if (oEffect.m_nTextSequence == m_arParIndexer.GetAt(ind))
									{
		...
		inline int GetMediaID (ExtTimeNodeContainer* pContainer)
		{
			...
			for (long i = 0; i < (long)pContainer->rgExtTimeNodeChildren.GetCount (); ++i )
			{
		...
		inline BOOL GetAttachedShapeToVideo (ExtTimeNodeContainer* pContainer)
		{
			if (TL_TNT_Media == pContainer->GetNodeType ())																//	нод типа Media
			{
				if (pContainer->clientVisualElement)
				{
					m_arrMedia.Add (pContainer->clientVisualElement->m_oVisualShapeAtom.m_nObjectIdRef);

					if (2 == (int)m_arrMedia.GetCount())																//	у видео есть статическое изображение
						return TRUE;
				}
			}
	...
	virtual void ReadFromStream (SRecordHeader oHeader, IStream* pStream)
	{
		...
		if ( RECORD_PROG_BINARY_TAG == m_oHeaderChild.RecType )
		{
			...
			if ( CStringW ( L"___PPT10" ) == tagName )
			{
				...
				pStream->Read (&rhData, sizeof (SRecordHeader), NULL);
