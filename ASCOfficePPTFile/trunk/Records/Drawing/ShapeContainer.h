...
public:
	...
	inline void SetUpPropertyShape(CShapeElement* pElement, CTheme* pTheme, CSlideInfo* pInfo, CSlide* pSlide, CProperty* pProperty)
	{
		...
		case NSOfficeDrawing::fillBlip:
			{
				...
				int nIndex	= pParentShape->m_oBrush.TexturePath.ReverseFind(WCHAR('\\'));
		...
		case NSOfficeDrawing::fillOpacity:
			{
				//pElemProps->SetAt(CElementProperty::epBrushAlpha1, min(255, CDirectory::NormFixedPoint(pProperty->m_lValue, 255)));
				pParentShape->m_oBrush.Alpha1 = (BYTE)min(255, CDirectory::NormFixedPoint(pProperty->m_lValue, 255));
				break;
			}
		case NSOfficeDrawing::fillBackOpacity:
			{
				pParentShape->m_oBrush.Alpha2 = (BYTE)min(255, CDirectory::NormFixedPoint(pProperty->m_lValue, 255));
				//pElemProps->SetAt(CElementProperty::epBrushAlpha2, min(255, CDirectory::NormFixedPoint(pProperty->m_lValue, 255)));
				break;
			}
		...
		case NSOfficeDrawing::lineOpacity:
			{
				...
				pParentShape->m_oPen.Alpha = (BYTE)min(255, CDirectory::NormFixedPoint(pProperty->m_lValue, 255));
				break;
			}
		...
		*ppElement = NULL;
			...
			default:
				{
					...
					if (TRUE)//if (/*отключим пока кастом*/OOXMLShapes::sptCustom != pShape->m_oShape.m_eType)
					{
						...
						{
							pShape->m_oShape.m_oBrush.TexturePath = pTextureInfo->m_strFilePath;
						}
		...
		if (NULL != pShapeElem)
		{
			...
			if (0 < oArrayTextHeader.GetSize())
			{
				...
				oInfo.m_lMasterTextType							= oArrayTextHeader[0]->m_nTextType;
			}
			else
			{
				...
				oInfo.m_lMasterTextType							= NSOfficePPT::NoPresent;
			}
			...
			{
				oInfo.m_lPersistIndex = oArrayTextRefs[0]->m_nIndex;
			}
			...
			if (0 < oArrayPlace.GetSize())
			{
				...
				oInfo.m_lMasterPlaceholderType = pElem->m_lPlaceholderType;
			...
			{
				oInfo.m_pStream				= m_pStream;
				oInfo.m_lOffsetTextStyle	= oArrayTextStyle[0]->m_lOffsetInStream;
			}
			...
			{
				oInfo.m_pStream				= m_pStream;
				oInfo.m_lOffsetTextProp		= oArrayTextProp[0]->m_lOffsetInStream;
			}
			...
			pSlideWrapper->m_mapElements.SetAt(pShapeElem->m_lID, oInfo);
...
protected:
	...
	void SetUpTextStyle(CString& strText, CTheme* pTheme, CLayout* pLayout, IElement* pElem, CSlideInfo* pThemeWrapper, CSlideInfo* pSlideWrapper, CSlide* pSlide = NULL)
	{
		...
		CAtlMap<LONG, CElementInfo>::CPair* pPair = pSlideWrapper->m_mapElements.Lookup(pShape->m_lID);
		if (NULL != pPair)
			oElemInfo = pPair->m_value;

		//  persist ----------------------------------------------------------------------
		CAtlArray<CTextFullSettings>* pArrayPlaseHolders	= &pSlideWrapper->m_arTextPlaceHolders;
		LONG lCountPersistObjects							= (LONG)pArrayPlaseHolders->GetCount();
		LONG lPersistIndex									= oElemInfo.m_lPersistIndex;

		if ((lPersistIndex >= 0) && (lPersistIndex < lCountPersistObjects))
		{
			CTextFullSettings* pSettings = &pArrayPlaseHolders->GetAt(lPersistIndex);
		...
		if (NULL != oElemInfo.m_pStream)
		{
			...
			if (-1 != oElemInfo.m_lOffsetTextStyle)
			{
				...
				oElemInfo.m_pStream->Read(&oHeader, sizeof(oHeader), NULL);
		...
		if ((_T("") != strText) && 0 == pTextSettings->m_arParagraphs.GetCount())
		{
			...
			oArrayPF.Add();
			oArrayPF[0].m_lCount = strText.GetLength();
			oArrayPF[0].m_lLevel = 0;
			CAtlArray<CTextCFRun_ppt> oArrayCF;
			oArrayCF.Add();
			oArrayCF[0].m_lCount = oArrayPF[0].m_lCount;
		...
		for (LONG nIndexRange = 0; nIndexRange < lCountHyper; ++nIndexRange)
		{
			...
			for (size_t nIndexPar = 0; nIndexPar < nCountPars; ++nIndexPar)
			{
				...
				for (size_t nIndexSpan = 0; nIndexSpan < pParagraph->m_arSpans.GetCount(); ++nIndexSpan)
				{
					...
					LONG lStart_	= max(lStart, lCurrentStart);
					LONG lEnd_		= min(lEnd, lCurrentEnd);
