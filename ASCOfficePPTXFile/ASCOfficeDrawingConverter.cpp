...
HRESULT CAVSOfficeDrawingConverter::SaveObject(SAFEARRAY* pBinaryObj, LONG lStart, LONG lLength, BSTR bsMainProps, BSTR* bsXml)
{
	...
	/*
	CString strId = _T("");
	strId.Format(_T("<wp:docPr id=\"%d\" name=\"\"/>"), m_oReader.GenerateNextId());
	strMainProps += strId;
	*/
	...
	BYTE typeRec1 = m_oReader.GetUChar(); // must be 0;
	LONG _e = m_oReader.GetPos() + m_oReader.GetLong() + 4;

	m_oReader.Skip(5); // type record (must be 1) + 4 byte - len record
	PPTX::Logic::SpTreeElem oElem;
