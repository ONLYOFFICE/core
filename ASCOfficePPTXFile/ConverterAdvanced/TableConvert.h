...
{
	int TableCellTextHeight(const PPTX::Logic::TableCell& pTableCell, const int WidthInEmu, const PPTX::Logic::ShapeProperties& TextProps)
	{
				...
				else
				{
					...
					for (size_t nIndexRun = 0; nIndexRun < nCountRun; ++nIndexRun)
					{
						...
						if(j->is<PPTX::Logic::Run>())
						{
							...
							LONG lTextIndex = -1;
							CStringA strA = (CStringA)TextProps.GetRunFont(level, CurrentRun.rPr, CurrentParagraph.pPr, lTextIndex);
	...
	int TableCellHeight(const PPTX::Logic::TableCell& pTableCell, const TableCellData& CellData, const PPTX::Logic::ShapeProperties& TextProps)
	{
	...
	int TableCellConvert(const PPTX::Logic::TableCell& pTableCell, CAtlArray<NSPresentationEditor::IElement*>& pElements, NSPresentationEditor::CShapeElement& lpShapeElement, const PPTX::Logic::TableStyle& Style, const TableCellData& CellData, NSPresentationEditor::CTheme* pTheme)
	{
		int result = CellData.height;
		...
		if(pTableCell.TxBody.is_init())
		{
			...
			if((ShapeText != _T("")) && (ShapeText != _T("\n")))
				TextParse(pTableCell.TxBody.get(), lpShapeElement.m_oShape.m_oText, ShapeProps, 1.0, 0.0, pTheme);
	...
	void TableConvert(const PPTX::Logic::GraphicFrame& pOle, CAtlArray<IElement*>& pElements, NSPresentationEditor::CTheme* pTheme)
	{
			...
			for(int j = 0; j < (int)pOle.Table->TableRows[i].Cells.GetCount(); ++j)
			{
				...
				if(!cell.HMerge.get_value_or(false))// && (!cell.VMerge.get_value_or(false)))
				{
					...
					if(!cell.VMerge.get_value_or(false))
					{
						...
						int res = TableCellConvert(cell, pElements, *TableOfCells[num], Style, CellData, pTheme);
		...
		for(int i = 0; i < CellData.rowsCount; i++)
		{
			for(int j = 0; j < (int)pOle.Table->TableRows[i].Cells.GetCount(); ++j)
			{
				...
				if((!cell.HMerge.get_value_or(false)) && (!cell.VMerge.get_value_or(false)))
				{
					...
					TableOfCells[num]->NormalizeCoords(c_dScaleElements, c_dScaleElements);
					TableOfCells[num]->m_oShape.m_rcBounds	= TableOfCells[num]->m_rcBounds;
					pElements.Add(TableOfCells[num]);
				}
			}
			y += Heights[i];
		}
