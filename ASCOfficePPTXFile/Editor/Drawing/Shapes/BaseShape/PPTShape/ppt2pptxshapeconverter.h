...
namespace NSGuidesVML
{
	...
	public:	
				...
				if (pHnPoint.position != _T(""))
				{
					...
					if (pHnPoint.polar != _T(""))
					{
						...
						if (oHandle.gdRefType.y == ptAdjust )
						{
							...
							CString strNewFmla = GetValue (m_lIndexDst, ptFormula, true);
							CString strOldFmla = GetValue (oHandle.gdRef.y, oHandle.gdRefType.y, false);
		...
		void ConvertTextRect ( CString strRect)
		{
			if (strRect == _T(""))
				return;
	...
	private:
		...
		CString GetXML (CString strFmlaNum, CString strSign, CString strFrmla)
		{
		...
		CString ConvertProd (CString strParam1, CString strParam2, LONG lParam3)
		{				
			LONG lRes;
			if ('#' == strParam2[0])
			{					
				lRes = (LONG)XmlUtils::GetInteger(strParam2.Mid(1));
		...
		CString CreateHandle ( SHandle oHnd )
		{
			CString strRes = _T("");
		...
		LONG GetHandlePos ( CString strParam, CString strBase, LONG lSize)
		{
			LONG lRes;
			CString strSize;
			CString strIndex;
		...
		LONG GetHandleValue(CString strParam, LONG lVal, ParamType& ptType)
		{
			ptType = ptValue;
			if ('#' == strParam[0])
			{
...
namespace NSGuidesVML
{
	...
	public:	
		...
		void ConvertPath(CString strPath, const NSPresentationEditor::CPath& oPath)
		{
			...
			for (int nIndex = 0; nIndex < nSizeArr; ++nIndex)
			{
				...
				if (!bStroke)
					m_oPathRes.WriteAttribute(_T("stroke"), (CString)_T("false"));
				if (!bFill)
			...
			for (int i = 0; i < nHandlesCount; ++i)
			{
				...
				if (pHnPoint.position != _T(""))
				{
					...
					if (pHnPoint.polar != _T(""))
					{
						...
						{
							//угол должен храниться в pptx формате, поэтому придётся его везде заменить
							CString strNewFmla = GetValue2(m_lIndexDst, ptFormula, true);
							CString strOldFmla = GetValue2(oHandle.gdRef.y, oHandle.gdRefType.y, false);
							LONG nIndex = m_lIndexDst;								
	...
	private:
		...
		void ConvertProd(const CString& strParam1, const CString& strParam2, const LONG& lParam3, NSBinPptxRW::CXmlWriter& oWriter)
		{
			oWriter.m_oWriter.AddSize(g_guide_max_len);
			oWriter.m_oWriter.AddStringNoCheck(g_guide_string1, g_guide_string1_len);
			oWriter.m_oWriter.AddIntNoCheck(m_lIndexDst);	
		...
		LONG GetHandlePos(const CString& strParam, const CString& strBase, LONG lSize)
		{
			LONG lRes;
			CString strSize;
			CString strIndex;
		...
		LONG GetHandleValue(const CString& strParam, const LONG& lVal, ParamType& ptType)
		{
			ptType = ptValue;
			if ('#' == strParam[0])
			{
