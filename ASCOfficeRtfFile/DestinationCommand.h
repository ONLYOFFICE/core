...
public: bool m_bHeader; //чтобы отличать заголовок от вложенных групп
...
//для разбивки на TextItem
private: CAtlArray< ITextItemPtr > aCellRenderables;
private: CAtlArray< int > aItaps; //вложенность параграфов
private: CAtlArray< RtfTableCellPtr > aCells;
private: CAtlArray< int > aCellItaps; //вложенность cell
private: CAtlArray< RtfTableRowPtr > aRows;
private: CAtlArray< int > aRowItaps; //вложенность row
private: int nCurItap;
private: RtfRowProperty oCurRowProperty;

private: bool m_bRaiseEvents;// посылать ли ивенты
private: RtfReader* m_oReader;
private: bool m_bPar;// если последняя команда была par, то не надо добавлять параграф
public: int nTargetItap;//уровень который считается не таблицей ( для того чтобы читать параграфы в таблицах )
...
private: void AddItem( RtfParagraphPtr oItem, RtfReader& oReader, bool bEndCell, bool bEndRow )
		 {
			...
			else
			{
				...
				else
				{
					...
					aCellRenderables.Add( oItem ); //содержит все параграфы, не разложенные по cell
					aItaps.Add( nCurItap ); //содержит все номера вложенности параграфов
...
public: void Finalize( RtfReader& oReader ) // потому что это не reader и нужно как-то загонять последний параграф
		{
			if( false == m_bPar )
			{
