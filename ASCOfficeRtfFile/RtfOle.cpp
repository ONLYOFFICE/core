...
CString RtfOle::RenderToOOXOnlyOle(RenderParameter oRenderParameter)
{
		...
		sResult.AppendFormat( _T(" ProgID=\"%s\""), m_sOleClass );
		...
		sResult.AppendFormat( _T(" ObjectID=\"%s\""), poDocument->m_oIdGenerator.Generate_OleId() );
		...
		sFilenameRels.AppendFormat( _T("embeddings/oleObject%d.%s"), poDocument->m_oIdGenerator.Generate_OleIndex(), sExtension);
		...
		sResult.AppendFormat( _T(" r:id=\"%s\""), srId );
		sResult.Append( _T("/>") );
		return sResult;
}
...
DWORD CALLBACK OlePut2(LPOLESTREAM oStream, const void FAR* pTarget, DWORD dwWrite)
	{
		...
		for( int i = 0; i < (int)dwWrite; i++ )
			piStream->aBuffer.Add( pSource[i] );
		return dwWrite;
	}