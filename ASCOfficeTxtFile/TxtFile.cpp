...
STDMETHODIMP CTxtFile::LoadFromFile(BSTR sSrcFileName, BSTR sDstPath, BSTR sXMLOptions)
{
	...
	BOOL result = node.FromXmlFile(CString(sSrcFileName));
	if(result)
	{
		IXmlFile* iXML = NULL;

		HRESULT hr = CoCreateInstance( __uuidof(CXmlFile), NULL, CLSCTX_INPROC_SERVER, __uuidof(IXmlFile), (void **)&(iXML) );
		if(hr==S_OK)
		{
			hr = iXML->LoadFromFile(sSrcFileName, sDstPath, sXMLOptions);
			if(hr == S_OK)
				return hr;
		}
	}

	const boost::filesystem::wpath txtFile	= sSrcFileName;
	...
	try
	{
		std::wstring options = sXMLOptions;
		options += L" ";
		...
		if (!options.empty())
		{
			...
			if (document.Root.element("TXTOptions").element("Encoding").exist())
				encoding = document.Root.element("TXTOptions").element("Encoding").text();
		}
		Progress(0, 0);
		Txt2Docx::Converter converter(origin, encoding);
...
void CTxtFile::createOriginDocx(const boost::filesystem::wpath& path) const
{
