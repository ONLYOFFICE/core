...
/*========================================================================================================*/
	...
	bool CompressFiles(_bstr_t zip_file_path, RequestFileCallback& callback, int compression_level)
	{
		...
		if(NULL != zip_file_handle)
		{
			...
			{
				_bstr_t in_zip_filename_wrapper(in_zip_filename, false); // This prevents memory leakage and guarantees the BSTR release on any exception
				CComSafeArray<BYTE> arr_wrapper;
				arr_wrapper.Attach(arr); // This prevents memory leakage and guarantees the SAFEARRAY release on any exception
