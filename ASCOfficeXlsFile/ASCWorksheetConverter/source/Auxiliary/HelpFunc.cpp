...
{
	return int2hex_str(alpha, sizeof(alpha)) + toRGB(red, green, blue);
}


const std::string toRGB(const BYTE red, const BYTE green, const BYTE blue)
{
	return int2hex_str(red, sizeof(red)) + int2hex_str(green,sizeof(green)) + int2hex_str(blue,sizeof(blue));
}
...
{
	if(tabid >= 0 && static_cast<WORD>(tabid) < sheets_names.size())
	{
...
const std::wstring xti2sheets(const WORD ixti, std::vector<std::wstring>& xti_parsed)
{
	if(ixti >= 0 && static_cast<WORD>(ixti) < xti_parsed.size())
	{
