...
/* ID */
	...
	const DWORD getStreamPointer() const;
/* Data */
...
/* Pointers */
	...
	void skipNBytes(const size_t n); // Skip the specified number of bytes without reading
	void RollRdPtrBack(const size_t n); // Move read pointer back to reread some data
	void resetPointerToBegin();
	void reserveNBytes(const size_t n); // Skip the specified number of bytes filled them in with zeros
	template<class DataType>
	void reserveNBytes(const size_t n, const DataType fill_data) // Skip the specified number of bytes filled them in with specified data
	{
		checkFitWrite(n);
...
/* Markup delayed data */
	...
	void registerDelayedDataSource(const DWORD data,  const CFRecordType::TypeId receiver_id);
	...
	void registerDelayedFilePointerAndOffsetSource(const DWORD offset,  const CFRecordType::TypeId receiver_id);
	...
	CFRecord& operator>>(BYTE& val)	{ loadAnyData(val);	return *this; };
	CFRecord& operator>>(WORD& val)	{ loadAnyData(val);	return *this; };
	CFRecord& operator>>(DWORD& val)	{ loadAnyData(val);	return *this; };
	...
	CFRecord& operator>>(_bstr_t& val);

	CFRecord& operator<<(BYTE& val)	{ storeAnyData(val);	return *this; };
	CFRecord& operator<<(WORD& val)	{ storeAnyData(val);	return *this; };
	CFRecord& operator<<(DWORD& val)	{ storeAnyData(val);	return *this; };
...
#define DET_TYPE(num_bits) num_bits <= 8 ? BYTE : num_bits <=16 ? WORD : DWORD 
