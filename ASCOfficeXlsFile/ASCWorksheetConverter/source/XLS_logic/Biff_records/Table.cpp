...
void Table::writeFields(CFRecord& record)
{
	...
	if(!fDeleted1)
	{
		CellRef r1_ref(static_cast<_bstr_t>(r1));
		rwInpRw = static_cast<WORD>(r1_ref.getRow());
	...
	if(fTbl2 && !fDeleted1)
	{
		CellRef r2_ref(static_cast<_bstr_t>(r2));
		rwInpCol = static_cast<WORD>(r2_ref.getRow());
...
void Table::readFields(CFRecord& record)
{
	...
	r1 = static_cast<_bstr_t>(CellRef(rwInpRw, colInpRw, true, true));
	if(fTbl2)
	{
		r2 = static_cast<_bstr_t>(CellRef(rwInpCol, colInpCol, true, true));
	}
}
