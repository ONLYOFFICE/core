...
void UserSViewBegin::writeFields(CFRecord& record)
{
	...
	record.reserveNBytes(2); // reserved
	record << pnnSel;
	record.reserveNBytes(3); // reserved
	...
	{
		colRPane = static_cast<WORD>(ref_tmp.getColumn());
	}
	if(fSplitH)
	{
		rwBPane = 65535;
	...
	{
		rwBPane = static_cast<WORD>(ref_tmp.getRow());
	}
...
void UserSViewBegin::readFields(CFRecord& record)
{
	...
	Ref8U top_left;
	record >> top_left;
	ref8TopLeft = static_cast<_bstr_t>(top_left.getTopLeftCell());
	...
	pane_top_left_cell = _bstr_t(CellRef(fSplitH ? rwBPane : 0 , fSplitV ? colRPane : 0, true, true).toString().c_str());
}
