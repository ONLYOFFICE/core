...
class CellRef : public BiffStructTagged
{
	BASE_OBJECT_DEFINE_CLASS_NAME(CellRef)
public:

	CellRef();
	CellRef(const _bstr_t str);
	CellRef(const std::wstring& str_ref);
...
{\
public:\
	const _bstr_t& operator() ()\
{\
	static _bstr_t name_str(L#name); \
	return name_str;\
};\
};
...
{
public:
	CellRef_T(const _bstr_t& str_ref) : CellRef(str_ref) {};
...
typedef CellRef_T<RgceLoc_name, WORD, WORD, rel_Present> RgceLoc;
typedef CellRef_T<RgceLocRel_name, WORD, short, rel_Present> RgceLocRel;
typedef CellRef_T<RgceElfLocExtra_name, WORD, WORD, rel_Present> RgceElfLocExtra;
typedef CellRef_T<RgceElfLoc_name, WORD, WORD, rel_PresentQuoted> RgceElfLoc;
