...
void DXFALC::load(CFRecord& record)
{
	DWORD flags;
	record >> flags;
	alc = static_cast<BYTE>(GETBITS(flags, 0, 2));
	fWrap = GETBIT(flags, 3);
	alcv = static_cast<BYTE>(GETBITS(flags, 4, 6));
	fJustLast = GETBIT(flags, 7);
	trot = static_cast<BYTE>(GETBITS(flags, 8, 15));
	cIndent = static_cast<BYTE>(GETBITS(flags, 16, 19));
	fShrinkToFit = GETBIT(flags, 20);
	fMergeCell = GETBIT(flags, 21);
	iReadingOrder = static_cast<BYTE>(GETBITS(flags, 22, 23));
