...
{	
	...
	fAutoFilter = static_cast<BYTE>(GETBIT(flags, 0));
	fAutoFilterHidden = static_cast<BYTE>(GETBIT(flags, 1));
	fLoadXmapi = static_cast<BYTE>(GETBIT(flags, 2));
	fLoadFmla = static_cast<BYTE>(GETBIT(flags, 3));	
	fLoadTotalFmla = static_cast<BYTE>(GETBIT(flags, 7));
	fLoadTotalArray = static_cast<BYTE>(GETBIT(flags, 8));
	fSaveStyleName = static_cast<BYTE>(GETBIT(flags, 9));
	fLoadTotalStr = static_cast<BYTE>(GETBIT(flags, 10));
	fAutoCreateCalcCol = static_cast<BYTE>(GETBIT(flags, 11));
