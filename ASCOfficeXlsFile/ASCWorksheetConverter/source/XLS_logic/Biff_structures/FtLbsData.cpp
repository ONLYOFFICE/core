...
void FtLbsData::setXMLAttributes(MSXML2::IXMLDOMElementPtr own_tag)
{
	...
	if(fValidPlex)
	{
		...
		{
			MSXML2::IXMLDOMElementPtr str_tag = XMLSTUFF::makeXMLNode(L"rgLine", own_tag);
	...
	if(0 != wListSelType)
	{
		for (std::vector<Boolean<BYTE>>::iterator it = bsels.begin(), itEnd = bsels.end(); it != itEnd; ++it)
		{
			MSXML2::IXMLDOMElementPtr bsel_tag = XMLSTUFF::makeXMLNode(L"bsel", own_tag);
...
void FtLbsData::getXMLAttributes(MSXML2::IXMLDOMElementPtr own_tag)
{
	...
	if(0 != wListSelType)
	{
		...
		for(size_t counter = 1; (str_tag = nodes->nextNode()); ++counter)
		{
			Boolean<BYTE> bsel = static_cast<bool>(getStructAttribute(own_tag, L"val"));
...
void FtLbsData::store(CFRecord& record, const WORD ot)
{
	WORD ft = 0x0013; // reserved
	record << ft;
...
void FtLbsData::load(CFRecord& record, const WORD ot)
{
