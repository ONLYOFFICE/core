...
{
public:
	OperatorPtg(const WORD ptg_id_init) : Ptg(ptg_id_init) {};
...
template<class T, WORD fixedPtgId, int priority>
...
public:
	...
	virtual const WORD getPtgId() const {return fixedPtgId;};
...
template<class T, WORD fixedType, int priority>
class UOperatorPtg_T: public OperatorPtg
{
public:
	UOperatorPtg_T() : OperatorPtg(fixedType) {};
	void assemble(AssemblerStack& ptg_stack, PtgQueue& extra_data, MSXML2::IXMLDOMElementPtr parent)
	{
		ptg_stack.top() = T::getSymbols() + ptg_stack.top();
	};
	virtual const WORD getPtgId() const {return fixedType;};
	virtual const std::wstring& getOperatorSymbols() const {return T::getSymbols();};
	virtual const int getOperatorPriority() const {return priority;};
};
