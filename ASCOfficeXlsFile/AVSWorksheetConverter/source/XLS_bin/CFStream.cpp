...
void CFStream::read(void* buf, const size_t size)
{
	...
	if(FAILED(hres))
	{
		...
		str << "Impossible to read " << size << " bytes from \"" << getStreamName() << "\" stream";
...
void CFStream::write(const void* buf, const size_t size)
{
	...
	{
		std::stringstream str;
		str << "Impossible to write " << size << " bytes to \"" << getStreamName() << "\" stream";
		throw EXCEPT::RT::CompoundFileFormatError(str.str(), hres);
	}
	if(num_written < size)
	{
		std::stringstream str;
		str << "Only " << num_written << " bytes were saved instead of " << size << " bytes requested.";
		throw EXCEPT::RT::CompoundFileFormatError(str.str(), hres);
	}
	// Tipa successful
}
...
// Stream pointer
const ULONGLONG CFStream::getStreamPointer() const
{
...
void CFStream::seekFromBegin(const ULONGLONG offset)
{
	LARGE_INTEGER seek_ptr;
	seek_ptr.QuadPart = offset;
...
// Stream current size
const ULONGLONG CFStream::getStreamSize() const
{
	STATSTG info;
...
const std::string CFStream::getStreamName() const
{
	...
	return static_cast<char*>(_bstr_t(info.pwcsName));
}
...
{
	const DWORD record_start_pointer = static_cast<DWORD>(getStreamPointer()) - sizeof(WORD)/*size_short*/ - sizeof(CFRecordType::TypeId);
