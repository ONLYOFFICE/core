...
CompoundFile::CompoundFile(const _bstr_t& file_path, const ReadWriteMode mode)
:	rwMode(mode)
{
	HRESULT hres = S_OK;
	DWORD grfMode = 0;
	...
	{
		case cf_ReadMode:
			hres = StgIsStorageFile(file_path);
			switch(hres)
			{
			case S_FALSE:
			...
			if(FAILED( hres = StgOpenStorage(file_path, NULL, grfMode, NULL, 0, &storage_)))
			{
		...
		case cf_WriteMode:
			...
			if(FAILED( hres = StgCreateStorageEx(file_path, grfMode, STGFMT_STORAGE, 0, NULL, NULL,
...
/*
	...
	{
		Log::info(std::string("name: \"") + static_cast<char*>(_bstr_t(elem.pwcsName)) + "\"  type: " + STR::int2str(elem.type) + " size: " +
			STR::int2hex_str(elem.cbSize.HighPart) + STR::int2hex_str(elem.cbSize.LowPart));
	}
*/
}
...
IStream* CompoundFile::openStream(const _bstr_t& stream_name)
{
	IStream* pStream = NULL;
	HRESULT hres = storage_->OpenStream(stream_name, NULL, STGM_READ | STGM_DIRECT | STGM_SHARE_EXCLUSIVE, NULL, &pStream );
	if(FAILED(hres))
	{
...
IStream* CompoundFile::createStream(const _bstr_t& stream_name)
{
	IStream* pStream = NULL;
	HRESULT hres = storage_->CreateStream(stream_name, STGM_READWRITE | STGM_SHARE_EXCLUSIVE, NULL, NULL, &pStream);
	if(FAILED(hres))
	{
