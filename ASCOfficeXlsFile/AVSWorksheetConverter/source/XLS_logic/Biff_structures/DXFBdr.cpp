...
void DXFBdr::load(CFRecord& record)
{
	DWORD flags;
	record >> flags;
	dgLeft = static_cast<BYTE>(GETBITS(flags, 0, 3));
	dgRight = static_cast<BYTE>(GETBITS(flags, 4, 7));
	dgTop = static_cast<BYTE>(GETBITS(flags, 8, 11));
	dgBottom = static_cast<BYTE>(GETBITS(flags, 12, 15));
	icvLeft = static_cast<BYTE>(GETBITS(flags, 16, 22));
	icvRight = static_cast<BYTE>(GETBITS(flags, 23, 29));
	bitDiagDown = GETBIT(flags, 30);
	bitDiagUp = GETBIT(flags, 31);

	record >> flags;
	icvTop = static_cast<BYTE>(GETBITS(flags, 0, 6));
	icvBottom = static_cast<BYTE>(GETBITS(flags, 7, 13));
	icvDiag = static_cast<BYTE>(GETBITS(flags, 14, 20));
	dgDiag = static_cast<BYTE>(GETBITS(flags, 21, 24));
