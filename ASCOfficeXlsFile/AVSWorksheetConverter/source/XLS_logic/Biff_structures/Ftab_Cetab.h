...
public:
	...
	Ftab_Cetab(const WORD func_index_init);

	const std::wstring getFuncName() const;
	const long getParamsNum() const;
	const WORD getIndex() const;
	const bool isMacro() const;

	void toXML(MSXML2::IXMLDOMElementPtr parent);

public:
	static const WORD recognizeFunction(const std::wstring& word);

private:
	class ValuesDetermination
	{
	public:
		static const std::wstring getName(const WORD iftab);
		static const long getParamsNum(const WORD iftab);
		static const WORD getIndex(const std::wstring& func_name);
	...
	private:
		...
		struct ParamsFixed{
			ParamsFixed(const WORD iftab_, const long params_num_, const std::wstring& name_): name(name_), iftab(iftab_), params_num(params_num_) {};
			std::wstring name;
			WORD iftab;
			long params_num;
		};
		...
		typedef boost::multi_index_container<
			...
			boost::multi_index::indexed_by<
				boost::multi_index::ordered_unique<boost::multi_index::tag<sort_by_iftab>, BOOST_MULTI_INDEX_MEMBER(ParamsFixed, WORD, iftab)>,
