...
void HyperlinkObject::getXMLAttributes(MSXML2::IXMLDOMElementPtr xml_tag)
{
	...
	if(hlstmfHasDisplayName)
	{
		displayName = static_cast<_bstr_t>(getStructAttribute(xml_tag, L"displayName"));
	}
	if(hlstmfHasFrameName)
	{
		targetFrameName = static_cast<_bstr_t>(getStructAttribute(xml_tag, L"targetFrameName"));
	}
	if(hlstmfHasMoniker && hlstmfMonikerSavedAsStr)
	{
		moniker = static_cast<_bstr_t>(getStructAttribute(xml_tag, L"moniker"));
	}
	if(hlstmfHasMoniker && !hlstmfMonikerSavedAsStr)
	{
	...
	if(hlstmfHasLocationStr)
	{
		location = static_cast<_bstr_t>(getStructAttribute(xml_tag, L"location"));
	}
	if(hlstmfHasGUID)
	{
