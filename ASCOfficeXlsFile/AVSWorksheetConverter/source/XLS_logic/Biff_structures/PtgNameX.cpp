...
PtgNameX::PtgNameX(const std::wstring& word, MSXML2::IXMLDOMElementPtr parent, const PtgDataType data_type)
...
void PtgNameX::assemble(AssemblerStack& ptg_stack, PtgQueue& extra_data, MSXML2::IXMLDOMElementPtr parent)
{
	RevNamePtr tab_id;
...
void PtgNameX::fromString(const std::wstring& word, MSXML2::IXMLDOMElementPtr parent)
{
 	_bstr_t query = L"root/WorkbookStreamObject/GlobalsSubstream/SUPBOOK";
...
// 				std::wstring query = std::wstring(L"root/WorkbookStreamObject/GlobalsSubstream/SUPBOOK/ExternSheet/XTI[@iSupBook = '") + STR::int2wstr(ncounter) + L"']";
				...
				{
					if(_variant_t(_bstr_t(L"-2")) == xti->getAttribute(L"itabFirst"))
					{
						ixti = xcounter;
						return;
					}
