...
{
	static _bstr_t text_str(L"text");
...
{
	static _bstr_t text_str(L"text");
	static _bstr_t mark_set_start_str(L"mark_set_start");
...
void XLUnicodeRichExtendedString::storeSymbols(CFRecord& record, const size_t start_pos, const size_t cch, const bool is_wide)
{
	...
	if(is_wide)
	{
		std::wstring int_str = std::wstring(static_cast<wchar_t*>(str_)).substr(start_pos, cch);
	...
	{
		std::string int_str = STR::toStdString(static_cast<wchar_t*>(str_), record.getGlobalWorkbookInfo()->CodePage).substr(start_pos, cch);
...
{	
	WORD size = sizeof(WORD)/*cch*/ + sizeof(BYTE)/*flags*/;
	if(fRichSt)
	{
		size += sizeof(WORD)/*cRun*/;
	}
	if(fExtSt)
	{
...
{	
	WORD size = getNonVariablePartSize();

	size += str_.length() << (fHighByte ? 1 : 0);

	size += rgRun.size() * (2 * sizeof(WORD))/*FormatRun*/;
