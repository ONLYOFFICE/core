	...
	**************************************************************/
	...
	public:
		...
		void LoadFromXmlNode2(XmlUtils::CXmlNode& oNode)
		{
            ...
            m_rcBounds.right	= m_rcBounds.left + (std::max)(1.0, oNode.ReadAttributeDouble(_T("width")));
            m_rcBounds.bottom	= m_rcBounds.top + (std::max)(1.0, oNode.ReadAttributeDouble(_T("height")));
		...
		void LoadFromXmlNode(XmlUtils::CXmlNode& oNode, CDoubleRect* pRectPlaceholder = NULL)
		{
			...
			if (oNode.GetNode(_T("shape"), oNodeShape))
			{
                ...
                m_rcBounds.right	= m_rcBounds.left + (std::max)(1.0, oNodeShape.ReadAttributeDouble(_T("width")));
                m_rcBounds.bottom	= m_rcBounds.top + (std::max)(1.0, oNodeShape.ReadAttributeDouble(_T("height")));
			...
			if (oNode.GetNode(_T("text"), oNodeText))
			{
                ...
                oTextRect.right		= oTextRect.left + (std::max)(1.0, oNodeText.ReadAttributeDouble(_T("width")));
                oTextRect.bottom	= oTextRect.top + (std::max)(1.0, oNodeText.ReadAttributeDouble(_T("height")));
				...
				else
				{
					...
					strTextRect.Format(_T("<rect b=\"%d\" l=\"%d\" r=\"%d\" t=\"%d\" xmlns=\"http://schemas.openxmlformats.org/drawingml/2006/main\"></rect>"),b, x, r, y);
					if (m_oShape.m_pShape)
					{
		...
		void LoadShapeGeom(XmlUtils::CXmlNode& oNode)
		{
			...
			//m_oShape.m_strPPTXShape.Replace(_T("a:"), _T(""));

			if (m_oShape.m_strPPTXShape.GetLength() > 0)
			{
				if (NULL == m_oShape.m_pShape)
				{
					m_ClassType = NSBaseShape::pptx;
					m_oShape.m_pShape = new CPPTXShape();
					m_oShape.m_pShape->LoadFromXML(m_oShape.m_strPPTXShape);
				}
			}
			//else
			//{
			...
			//		m_ClassType = NSBaseShape::ppt;
			//		m_oShape.m_pShape = new CPPTShape();
			//		m_oShape.m_pShape->LoadFromXML(oNode);
			//	}
			//}
			...
			NSPresentationEditor::CSVGConverter* pSVGConverter		= new NSPresentationEditor::CSVGConverter();
			SVGTransformer* pSVGTransformer	= new SVGTransformer();

			pSVGConverter->SetOffsetXY(m_rcBounds.left, m_rcBounds.top, m_rcBounds.right - m_rcBounds.left, m_rcBounds.bottom - m_rcBounds.top);

			pSVGTransformer->Load(strSvg);

			pSVGConverter->put_Width(m_rcBounds.GetWidth());
			pSVGConverter->put_Height(m_rcBounds.GetHeight());
		
			pSVGTransformer->Draw(pSVGConverter, 0, 0, nW, nH);
			RELEASEOBJECT(pSVGTransformer);

			if (NULL == m_oShape.m_pShape)
				m_oShape.m_pShape = new CPPTShape();

			pSVGConverter->SetSettings(m_oShape.m_oPen, m_oShape.m_oBrush, m_oShape.m_pShape->m_oPath);
			RELEASEOBJECT(pSVGConverter);
#endif
		}
#endif
