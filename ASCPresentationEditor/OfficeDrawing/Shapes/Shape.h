...
public:
	...
	virtual void GetTextRect(CGeomShapeInfo& oInfo)
	{
		...
		if (!bIsFound)
		{
			if (0 < m_pShape->m_arTextRects.size())
			{
				...
				oInfo.m_dLeft += m_pShape->m_arTextRects[0].left * koefX;
				oInfo.m_dTop += m_pShape->m_arTextRects[0].top * koefY;
				oInfo.m_dWidth = (m_pShape->m_arTextRects[0].right - m_pShape->m_arTextRects[0].left) * koefX;
				oInfo.m_dHeight = (m_pShape->m_arTextRects[0].bottom - m_pShape->m_arTextRects[0].top) * koefY;
			}
			bIsFound = true;
		}
#endif
	...
	virtual void GetTextRect(CDoubleRect& oInfo)
	{
		...
		if (!bIsFound)
		{
			if (0 < m_pShape->m_arTextRects.size())
			{
				...
				dLeft	+= m_pShape->m_arTextRects[0].left * koefX;
				dTop	+= m_pShape->m_arTextRects[0].top * koefY;
				dWidth	= (m_pShape->m_arTextRects[0].right - m_pShape->m_arTextRects[0].left) * koefX;
				dHeight	= (m_pShape->m_arTextRects[0].bottom - m_pShape->m_arTextRects[0].top) * koefY;
			}
			bIsFound = true;
		}
#endif
	...
	virtual bool LoadFromXML(XmlUtils::CXmlNode& root)
	{
		...
		if(_T("draw:enhanced-geometry") == root.GetName())
		{
			...
			m_pShape = new COdpShape();
			//return m_pShape->LoadFromXML(xml);
			bool bRet = ((COdpShape*)m_pShape)->LoadFromXML(root);
			m_dWidthLogic	= ((COdpShape*)m_pShape)->FManager.GetValue(_T("width"));
			m_dHeightLogic	= ((COdpShape*)m_pShape)->FManager.GetValue(_T("height"));
		}
#endif
		return false;
	}
