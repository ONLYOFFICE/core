...
namespace FileUtils
{
	static inline bool FileExists(LPCTSTR szPath)
	{
	...
	static inline CString GetFullPathName (const CString& sPath)
	{
		LPWSTR sPart;
		CString sBuffer;
		::GetFullPathName (sPath, BUFSIZE, sBuffer.GetBuffer(BUFSIZE), &sPart);
		sBuffer.ReleaseBuffer();

		return sBuffer;
	}
	static inline CString FilePath(const CString& sFile)
	{
		CString sBuffer = sFile;
		PathRemoveFileSpec(sBuffer.GetBuffer(BUFSIZE));
		sBuffer.ReleaseBuffer();

		return sBuffer;
	}
	static inline CString GetTempFolder()
	{
		CString sBuffer;
		GetTempPath (BUFSIZE, sBuffer.GetBuffer(BUFSIZE));
		sBuffer.ReleaseBuffer();
...
namespace SVG
{
	...
	public:
		...
		BOOL HasChildNodes ()
		{
			...
			if ( m_pXmlNode->hasChildNodes () )
				return TRUE;

			return FALSE;
		}
		BOOL GetChild ( CXmlNode& oXmlNode )
		{
			if (!IsValid())
				return FALSE;
			...
			try
			{
				XML::IXMLDOMNodePtr pNewNode = m_pXmlNode->firstChild;
		...
		BOOL GetSubNode ( CXmlNode& oXmlNode )
		{
			if (!IsValid())
				return FALSE;
			...
			try
			{
				XML::IXMLDOMNodePtr pNewNode = m_pXmlNode->nextSibling;
		...
		BOOL GetNodes ( XML::IXMLDOMNodeListPtr& oXmlNodes )
		{
			if (!IsValid())
				return FALSE;
			...
			try
			{
				if ( m_pXmlNode->childNodes )
				{
					oXmlNodes = m_pXmlNode->childNodes;
...
namespace SVG
{
	...
	public:
		...
		static inline BOOL DoubleValues(const CString& Source, CSimpleArray<double>& Values)
		{
			CString number	=	_T("");			
		...
		static inline BOOL DoubleValues(const CString& Source, CAtlArray<double>& Values)
		{
			CString number	=	_T("");			
			int length = Source.GetLength();
	...
	public:
		...
		inline const CSimpleArray <CubicBezier>& GetCurves() const
		{
			return m_arrCurves;
		}
	...
	private:
		...
		CSimpleArray <CubicBezier> m_arrCurves;
	};
}
...
namespace SVG
{
	...
	public:
		...
		{
			int index = m_Table.FindKey(sKey);
			if (-1 != index)
			{
				long clr = m_Table.GetValueAt(index);
	...
	private:
		...
		CSimpleMap <CString, unsigned long>	m_Table;
	};
	class ColorParser
	{
	public:
			...
			if (Styles.GetLength())
			{
				...
				while (-1 != sTok)
				{
					...
					if (-1 != sTok)
					{
						...
						CString sAttr			=	StrUtils::RemoveSpaces(sSrc.Tokenize(_T(":"), index));
						if (-1 == index)
							continue;

						CString sVal			=	StrUtils::RemoveSpaces(sSrc.Mid(index, sSrc.GetLength() - index));
...
namespace SVG
{
	...
	public:
		...
		BOOL FromXml (CXmlNode& oXmlNode)
		{
	...
	public:
		...
		inline BOOL SetStyle (const CString& Styles, BOOL Clear, UnitSystem& oUnitSystem, IRefStorage* pStorage, const ColorTable& Table = ColorTable()) //	from string
		{
			...
			while ( -1 != To )
			{
				...
				if (-1 != To)
				{
					...
					CString attr = StrUtils::RemoveSpaces(Source.Tokenize ( _T(":"), Ind));
					if ( -1 == Ind)
						continue;

					CString Value		=	StrUtils::RemoveSpaces(Source.Mid(Ind, Source.GetLength() - Ind));
		...
		inline BOOL SetStyle (CXmlNode& oXmlNode, BOOL bResetStyle, UnitSystem& oUnitSystem, IRefStorage* pStorage, const ColorTable& Table = ColorTable())
		{
			if (bResetStyle)
				SetDefault ();
			...
			m_nStrokeOpacity	=	DoubleFromString ( oXmlNode.GetAttributeOrValue ( _T("stroke-opacity"), _T("1.0") ) );
		...
		inline BOOL UpdateStyle (CXmlNode& oXmlNode, UnitSystem& oUnitSystem, IRefStorage* pStorage, const ColorTable& Table = ColorTable())
		{
			// FILL
	...
	private:
		...
		inline static double DoubleValue (CXmlNode& oXmlNode, CString strAttr, UnitSystem& oUnitSystem)
		{
	...
	public:
		...
		BOOL SetStyle(const CString& Styles, BOOL Clear)
		{
			...
			while ( -1 != To )
			{
				...
				if ( -1 != To )
				{
					...
					CString Attribute	=	StrUtils::RemoveSpaces ( Source.Tokenize ( _T(":"), Ind ) );
					if ( -1 == Ind )
						continue;
		...
		BOOL UpdateStyle(CXmlNode& oXmlNode)
		{
	...
	public:
		...
		inline BOOL Read (CXmlNode& oXml, UnitSystem& us, IRefStorage* model, const ColorTable& colors = ColorTable())
		{
			...
			while (-1 != right)
			{
				...
				if (selector.GetLength())
				{
					...
					for (size_t i = 0; i < m_selectors.GetCount(); ++i)
					{
		...
		{
			CAtlMap<CString, Style>::CPair* cp = m_classStyles.Lookup(name);
			if (cp)
			{
				style = cp->m_value;
				return TRUE;
	...
	private:
		...
		CAtlMap<CString, Style>	m_classStyles;					//	селекторы классов
		Style					m_typeStyles[ENodesCount];		//	селекторы элементов
		CAtlArray<CString>		m_selectors;
