...
void ImplWritePolyPolyAction( SvStream& rOStm, const PolyPolygon& rPolyPoly )
{
	...
	for( n = 0; n < nPoly; n++ )
		nPoints = static_cast<USHORT>(nPoints + rPolyPoly[ n ].GetSize());
...
void ImplReadColor( SvStream& rIStm, Color& rColor )
{
	...
	rIStm >> nVal; rColor.SetRed( static_cast<UINT8>((USHORT)nVal >> 8) );
	rIStm >> nVal; rColor.SetGreen( static_cast<UINT8>((USHORT)nVal >> 8) );
	rIStm >> nVal; rColor.SetBlue( static_cast<UINT8>((USHORT)nVal >> 8) );
}
#ifdef AVS
// ------------------------------------------------------------------------
...
void ImplReadUnicodeComment( sal_uInt32 nStrmPos, SvStream& rIStm, String& rString )
{
	...
	if ( nStrmPos )
	{
		...
		nStringLen = static_cast<xub_StrLen>(( nActionSize - 4 ) >> 1);
