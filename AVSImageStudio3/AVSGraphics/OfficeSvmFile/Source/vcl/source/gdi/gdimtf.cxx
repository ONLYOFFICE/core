...
SvStream& operator>>( SvStream& rIStm, GDIMetaFile& rGDIMetaFile )
{
	if( !rIStm.GetError() )
	{
		int nStartPos = rIStm.Tell();
		char	aCode[ 6 ];
		rIStm.Read( (char*) &aCode, sizeof( aCode ) );
		if( memcmp( aCode, "SVGDI", 5 ) == 0 )
		{
			rIStm.Seek( nStartPos );
			SVMConverter oConverter( rIStm, rGDIMetaFile, CONVERT_FROM_SVM1 );
		}
		else if( memcmp( aCode, "VCLMTF", 6) == 0 )
		{
			rIStm.Seek( nStartPos );
...
BOOL GDIMetaFile::CreateThumbnail( sal_uInt32 nWidth, sal_uInt32 nHeight, int nResizeType,
									BitmapEx& rBmpEx, 
									...
									const Rectangle* pOverlayRect ) const
{
	...
	if ( aSizePix.Width() && aSizePix.Height()
	  && ( static_cast< unsigned long >(aSizePix.Width()) >
               nMaximumExtent ||
           static_cast< unsigned long >(aSizePix.Height()) >
               nMaximumExtent ) )
	{
...
#endif
	if ( aSizePix.Width() && aSizePix.Height() && ( -1 != nWidth || -1 != nHeight ) )
	{
		...
		if ( 1 == nResizeType )
		{
			...
			if( -1 != nWidth && -1 != nWidth )
			{	
				if ( fWH <= static_cast< double >( nWidth / nHeight ) )
				{
					aSizePix.Width() = FRound( nHeight * fWH );
					aSizePix.Height() = nHeight;
				}
				else
				{
					aSizePix.Width() = nWidth;
					aSizePix.Height() = FRound(  nWidth / fWH );
				}
			}
			else if( -1 != nWidth )
			{
...
void GDIMetaFile::SetGraphics( SalGraphics* oGraphics )
{
	m_poGraphics = oGraphics;
}