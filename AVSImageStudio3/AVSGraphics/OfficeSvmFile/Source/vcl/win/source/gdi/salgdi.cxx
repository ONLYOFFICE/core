...
{
	return new WinSalGraphics();
}
}

#ifndef AVS_INSERT
void WinSalGraphics::SetOutputSizePixel( const Size& rNewSize)
{
	SelectObject( mhDC, mhDefBmp );
	DeleteObject( mhBmp );
	mhBmp = NULL;
	...
	aBitmapInfo.bmiHeader.biWidth = rNewSize.getWidth();
	aBitmapInfo.bmiHeader.biHeight = rNewSize.getHeight();
	aBitmapInfo.bmiHeader.biPlanes = 1;
	aBitmapInfo.bmiHeader.biBitCount = (WORD)GetDeviceCaps( mhDC,
													        BITSPIXEL );
	...
	void* pDummy;
	mhBmp = CreateDIBSection( mhDC, &aBitmapInfo, 
...
int SVMCore::OutputDevice::ImplGetGraphics()
{
	if ( mpGraphics )
		return TRUE;
...
BOOL WIN_Rectangle( HDC hDC, int X1, int Y1, int X2, int Y2 )
{
...
BOOL WIN_Polygon( HDC hDC, CONST POINT * ppt, int ncnt )
{
...
BOOL WIN_PolyPolygon( HDC hDC, CONST POINT * ppt, LPINT npcnt, int ncnt )
{
...
HDC WinSalGraphics::ImplGetCachedDC( ULONG nID, HBITMAP hBmp )
{
#ifdef AVS
	SalData*	pSalData = GetSalData();
	...
	if( !pC->mhDC )
	{
		...
		mhBmp = CreateCompatibleBitmap( hDC, CACHED_HDC_DEFEXT, CACHED_HDC_DEFEXT );
		mhDefBmp = (HBITMAP) SelectObject( hCompDC, mhBmp );
		ReleaseDC( 0, hDC );
	}
...
#endif
	HDC hDC = 0;
	HBITMAP* hDefBmp = NULL;

	switch( nID )
	{
	case CACHED_HDC_1: hDC = mhDC0; hDefBmp = &mhDefBmp0;break;
	case CACHED_HDC_2: hDC = mhDC1; hDefBmp =&mhDefBmp1;break;
	case CACHED_HDC_DRAW: hDC = mhDC2; hDefBmp = &mhDefBmp2;break;
	}
	if( 0 != hDC && NULL !=hDefBmp )
		*hDefBmp = (HBITMAP)SelectObject( hDC, hBmp );
	return hDC;
}

// =======================================================================

void WinSalGraphics::ImplReleaseCachedDC( ULONG nID )
{
#ifdef AVS
	SalData*	pSalData = GetSalData();
	HDCCache*	pC = &pSalData->mpHDCCache[ nID ];
...
WinSalGraphics::WinSalGraphics()
{
	...
	mbStockPen = FALSE; 		// is Pen a stockpen
	mbStockBrush = FALSE;		// is Brush a stcokbrush
	mbPen = FALSE;				// is Pen (FALSE == NULL_PEN)
	mbBrush = FALSE;			// is Brush (FALSE == NULL_BRUSH)
	mbPrinter = FALSE;			// is Printer
	mbVirDev = FALSE;			// is VirDev
	mbWindow = FALSE;			// is Window
	mbScreen = FALSE;			// is Screen compatible
#ifndef AVS_INSERT
		HDC hDC = GetDC( 0 );
		...
		mhDC0 = CreateCompatibleDC( hDC );
		mhDC1 = CreateCompatibleDC( hDC );
		mhDC2 = CreateCompatibleDC( hDC );
		...
		mhBmp = CreateCompatibleBitmap( hDC, CACHED_HDC_DEFEXT, CACHED_HDC_DEFEXT );
		mhDefBmp = (HBITMAP) SelectObject( mhDC, mhBmp );
...
WinSalGraphics::~WinSalGraphics()
{
	...
	if ( mpFontKernPairs )
		...
		DeleteObject( mhDefBmp0 );
		DeleteObject( mhDefBmp1 );
		DeleteObject( mhDefBmp2 );
...
SVMCore::BOOL WinSalGraphics::unionClipRegion( long nX, long nY, long nWidth, long nHeight )
{
	if ( nWidth && nHeight )
	{
