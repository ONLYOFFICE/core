...
inline void SImage::PipeSetXY(SPipe *pPipe, int nX, int nY) 
{
	...
	case colorModeBGR8:
		...
		break;
	case colorModeBGRA8:
		pPipe->pColorDst = &m_pBitmap->m_pData[nY * m_pBitmap->m_nStride + 4 * nX];
	}
...
{
	return m_pState->m_pClip->СlipToRect( dX0, dY0, dX1, dY1 );
}

int  SImage::ClipToPath(SPath *pPath, BOOL bEo) 
{
	return m_pState->m_pClip->СlipToPath( pPath, m_pState->m_arrdMatrix, m_pState->m_dFlatness, bEo );
}
...
int SImage::DrawImage(SImageSource pSrc, void *pSrcData, SColorMode eColorMode, BOOL bAlpha, int nWidth, int nHeight, double *pMatrix) 
{
	...
	case colorModeBGR8:
		...
		nComponentsCount = 3;
	case colorModeBGRA8:
		bValidMode = ( eColorMode == colorModeBGRA8 );
		nComponentsCount = 4;
	break;
	}
	if ( !bValidMode ) 
	...
	if ( pSrc->m_pAlpha ) 
	{
		...
		for ( int nY = 0; nY < nHeight; ++nY ) 
		{
			...
			for ( int nX = 0; nX < nWidth; ++nX ) 
			{
				...
				{
					// Используем dShape вместо альфы(вообще говоря, это неправильно, но работает)
