...
namespace ImageRendering
{
	...
	namespace NViewSettingsConstants
	{
		...
		const static CComBSTR	g_csViewSettings_EnableFastWindow		=	CComBSTR ( L"EnableFastWindow" );			
		const static CComBSTR	g_csViewSettings_ZoomToWindowHalf		=	CComBSTR ( L"ZoomToWindowHalf" );			
	
		const static CComBSTR	g_csViewSettings_BeginUpdate			=	CComBSTR ( L"BeginUpdate" );			
		const static CComBSTR	g_csViewSettings_EndUpdate				=	CComBSTR ( L"EndUpdate" );			
		...
		const static long		g_cnViewSettings_EnableFastWindow		=	10018;
		const static long		g_cnViewSettings_ZoomToWindowHalf		=	10019;
		
		const static long		g_cnViewSettings_BeginUpdate			=	10020;
		const static long		g_cnViewSettings_EndUpdate				=	10021;
	}
}
...
{	
	...
	public:
		...
		{			  
			...
			m_nRullerType			=	ImageRendering::CRullers::sc_nRullerTypePixels;
		...
		inline int	GetRullerSize		( ) const
		{
			return m_nRullerSize;
		}

		// 
		inline bool RullerUp			( )	{ return m_nRullersUp;		}
		inline bool RullerDown			( )	{ return m_nRullersDown;	}
		inline bool RullerRight			( )	{ return m_nRullersRight;	}
		inline bool RullerLeft			( )	{ return m_nRullersLeft;	}
	...
	private:
		...
		inline void OnPaintRullesPX		( HDC hDC, int nWindowWidth, int nWindowHeight, int nImageWidth, int nImageHeight,	const double& nZoom, const double& OffsetX, const double& OffsetY, Gdiplus::Graphics* pGraphics )
		{
			...
			if ( m_nRullersUp )	// UP
			{
				Gdiplus::Pen oBlackPen ( Gdiplus::Color::Black );
				pGraphics->DrawLine ( &oBlackPen, 0, m_nRullerSize, nWindowWidth, m_nRullerSize );	// огранка со стороны картинки
			...
			{
				Gdiplus::Pen oBlackPen ( Gdiplus::Color::Black );
				pGraphics->DrawLine ( &oBlackPen, m_nRullerSize, 0, m_nRullerSize, nWindowHeight );	// огранка со стороны картинки
			...
			{
				Gdiplus::Pen oBlackPen ( Gdiplus::Color::Black );
				pGraphics->DrawLine ( &oBlackPen, 0, nWindowHeight - m_nRullerSize, nWindowWidth, nWindowHeight - m_nRullerSize );	// огранка со стороны картинки
			...
			{
				Gdiplus::Pen oBlackPen ( Gdiplus::Color::Black );
				pGraphics->DrawLine ( &oBlackPen, nWindowWidth - m_nRullerSize, 0, nWindowWidth - m_nRullerSize, nWindowHeight );	// огранка со стороны картинки
			...
			const wchar_t Number [] = L"px";

			// надпись единиц измерения
			if ( m_nRullersLeft && m_nRullersUp )		
			{
				pGraphics->DrawString ( Number, 2, m_pFontH, Gdiplus::RectF ( 0.0f, 0.0f, 65535.0f, (float)m_nRullerSize ),	&oStringFormat, &oBlackSolidBrush );

				return;
			}
		
			if ( m_nRullersRight && m_nRullersUp )	
			{
				pGraphics->DrawString ( Number, 2, m_pFontH, Gdiplus::RectF ( (Gdiplus::REAL) ( nWindowWidth - m_nRullerSize ), 0.0f, 65535.0f, (float)m_nRullerSize ),	&oStringFormat, &oBlackSolidBrush );
			
				return;
			}
		
			if ( m_nRullersRight && m_nRullersDown )	
			{
				pGraphics->DrawString ( Number, 2, m_pFontH, Gdiplus::RectF ( (Gdiplus::REAL) ( nWindowWidth - m_nRullerSize ), (float)(nWindowHeight - m_nRullerSize), 65535.0f, (float)m_nRullerSize ),	&oStringFormat, &oBlackSolidBrush );
			
				return;
			}
		
			if ( m_nRullersLeft && m_nRullersDown )		
			{
				pGraphics->DrawString ( Number, 2, m_pFontH, Gdiplus::RectF ( 0.0f, (float)(nWindowHeight - m_nRullerSize), 65535.0f, (float)m_nRullerSize ),	&oStringFormat, &oBlackSolidBrush );
		...
		inline void OnPaintRullesCM		( HDC hDC, int nWindowWidth, int nWindowHeight, int nImageWidth, int nImageHeight,	const double& nZoom, const double& OffsetX, const double& OffsetY, Gdiplus::Graphics* pGraphics )
		{
			...
			double PIXELSX	=	(double) ( GetDeviceCaps ( hDC, LOGPIXELSX ) ) / 2.540005;
			...
			int RullerStep			=	1;	// шаг линейки для текста

			if ( nZoom < 2.0 )
			{
				RullerStep			=	1;

				nPixelDashStepMax	=	(int)(PIXELSX);
				nPixelsMinDashCount	=	(int)(PIXELSX * 2 * nZoom);
				nPixelsMinDashStep	=	(int)(PIXELSX / 10 * nZoom);
			}

			if ( nZoom >= 1.0 && nZoom < 2.0 )
			{
				RullerStep			=	2;

				nPixelDashStepMax	=	(int)(PIXELSX * 2);
				nPixelsMinDashCount	=	(int)(PIXELSX * 2);
				nPixelsMinDashStep	=	(int)(PIXELSX / 10);
			}

			if ( nZoom < 1.0 && nZoom >= 0.33 )
			{
				RullerStep			=	5;

				nPixelDashStepMax	=	(int)(PIXELSX * 5);
				nPixelsMinDashCount	=	(int)(PIXELSX * 5);
				nPixelsMinDashStep	=	(int)(PIXELSX / 10);
			}
			
			if ( nZoom < 0.33 && nZoom >= 0.16 )
			{
				RullerStep			=	10;

				nPixelDashStepMax	=	(int)(PIXELSX * 10);
				nPixelsMinDashCount	=	(int)(PIXELSX * 10);
				nPixelsMinDashStep	=	(int)(PIXELSX / 10);
			}
		
			if ( nZoom < 0.16 && nZoom >= 0.08 )
			{
				RullerStep			=	20;

				nPixelDashStepMax	=	(int)(PIXELSX * 20);
				nPixelsMinDashCount	=	(int)(PIXELSX * 20);
				nPixelsMinDashStep	=	(int)(PIXELSX / 10);
			}
			
			if ( nZoom < 0.08 && nZoom >= 0.04 )
			{
				RullerStep			=	50;

				nPixelDashStepMax	=	(int)(PIXELSX * 50);
				nPixelsMinDashCount	=	(int)(PIXELSX * 50);
				nPixelsMinDashStep	=	(int)(PIXELSX / 10);
			}
			
			if ( nZoom < 0.04 && nZoom >= 0.02 )
			{
				RullerStep			=	100;

				nPixelDashStepMax	=	(int)(PIXELSX * 100);
				nPixelsMinDashCount	=	(int)(PIXELSX * 100);
				nPixelsMinDashStep	=	(int)(PIXELSX / 10);
			}
			...
			if ( nZoom < 0.02 )
			{
				...
				nPixelDashStepMax	=	(int)(PIXELSX * 200);
				nPixelsMinDashCount	=	(int)(PIXELSX * 200);
				nPixelsMinDashStep	=	(int)(PIXELSX / 10);
			}

			static const float	FRULLER_LINE_SMALL		=	0.18f;	//	длина для малой черты делений линейки
			static const float	FRULLER_LINE_AVERAGE	=	0.23f;
			static const float	FRULLER_LINE_BIG		=	0.55f;

			static const int 	COUNT_RULLERS_STRIP		=	10;		// кол-во делений на один отображаемый текстовый эл-т 
			...
			if ( m_nRullersUp )	// UP
			{
				...
				if ( OffsetX > 0.0 )
				{
					// отрисовка деление до первой текстовой метки
					int LinePosX	=	static_cast <int> ( nBeginX * nZoom + OffsetX ) - nPixelsMinDashStep;									// позиция следующей черты
					
					for ( int j = 0, RullerSize = (int)(m_nRullerSize * FRULLER_LINE_AVERAGE); j < COUNT_RULLERS_STRIP; ++j )
					{
						( COUNT_RULLERS_STRIP / 2 == j ) ? RullerSize = (int)(m_nRullerSize * FRULLER_LINE_BIG) : RullerSize = (int)(m_nRullerSize * FRULLER_LINE_SMALL);

						int PoSX	=	(int)(LinePosX - ( j * nPixelDashStepMax / COUNT_RULLERS_STRIP ) * nZoom);
						if ( PoSX <= 0.1 )
							break;

						if ( PoSX > m_nRullerSize + 1 )
							pGraphics->DrawLine ( m_pPenMinDash, PoSX, 0, PoSX, RullerSize );
					}
				}

				// 0 -> +
				for ( int i = 0, ruller = 0; i <= nEndX; i += nPixelDashStepMax, ruller +=RullerStep )															// для максимальных делений
				{	
					// позиция следующей черты
					int LinePosX	=	static_cast <int> ( i * nZoom + OffsetX );

					// отрисовка мелких делений 
					for ( int j = 0, RullerSize = (int)(m_nRullerSize * FRULLER_LINE_AVERAGE); j < COUNT_RULLERS_STRIP; ++j )
					{
						( COUNT_RULLERS_STRIP / 2 == j ) ? RullerSize = (int)(m_nRullerSize * FRULLER_LINE_BIG) : RullerSize = (int)(m_nRullerSize * FRULLER_LINE_SMALL);
					...
					wchar_t Number [ 32 ];
					swprintf_s ( Number, 32, L"%d", ruller );
				...
				// 0 -> -
				for ( int i = 0, ruller = 0; i >= nBeginX; i -= nPixelDashStepMax, ruller -=RullerStep )															// для максимальных делений
				{	
					// позиция следующей черты
					int LinePosX	=	static_cast <int> ( i * nZoom + OffsetX );
			...
			if ( m_nRullersLeft )	// LEFT
			{
				...
				{
					// отрисовка деление до первой текстовой метки
					int LinePosY	=	static_cast <int> ( nBeginY * nZoom + OffsetY ) - nPixelsMinDashStep;									// позиция следующей черты
					
					for ( int j = 0, RullerSize = (int)(m_nRullerSize * FRULLER_LINE_AVERAGE); j < COUNT_RULLERS_STRIP; ++j )
					{
						( COUNT_RULLERS_STRIP / 2 == j ) ? RullerSize = (int)(m_nRullerSize * FRULLER_LINE_BIG) : RullerSize = (int)(m_nRullerSize * FRULLER_LINE_SMALL);

						int PoSY	=	(int)(LinePosY - ( j * nPixelDashStepMax / COUNT_RULLERS_STRIP ) * nZoom);
						if ( PoSY <= 0.1 )
							break;

						if ( PoSY > m_nRullerSize + 1 )
							pGraphics->DrawLine ( m_pPenMinDash, 0, PoSY, RullerSize, PoSY );
					}
				}
				
				// 0 -> +
				for ( int i = 0, ruller = 0; i <= nEndY; i += nPixelDashStepMax, ruller +=RullerStep )															// для максимальных делений
				{	
					// позиция следующей черты
					int LinePosY	=	static_cast <int> ( i * nZoom + OffsetY );
					...
					if ( LinePosY < m_nRullerSize + 1 )
						continue;

					pGraphics->DrawLine ( m_pPenMinDash, 0, LinePosY, m_nRullerSize, LinePosY );
				
					if ( LinePosY > nWindowHeight - m_nRullerSize )
						continue;
				...
				// 0 -> -
				for ( int i = 0, ruller = 0; i >= nBeginY; i -= nPixelDashStepMax, ruller -=RullerStep )															// для максимальных делений
				{	
					// позиция следующей черты
					int LinePosY	=	static_cast <int> ( i * nZoom + OffsetY );
			...
			if ( m_nRullersDown )	// DOWN
			{
				...
				if ( OffsetX > 0.0 )
				{
					...
					for ( int j = 0, RullerSize = (int)(m_nRullerSize * FRULLER_LINE_AVERAGE); j < COUNT_RULLERS_STRIP; ++j )
					{
						...
						if ( PoSX > m_nRullerSize + 1 )
							pGraphics->DrawLine ( m_pPenMinDash, PoSX, nWindowHeight, PoSX, nWindowHeight - RullerSize );
					}
				}
				...
				{	
					...
					pGraphics->DrawString ( Number, (INT)wcslen ( Number ), m_pFontH, 
						Gdiplus::RectF ( (Gdiplus::REAL)LinePosX + 2.0f, (Gdiplus::REAL)nWindowHeight - (Gdiplus::REAL)m_nFontSize - 4.0f, 50.0f, (Gdiplus::REAL)m_nFontSize + 2.0f ),
						&oStringFormat, &oBlackSolidBrush );
				}
			...
			if ( m_nRullersRight )	// RIGHT
			{
				...
				if ( OffsetY > 0.0 )
				{
					...
					for ( int j = 0, RullerSize = (int)(m_nRullerSize * FRULLER_LINE_AVERAGE); j < COUNT_RULLERS_STRIP; ++j )
					{
						...
						if ( PoSY > m_nRullerSize + 1 )
							pGraphics->DrawLine ( m_pPenMinDash, nWindowWidth, PoSY, nWindowWidth - RullerSize, PoSY );
					}
				}
				...
				{	
					...
					oMatrix.Translate			( (Gdiplus::REAL) ( nWindowWidth - m_nRullerSize ), (Gdiplus::REAL)LinePosY + 2.0f );
					oMatrix.Rotate				( -90.0f );
					pGraphics->SetTransform		( &oMatrix );

					pGraphics->DrawString		( Number, (INT)wcslen ( Number ), m_pFontH, Gdiplus::RectF ( 0.0f, 0.0f, 50.0f, (Gdiplus::REAL)m_nFontSize + 2.0f ), &oStringFormat, &oBlackSolidBrush ); 
			...
			const wchar_t Number [] = L"cm";

			// надпись единиц измерения
			if ( m_nRullersLeft && m_nRullersUp )		// CM
			{
				pGraphics->DrawString ( Number, 2, m_pFontH, Gdiplus::RectF ( - 2.0f, 0.0f, 65535.0f, (float)m_nRullerSize ),	&oStringFormat, &oBlackSolidBrush );

				return;
			}
		
			if ( m_nRullersRight && m_nRullersUp )	// CM 
			{
				pGraphics->DrawString ( Number, 2, m_pFontH, Gdiplus::RectF ( (Gdiplus::REAL) ( nWindowWidth - m_nRullerSize - 2.0f ), 0.0f, 65535.0f, (float)m_nRullerSize ),	&oStringFormat, &oBlackSolidBrush );
			
				return;
			}
		
			if ( m_nRullersRight && m_nRullersDown )	// CM 
			{
				pGraphics->DrawString ( Number, 2, m_pFontH, Gdiplus::RectF ( (Gdiplus::REAL) ( nWindowWidth - m_nRullerSize - 2.0f ), (float)(nWindowHeight - m_nRullerSize), 65535.0f, (float)m_nRullerSize ),	&oStringFormat, &oBlackSolidBrush );
			
				return;
			}
		
			if ( m_nRullersLeft && m_nRullersDown )		// CM
			{
				pGraphics->DrawString ( Number, 2, m_pFontH, Gdiplus::RectF ( - 2.0f, (float)(nWindowHeight - m_nRullerSize), 65535.0f, (float)m_nRullerSize ),	&oStringFormat, &oBlackSolidBrush );
		...
		inline void OnPaintRullesIN		( HDC hDC, int nWindowWidth, int nWindowHeight, int nImageWidth, int nImageHeight,	const double& nZoom, const double& OffsetX, const double& OffsetY, Gdiplus::Graphics* pGraphics )
		{
			...
			int nPixelsWCM		=	(int) ( GetDeviceCaps ( hDC, LOGPIXELSX ) );
			int nPixelsHCM		=	(int) ( GetDeviceCaps ( hDC, LOGPIXELSY ) );
			...
			if ( m_nRullersRight && m_nRullersUp )	// CM 
			{
				pGraphics->DrawString ( Number, 2, m_pFontH, Gdiplus::RectF ( (Gdiplus::REAL) ( nWindowWidth - m_nRullerSize - 2.0f ), 0.0f, 65535.0f, (Gdiplus::REAL) m_nRullerSize ),	&oStringFormat, &oBlackSolidBrush );
			...
			if ( m_nRullersRight && m_nRullersDown )	// CM 
			{
				pGraphics->DrawString ( Number, 2, m_pFontH, Gdiplus::RectF ( (Gdiplus::REAL) ( nWindowWidth - m_nRullerSize - 2.0f ), (Gdiplus::REAL) ( nWindowHeight - m_nRullerSize ), 65535.0f, (Gdiplus::REAL) m_nRullerSize ),	&oStringFormat, &oBlackSolidBrush );
			...
			if ( m_nRullersLeft && m_nRullersDown )		// CM
			{
				pGraphics->DrawString ( Number, 2, m_pFontH, Gdiplus::RectF ( - 2.0f, (Gdiplus::REAL) ( nWindowHeight - m_nRullerSize ), 65535.0f, (Gdiplus::REAL) m_nRullerSize ),	&oStringFormat, &oBlackSolidBrush );
		...
		inline void FillRectangles		( Gdiplus::Graphics* pGraphics, int nWindowWidth, int nWindowHeight )
		{
			if ( m_nRullersUp || m_nRullersLeft )
			{
				...
				Gdiplus::SolidBrush oSolidBrush ( Gdiplus::Color::White );
				
				pGraphics->FillRectangle ( &oSolidBrush, oRectangle );
				
				Gdiplus::Pen oBlackPen ( Gdiplus::Color::Black );
				
				pGraphics->DrawLine ( &oBlackPen, 0, m_nRullerSize, m_nRullerSize, m_nRullerSize );
				pGraphics->DrawLine ( &oBlackPen, m_nRullerSize, 0, m_nRullerSize, m_nRullerSize );
			}
			...
			if ( m_nRullersUp || m_nRullersRight )
			{
				...
				pGraphics->DrawLine ( &oBlackPen, nWindowWidth - m_nRullerSize, 0, nWindowWidth - m_nRullerSize, m_nRullerSize );
				pGraphics->DrawLine ( &oBlackPen, nWindowWidth, m_nRullerSize, nWindowWidth - m_nRullerSize, m_nRullerSize );
			}
			...
			if ( m_nRullersDown || m_nRullersRight )
			{
				...
				pGraphics->DrawLine ( &oBlackPen, nWindowWidth - m_nRullerSize, nWindowHeight - m_nRullerSize, nWindowWidth - m_nRullerSize, nWindowHeight );
				pGraphics->DrawLine ( &oBlackPen, nWindowWidth - m_nRullerSize, nWindowHeight - m_nRullerSize, nWindowWidth, nWindowHeight - m_nRullerSize );
			}
			...
			if ( m_nRullersDown || m_nRullersLeft )
			{
				...
				pGraphics->DrawLine ( &oBlackPen, 0, nWindowHeight - m_nRullerSize, m_nRullerSize, nWindowHeight - m_nRullerSize );
				pGraphics->DrawLine ( &oBlackPen, m_nRullerSize, nWindowHeight - m_nRullerSize, m_nRullerSize, nWindowHeight );
			}
		}


		
		inline void CreatePaintStuff	()
		{
			if ( NULL == m_pFontH )
			{
	...
	public:
		...
		inline void UpdateLinesPen		( DWORD dwForeColor = 0x0, DWORD dwBackColor = 0xFF000000, FLOAT fPenWidth = 1.0f )
		{
			if ( NULL == m_pLinesPen || ( m_dwForeColor != dwForeColor ) || ( dwBackColor != m_dwBackColor ) || ( m_fPenWidth != fPenWidth ) )
			{
				RELEASEOBJECT ( m_pLinesPen );
				...
				Gdiplus::HatchBrush oHatchBrush ( Gdiplus::HatchStyle50Percent, Gdiplus::Color ( m_dwForeColor ), Gdiplus::Color ( m_dwBackColor ) );
				
				m_pLinesPen			=	new Gdiplus::Pen ( &oHatchBrush, m_fPenWidth );
			}
		}
	...
	public:
		...
		inline void OnPaint			( Gdiplus::Graphics* pGraphics, Gdiplus::Rect rc )
		{
			...
			{
				Gdiplus::HatchBrush oHatchBrush ( Gdiplus::HatchStyleLargeCheckerBoard, Gdiplus::Color ( m_nLightColor ), Gdiplus::Color ( m_nDarkColor ) );
				
				pGraphics->FillRectangle ( &oHatchBrush,  rc );
			}
		}
		...
		inline void SetDarkColor	( DWORD Value )	{	m_nDarkColor	=	Value; DeleteStuff (); UpdateStuff (); }
		inline void SetLightColor	( DWORD Value )	{	m_nLightColor	=	Value; DeleteStuff (); UpdateStuff (); }
		inline void SetSizesCell	( DWORD Value )	{	m_nSizesCell	=	Value; DeleteStuff (); UpdateStuff (); }

	private:

		inline void DeleteStuff	()
		{
			RELEASEOBJECT ( m_pBrush );
		}

		inline void UpdateStuff	()
		{
			if ( NULL == m_pBrush )
			{
				...
				{
					Gdiplus::Graphics render ( &oBitmap );
					if ( Gdiplus::Ok == render.GetLastStatus() )
					{
						render.Clear ( 0x0 );

						Gdiplus::REAL fWidthCell = (Gdiplus::REAL)m_nSizesCell;

						Gdiplus::SolidBrush* pBrush = new Gdiplus::SolidBrush ( Gdiplus::Color ( m_nDarkColor ) );
						if ( pBrush )
						{
							if ( Gdiplus::Ok == pBrush->GetLastStatus () )
							{
								render.FillRectangle ( pBrush, Gdiplus::RectF ( 0.0f, 0.0f, fWidthCell, fWidthCell ) );
								render.FillRectangle ( pBrush, Gdiplus::RectF ( fWidthCell, fWidthCell, fWidthCell, fWidthCell ) );
							}

							RELEASEOBJECT ( pBrush );
						}

						Gdiplus::SolidBrush* pBrush2 = new Gdiplus::SolidBrush ( Gdiplus::Color ( m_nLightColor ) );
						if ( pBrush2 )
						{
							if ( Gdiplus::Ok == pBrush2->GetLastStatus () )
							{
								render.FillRectangle ( pBrush2, Gdiplus::RectF ( 0.0f, fWidthCell, fWidthCell, fWidthCell ) );
								render.FillRectangle ( pBrush2, Gdiplus::RectF ( fWidthCell, 0.0f, fWidthCell, fWidthCell ) );
							}
...
#pragma endregion
	...
	public:
		...
		CViewManager ()
		{
			...
			m_nEnableFastWindow		=	0L;

			m_nUpdateWindow			=	TRUE;

			m_bUserRullers			=	FALSE;
			
			m_nUserWindowWidth		=	0;
			m_nUserWindowHeight		=	0;
			m_nUserImageWidth		=	0;
			m_nUserImageHeight		=	0;
			m_nUserZoom				=	1.0;
			m_nUserOffsetX			=	0.0;
			m_nUserOffsetY			=	0.0;

			m_bShowCursorMarkers	=	FALSE;
		}
		...
		void SetHWND ( HWND hwnd )
		{
			...
			// m_oFrameFastWindow.Create ( m_hWnd );
			// SetEnableFastWindow ( 1 );
		}

		HWND GetHWND()            {return m_hWnd;}

		bool BeginPaint ( HDC hDC, RECT& WindowRC, bool bZoomToWindow = false )
		{
			if ( FALSE == IsWindow ( m_hWnd ) )
				return false;

			if ( FALSE == m_nUpdateWindow )
				return false;
			...
			if ( m_nWindowWidth != nWindowWidth || m_nWindowHeight != nWindowHeight )
			{
				...
				if ( m_nEnableFastWindow )
				{
					if ( m_oFrameFastWindow.IsEnable() )
					{
						m_oFrameFastWindow.Reset();
					}
				}
			}
			...
			if ( bZoomToWindow || m_bZoomToWindow )
			{
				...
				m_oFrameFastWindow.SetCenterX	( (float) m_nCenterX );
				m_oFrameFastWindow.SetCenterY	( (float) m_nCenterY );
				m_oFrameFastWindow.SetZoom		( (float) m_nZoom, false /*ForseZoomToWindow*/ );
			}
			else 
			if ( m_nZoomToWindowHalf > -1 && m_nZoomToWindowHalf < 4 )	// включен ZoomToFit половиной изображения
			{
				ZoomToWindowHalf ();
							...
							if ( ImageRendering::PixelsUtils::sc_nInterpolationModeNone != m_nInterpolationMode )
							{
								if ( RENDER_FLAGS [ ImageRendering::Constants::sc_RenderModeUseAlphaLayer ] )
								{
									...
									else
									{
										...
										Gdiplus::RectF DownRect		( 0.0f, (Gdiplus::REAL)nEndDY ,	(Gdiplus::REAL)m_nWindowWidth,	__max ( (Gdiplus::REAL)m_nWindowHeight - (Gdiplus::REAL)nEndDY, (Gdiplus::REAL)m_nWindowHeight ) );
										Gdiplus::RectF LeftRect		( 0.0f, 0.0f,					(Gdiplus::REAL)nStartDX,		(Gdiplus::REAL)m_nWindowHeight );
										Gdiplus::RectF RightRect	( (Gdiplus::REAL)nEndDX , 0.0f, __max ( (Gdiplus::REAL)GetStartDX () + (Gdiplus::REAL)m_nAlphaWidth, (Gdiplus::REAL)m_nWindowHeight ), (Gdiplus::REAL)m_nWindowHeight );
								...
								// Gdiplus::SolidBrush oSolidBrush2 ( Color::Red );
								//m_pGraphics2->FillRectangle ( &oSolidBrush2, 
								//	Gdiplus::RectF ( m_nWindowWidth	-50,//	- m_nCenterX * m_nZoom,
								//	m_nWindowHeight	* 0.5	- m_nCenterY * m_nZoom,
								//	50.0f, 50.0f ) );
		...
		bool BeginPaint ( HDC hDC, RECT& WindowRC, BYTE* pScreenBuffer, bool bZoomToWindow = false )
		{
			if ( FALSE == IsWindow ( m_hWnd ) )
				return false;
							...
							if ( ImageRendering::PixelsUtils::sc_nInterpolationModeNone != m_nInterpolationMode )
							{
								if ( RENDER_FLAGS [ ImageRendering::Constants::sc_RenderModeUseAlphaLayer ] )
								{
									...
									else
									{
										if ( ImageRendering::PixelsUtils::sc_nInterpolationModeNearestNeighbor == m_nInterpolationMode )
										{
											if ( m_nAlphaWidth > 0 && m_nAlphaHeight > 0 )
											{
												Gdiplus::RectF UpRect		( 0.0f, 0.0f,					(Gdiplus::REAL)m_nWindowWidth,	(Gdiplus::REAL)nStartDY );
												Gdiplus::RectF DownRect		( 0.0f, (Gdiplus::REAL)nEndDY,	(Gdiplus::REAL)m_nWindowWidth,	__max ( (Gdiplus::REAL)m_nWindowHeight - (Gdiplus::REAL)nEndDY, (Gdiplus::REAL)m_nWindowHeight ) );
												Gdiplus::RectF LeftRect		( 0.0f, 0.0f,					(Gdiplus::REAL)nStartDX,		(Gdiplus::REAL)m_nWindowHeight );
												Gdiplus::RectF RightRect	( __max ( (Gdiplus::REAL)nEndDX, 0.0f ), 0.0f,	__max ( (Gdiplus::REAL)GetStartDX () + (Gdiplus::REAL)m_nAlphaWidth, (Gdiplus::REAL)m_nWindowWidth ), (Gdiplus::REAL)m_nWindowHeight );
		...
		Gdiplus::Bitmap* GetDrawImage ()		// получить изображение которое будет отрисовано с учетом zomm, положения центра
		{
			return m_pBitmap;
		}
		Gdiplus::Graphics* GetPainter ()
		{
			return m_pGraphics2;
		}


		void SetImage ( const BYTE* pImage, int Width, int Height, double ScaleInnerImage = 1.0, int RealWidth = -1, int RealHeight = -1, int nChannels = 4 )
		{
			m_pImage				=	pImage;
			...
			if ( m_nEnableFastWindow )
			{
				if ( m_oFrameFastWindow.IsEnable () )
				{
					m_oFrameFastWindow.SetImage ( (BYTE*)pImage, Width, Height, (float) m_nScaleInnerImage );
					m_oFrameFastWindow.SetEnableRender ( true );
					m_oFrameFastWindow.Reset();
				}
			}
		...
		void ZoomToWindowHalf ()
		{
			long nViewWidth		=	m_nWindowWidth;
			long nViewHeight	=	m_nWindowHeight;

			if ( 0 == m_nZoomToWindowHalf || 1 == m_nZoomToWindowHalf )
			{
				//nViewWidth		=	m_nWindowWidth * 2;
			}
			
			if ( 2 == m_nZoomToWindowHalf || 3 == m_nZoomToWindowHalf )
			{
				//nViewHeight	=	m_nWindowHeight * 2;
			}
			...
			double ZoomTo		=	min ( 
				( (double) nViewWidth	- m_nZoomToFitOffsetPX ) / ( m_nScaleInnerImage * (double) m_nImageWidth ),
				( (double) nViewHeight	- m_nZoomToFitOffsetPX ) / ( m_nScaleInnerImage * (double) m_nImageHeight ) );
			...
			if ( 0 == m_nZoomToWindowHalf )	//	TO LEFT
			{
				CenterX		=	-( m_nWindowWidth * 0.5 ) / ZoomTo + 0.5 * m_nImageWidth * m_nScaleInnerImage;
			}

			if ( 1 == m_nZoomToWindowHalf )	//	TO RIGHT
			{
				CenterX		=	( m_nWindowWidth * 0.5 ) / ZoomTo + 0.5 * m_nImageWidth * m_nScaleInnerImage;
			}

			if ( 2 == m_nZoomToWindowHalf )	//	TO BOTTOM
			{
				CenterY		=	-( m_nWindowHeight * 0.5 ) / ZoomTo + 0.5 * m_nImageHeight * m_nScaleInnerImage;
			}

			if ( 3 == m_nZoomToWindowHalf )	//	TO TOP
			{
				CenterY		=	( m_nWindowHeight * 0.5 ) / ZoomTo + 0.5 * m_nImageHeight * m_nScaleInnerImage;
			}
		...
		bool GetScreenImage ( BYTE* pToCopyMemory )
		{
			if ( m_pBitmap2 )
			{
				BitmapData oBitmapData; 
	...
	public:
		...
		PROPERTY_GET_CONST	( ZoomToWindowHalf,		LONG );
		...
		void SetZoom				( double Zoom, bool ForseZoomToWindow = true )
		{		
			if ( m_nZoom < 0.0 )
				return;
		...
		void SetEnableRullerUp		( bool RullerValue )
		{
			m_nRullersUp			=	RullerValue;
		...
		void SetEnableRullerDown	( bool RullerValue )
		{
			m_nRullersDown			=	RullerValue;
		...
		void SetEnableRullerRight	( bool RullerValue )
		{
			m_nRullersRight			=	RullerValue;
		...
		void SetEnableRullerLeft	( bool RullerValue )
		{
			m_nRullersLeft			=	RullerValue;
		...
		void SetRenderMode			( int Mode, int Value)
		{
			RENDER_FLAGS [ Mode ]	=	Value;

			m_oFrameFastWindow.SetEnableAlphaLayer ( RENDER_FLAGS [ ImageRendering::Constants::sc_RenderModeUseAlphaLayer ] ? true : false );
		}
		void SetRefresh				( long RefreshImage )
		{
			m_nRefreshImage			=	RefreshImage;
		}

		void SetInterpolationMode	( long InterpolationMode )
		{
			m_nInterpolationMode	=	InterpolationMode;
			m_nRefreshImage			=	ImageRendering::Constants::sc_nRefreshForRebuild;
		}

		void SetZoomToWindow		( bool bZoomToWindow )
		{
			m_bZoomToWindow			=	bZoomToWindow;
			m_nRefreshImage			=	ImageRendering::Constants::sc_nRefreshForRebuild;
		
			m_oFrameFastWindow.SetZoomToWindow ( m_bZoomToWindow );
		}

		void SetZoomToWindowHalf	( LONG ZoomToWindowHalf )
		{
			m_nZoomToWindowHalf		=	ZoomToWindowHalf;
			
			m_nRefreshImage			=	ImageRendering::Constants::sc_nRefreshForRebuild;
		
			// ZoomToWindowHalf не совместим с m_bZoomToWindow
			m_bZoomToWindow			=	false;

			m_oFrameFastWindow.SetZoomToWindow ( false );
		}
		...
		void SetBackClearColor		( DWORD Color )
		{
			m_nBackClearColor		=	Color;
			m_nRefreshImage			=	ImageRendering::Constants::sc_nRefreshForRebuild;

			m_oFrameFastWindow.SetBackClearColor ( m_nBackClearColor );
		}
		bool GetZoomToWindow		( )
		{
			return m_bZoomToWindow;
		}

		ImageRendering::CRullers&	GetRullers ()	
		{
			return	m_oRullers;
		}
		ImageRendering::CGridRender&	GetGridRender ()
		{
			return m_oGridRender;
		}
		ImageRendering::CChessBrush&	GetChessBrush ()
		{
			return m_oChessBrush;
		}

		void SetRullersType			( long Type )
		{
		...
		void SetZoomToFitOffsetPX	( double Value )
		{
		...
		void SetEnableFastWindow	( long Mode )
		{
			if ( -1 == m_oFrameFastWindow.IsInitialized () )
			{
				m_oFrameFastWindow.Create ( m_hWnd );
			}
			...
			if ( m_oFrameFastWindow.IsEnable() )
			{
				m_nEnableFastWindow		=	Mode;

				m_oFrameFastWindow.SetEnableRender ( 1 == m_nEnableFastWindow );
				
				if ( m_nEnableFastWindow )
				{
					if ( m_oFrameFastWindow.IsEnable () )
					{
						m_oFrameFastWindow.SetImage ( (BYTE*)m_pImage, m_nImageWidth, m_nImageHeight, (float)m_nScaleInnerImage );
						m_oFrameFastWindow.SetEnableRender ( true );
						m_oFrameFastWindow.Reset();
					}
				}
				...
				{
					m_oFrameFastWindow.ClearImage ();
				}
			}
		}
		
		long GetEnableFastWindow	()
		{
			if ( m_nEnableFastWindow && m_oFrameFastWindow.IsEnable() ) 
				return 1L;
		...
		BOOL SetUserRullers			( BOOL Enable, int WindowWidth = 0, int WindowHeight = 0, int ImageWidth = 0, int ImageHeight = 0, double Zoom = 0.0, double OffsetX = 0.0, double OffsetY = 0.0 )
		{												  
			m_bUserRullers			=	Enable;
			
			m_nUserWindowWidth		=	WindowWidth;
			m_nUserWindowHeight		=	WindowHeight;
			m_nUserImageWidth		=	ImageWidth;
			m_nUserImageHeight		=	ImageHeight;
			m_nUserZoom				=	Zoom;
			m_nUserOffsetX			=	OffsetX;
			m_nUserOffsetY			=	OffsetY;

			return TRUE;		
		}			

		BOOL SetShowMouseMarkers	( BOOL Show )
		{
			m_bShowCursorMarkers	=	Show;
	...
	private:
		...
		void GetPaintingBound ( double Zoom, double ScaleZoom, double& OffSetX, double& OffSetY, double& InvertZoom, long& X, long& Y, long& XW, long& YH )
		{
		...
		void GetScalePaintingBound ( double Zoom, double& OffSetX, double& OffSetY, long& X, long& Y, long& XW, long& YH )
		{
			long nViewWidth		=	m_nWindowWidth;
			long nViewHeight	=	m_nWindowHeight;
		...
		void PrepareFrontImage ()
		{
			if ( ImageRendering::Constants::sc_nRefreshForRebuild == m_nRefreshImage )
			{
				...
				if ( NULL != m_pBitmap )
				{
							...
							if ( ScaleZoom < 1.0 ||  ScaleZoom > 1.0 )
							{
								...
								if ( RENDER_FLAGS [ ImageRendering::Constants::sc_RenderModeEnableImageSurface ] )
								{
									...
									m_oImageSurface.UpdateResize ( 
										...
										-(long) ( OffSetX ), -(long) ( OffSetY ),								// Destination offset
										(long) SrcOffsetX, (long) SrcOffsetY,													// Source offset
										(long) StartDX, (long) EndDX, (long) StartDY, (long) EndDY,				// Source step
					...
					if ( ImageRendering::PixelsUtils::sc_nInterpolationModeBilinear == m_nInterpolationMode )
					{
						Gdiplus::Graphics oRender ( m_pBitmap );

						oRender.Clear ( m_nBackClearColor );

						if ( RENDER_FLAGS [ ImageRendering::Constants::sc_RenderModeUseAlphaLayer ] )
						{
							m_oChessBrush.OnPaint ( &oRender, Gdiplus::Rect ( 
								(INT)OffSetX, 
								(INT)OffSetY, 
								(INT) ( m_nImageWidth * m_nZoom ), 
								(INT) ( m_nImageHeight * m_nZoom ) ) );
						}

						if( m_pImage && m_nImageWidth > 0 && m_nImageHeight > 0 )
						{
							//Gdiplus::Bitmap oBitmap ( m_nImageWidth, m_nImageHeight, m_nImageWidth * 4, PixelFormat32bppARGB, m_pImage );
							Gdiplus::Bitmap oBitmap ( m_nImageWidth, m_nImageHeight, PixelFormat32bppARGB );
							if( Gdiplus::Ok == oBitmap.GetLastStatus() )
							{
								Gdiplus::BitmapData oLockData;
								if( Gdiplus::Ok == oBitmap.LockBits( NULL, Gdiplus::ImageLockModeWrite, PixelFormat32bppARGB, &oLockData ) )
								{
									const BYTE* src = m_pImage;
									BYTE* dst = (BYTE*)oLockData.Scan0;

									for( int y = 0; y < m_nImageHeight; ++y, src += m_nImageWidth * 4, dst += oLockData.Stride  )
									{
										memcpy( dst, src, m_nImageWidth * 4 );
									}

									oBitmap.UnlockBits( &oLockData );
									...
									oRender.DrawImage ( &oBitmap, 
										(FLOAT) OffSetX - 1.0f, 
										(FLOAT) OffSetY - 1.0f, 
										(FLOAT) ( m_nImageWidth * m_nZoom + m_nZoom + 0.5 ), 
										(FLOAT) ( m_nImageHeight * m_nZoom + m_nZoom + 0.5 ) );
								}
							}
						}
					}
				...
				i++;
				wchar_t buffer [256];
				wsprintfW ( buffer, L"C:\\FrontImage_%i.png", i);

				CLSID ImageGUID;
				ImageRendering::ImageUtils::GetEncoderCLSID ( L"image/png", &ImageGUID );
				m_pBitmap->Save ( buffer, &ImageGUID );
		...
		void RenderRullers ()
		{
			if ( m_bUserRullers )
			{
				m_oRullers.OnPaint ( m_hDC, m_nUserWindowWidth, m_nUserWindowHeight, m_nUserImageWidth, m_nUserImageHeight, m_nUserZoom, m_nUserOffsetX, m_nUserOffsetY, m_pGraphics2 );

				if ( m_bShowCursorMarkers )
				{
					POINT CurPos;
					if ( GetCursorPos ( &CurPos ) )
					{
						...
						int RullerSize = m_oRullers.GetRullerSize ();

						if ( CurPos.x > RullerSize && CurPos.x < m_nUserWindowWidth - RullerSize &&
							CurPos.y > RullerSize && CurPos.y < m_nUserWindowHeight - RullerSize )
						{
							Gdiplus::Pen oBlackPen ( Gdiplus::Color::Black, 2 );
							
							if ( m_oRullers.RullerUp () )
								m_pGraphics2->DrawLine ( &oBlackPen, CurPos.x, 0, CurPos.x, RullerSize );

							if ( m_oRullers.RullerDown () )
								m_pGraphics2->DrawLine ( &oBlackPen, CurPos.x, m_nUserWindowWidth - RullerSize, CurPos.x, m_nWindowWidth );

							if ( m_oRullers.RullerLeft () )
								m_pGraphics2->DrawLine ( &oBlackPen, 0, CurPos.y, RullerSize, CurPos.y );

							if ( m_oRullers.RullerRight () )
								m_pGraphics2->DrawLine ( &oBlackPen, m_nUserWindowHeight, CurPos.y, m_nUserWindowHeight - RullerSize, CurPos.y );
						}
					}
				}
			}
		...
		void RenderBackBitmapNonCopyMode ()
		{
			if ( NULL == m_pBitmap2 )
				return;
	...
	private:
		...
		long							m_nZoomToWindowHalf;				// режим в котором включен ZoomToFit но со смещением изображение в половину видимой области в направление по осям 
		...
		// 
		ImageRendering::FrameFastWindow	m_oFrameFastWindow;
		long							m_nEnableFastWindow;
		long							m_nEnableSmoothZoom;

		BOOL							m_nUpdateWindow;

		// USER RULLERS
		BOOL							m_bUserRullers;
		int 							m_nUserWindowWidth;
		int 							m_nUserWindowHeight;
		int 							m_nUserImageWidth;
		int 							m_nUserImageHeight;
		double 							m_nUserZoom;
		double 							m_nUserOffsetX;
		double 							m_nUserOffsetY;

		BOOL							m_bShowCursorMarkers;
	};
...
#pragma endregion
	...
	public:
		...
		static bool SetProperty ( ImageRendering::CViewManager* pViewManager, BSTR Name, VARIANT Value )
		{
			...
			EPROPERTY_SET ( EnableFastWindow,	( Value.lVal ) );
			EPROPERTY_SET ( ZoomToWindowHalf,	( Value.lVal ) );

			if ( NViewSettingsConstants::g_csViewSettings_BeginUpdate == PropertyName )
			{
				pViewManager->BeginUpdate ();
				
				return true;
			}
			
			if ( NViewSettingsConstants::g_csViewSettings_EndUpdate == PropertyName )
			{
				pViewManager->EndUpdate ();
		...
		static bool GetProperty ( ImageRendering::CViewManager* pViewManager, BSTR Name, VARIANT* pValue )
		{
			...
			EPROPERTY_GET ( EnableFastWindow,	lVal,	LONG,	VT_I4 );
			EPROPERTY_GET ( ZoomToWindowHalf,	lVal,	LONG,	VT_I4 );
