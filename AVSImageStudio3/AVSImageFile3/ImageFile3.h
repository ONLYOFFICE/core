...
protected:
	...
	VARIANT_BOOL LoadImageTo(BSTR Path, void** pData, BOOL (*ImageTo)(Bitmap*, void**), BOOL bArray)
	{
		...
		try 
		{
			...
			{
				//неоднозначные ситуации не совпадени€ sync-word с заданным расширением
				if (eFileType)
				{
	...
	VARIANT_BOOL SaveImageAsAnimated(BSTR Path, LONG Format, void** Image, BOOL bArrayOrInterface = TRUE)
	{
		...
		//через VideoFormat выставл€етс€ Width, Height, ColorSpace
		...
		if( SUCCEEDED( hRes ) )
		{
			...
			// ≈сли файл одностраничный закрываем его
	...
	VARIANT_BOOL SaveImageAsMF(BSTR Path, void** Image, BOOL bEnhancedMetaFile, BOOL bArray = TRUE)
	{
		...
		try 
		{
			...
			// ѕри записи в Wmf или Emf, некоторые пикселы становились прозрачными.
			// ќказалось, что это пиксели: R = 13, G = 11, B = 12. ƒостаточно изменить, например,
			// синий канал на 13 и все станет нормально.

			// Ќепон€тно из-за чего это происходит, но теперь все работает хорошо.
