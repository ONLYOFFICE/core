/*! \file	dcraw_indi.c
*	\brief	DCRaw functions made independent (Адаптация Unidentified Flying Raw converter для COM-компоненты AVSImageRaw3)
*
*	За основу взят файл dcraw_indi.c из пакета ufraw-0.14.1.tar.gz
...
//#define DCRAW_SWAP(a,b) { a ^= b; a ^= (b ^= a); }
...
skip_block:
	...
	if (use_camera_wb && cam_mul[0] != -1)
	{
		...
		else if (cam_mul[0] && cam_mul[2])
		{
			...
			dcraw_message (dcraw, DCRAW_NO_CAMERA_WB,_("%s: Cannot use camera white balance.\n"), ifname_display);
	}
	//  if (user_mul[0])
	...
	dcraw_message(dcraw, DCRAW_VERBOSE,_("Scaling with black %d, multipliers"),dblack);
	FORC4
		dcraw_message(dcraw, DCRAW_VERBOSE, " %f", pre_mul[c]);
