/*!
 *	\file	nikon_curve.h
 *	\brief	Header file: read Nikon NTC/NCV files (Адаптация Unidentified Flying Raw converter для COM-компоненты AVSImageRaw3)
 *
...
typedef struct 
{
    ...
    unsigned int m_curveType; //< Type for this curve
    ...
    unsigned char m_numAnchors; //< Number of anchor points
...
/*!
 *	\brief	Helper function for calculating and storing tridiagnol matrices.
 *			Cubic spline calculations involve these types of matrices.
 */
double *d3_np_fs ( int n, double a[], double b[] );

/*!
 *	\brief	spline_cubic_set gets the second derivatives for the curve to be used in
 *		spline construction
 *
 *	\param	n		number of control points
 *  \param	t		x array
 *  \param	y		y array
 *	\param	ibcbeg	initial point condition (see function notes).
 *	\param	ybcbeg	beginning value depending on above flag
 *	\param	ibcend	end point condition (see function notes).
 *	\param	ybcend	end value depending on above flag
 *  
 *	\return			the y value at the given tval point
 */
double *spline_cubic_set ( int n, double t[], double y[], int ibcbeg,
    double ybcbeg, int ibcend, double ybcend );

/*!
 *	\brief	gets a value from spline curve.
 *
 *	\param	n		number of control points
 *	\param	t		x array
 *	\param	tval	x value you're requesting the data for, can be anywhere on the interval.
 *	\param	y		y array
 *	\param	ypp		second derivative array calculated by spline_cubic_set
 *	\param	ypval	first derivative value of requested point
 *	\param	yppval	second derivative value of requested point
 *  
 *	\return			the y value at the given tval point
 */
...
/*!
 *	\brief	Loads a curve from a Nikon ntc or ncv file.
 *    
 *	\param	fileNameThe	filename.
...
/*!
 *	\brief	Reset curve to straight line but don't touch the curve name.
 */
void CurveDataReset(CurveData *curve);

/*!
 *	\brief	Check if the curve is a trivial linear curve.
 */
...
/*!
 *	\brief		Transforms the curve generated by sampling the
...
/*!
 *	\brief				Savess a curve to a Nikon ntc or ncv file.
 *   
...
/*!
 *	\brief		Saves out curves to a Nikon ntc or ncv file.
 *
 *	\details	This function takes a single curve and uses defaults for the other curves.
 *				Typically, the curve used is the tone curve.
 *	
 *	\param	curve		A NikonCurve structure. This is usually the tone curve
 ...
 *	
 *	\parmam	sample		Pointer to the sampled curve.
 *	\parmam	fileName	The filename.
*/
int SaveSampledNikonCurve(CurveSample *sample, char *outfile);

/*!
 *	\brief	Moves the file pointer to the location
 *	indicated by the TAG-TYPE pairing.
 *
 *	\details	This is meant just
...
/*!
 *	\brief		Gets Nikon NEF data.
 *
 *	\details	For now, this is just the tone
...
/*!
 *	\brief	The actual retriever for the curve data from the NEF
 *			file.
 *
...
/*!
 * \brief	Init and allocate curve sample.
 */
