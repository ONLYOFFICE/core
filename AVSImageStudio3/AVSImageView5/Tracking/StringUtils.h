...
#include <Base64.h>
	...
	namespace Strings
	{
		...
		{
			// эта функция занимается максимально быстрым сравнением строк (не несовпадение)
			// не надо здесь ничего "более крутого" implement'ить... 
		...
		static BOOL ToBinary(const CString& strValue, BYTE*& pData, int& nSizeAllocated, int& nSizeArray)
		{
			...
			if (!Base64::Base64Decode((LPCSTR)strValue, strValue.GetLength(), pData, &nSizeArray))
			{
				delete[] pData;
				pData = NULL;
				nSizeArray = 0;
		...
		static CString FromBinary(const BYTE* pData, long lSize)
		{
		...
		public:
			...
			int Format(const TCHAR* strFormat,...)
			{
				...
				//int ret = _vsnprintf_s(&_buffer[0], nMaxChars, nMaxChars, strFormat, argList);
			...
			int CompareNoCase(const CStdString& str)
			{
				...
				{ 
					...
					if (::toupper(*oIterator1) != ::toupper(*oIterator2))
						return (::toupper(*oIterator1) < ::toupper(*oIterator2)) ? -1 : 1;  // return -1 to indicate smaller than, 1 otherwise
