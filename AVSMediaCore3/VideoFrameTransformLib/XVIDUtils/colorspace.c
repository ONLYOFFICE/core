...
int32_t fastconvert_RGB_Y_tab[256];
int32_t fastconvert_B_U_tab[256];
int32_t fastconvert_G_U_tab[256];
int32_t fastconvert_G_V_tab[256];
int32_t fastconvert_R_V_tab[256];
...
MAKE_COLORSPACE(fastconvert_rgb555_to_yv12_c,  2,2,2, RGB16_TO_YV12,  MK_RGB555, 0,0,0)
MAKE_COLORSPACE(fastconvert_rgb565_to_yv12_c,  2,2,2, RGB16_TO_YV12,  MK_RGB565, 0,0,0)
MAKE_COLORSPACE(fastconvert_bgr_to_yv12_c,     3,2,2, RGB_TO_YV12,    2,1,0, 0)
MAKE_COLORSPACE(fastconvert_bgra_to_yv12_c,    4,2,2, RGB_TO_YV12,    2,1,0, 0)
MAKE_COLORSPACE(fastconvert_abgr_to_yv12_c,    4,2,2, RGB_TO_YV12,    3,2,1, 0)
MAKE_COLORSPACE(fastconvert_rgba_to_yv12_c,    4,2,2, RGB_TO_YV12,    0,1,2, 0)
MAKE_COLORSPACE(fastconvert_argb_to_yv12_c,    4,2,2, RGB_TO_YV12,    1,2,3, 0)
MAKE_COLORSPACE(fastconvert_yuyv_to_yv12_c,    2,2,2, YUYV_TO_YV12,   0,1,2,3)
MAKE_COLORSPACE(fastconvert_uyvy_to_yv12_c,    2,2,2, YUYV_TO_YV12,   1,0,3,2)

MAKE_COLORSPACE(fastconvert_rgb555i_to_yv12_c, 2,2,4, RGB16I_TO_YV12, MK_RGB555, 0,0,0)
MAKE_COLORSPACE(fastconvert_rgb565i_to_yv12_c, 2,2,4, RGB16I_TO_YV12, MK_RGB565, 0,0,0)
MAKE_COLORSPACE(fastconvert_bgri_to_yv12_c,    3,2,4, RGBI_TO_YV12,   2,1,0, 0)
MAKE_COLORSPACE(fastconvert_bgrai_to_yv12_c,   4,2,4, RGBI_TO_YV12,   2,1,0, 0)
MAKE_COLORSPACE(fastconvert_abgri_to_yv12_c,   4,2,4, RGBI_TO_YV12,   3,2,1, 0)
MAKE_COLORSPACE(fastconvert_rgbai_to_yv12_c,   4,2,4, RGBI_TO_YV12,   0,1,2, 0)
MAKE_COLORSPACE(fastconvert_argbi_to_yv12_c,   4,2,4, RGBI_TO_YV12,   1,2,3, 0)
MAKE_COLORSPACE(fastconvert_yuyvi_to_yv12_c,   2,2,4, YUYVI_TO_YV12,  0,1,2,3)
MAKE_COLORSPACE(fastconvert_uyvyi_to_yv12_c,   2,2,4, YUYVI_TO_YV12,  1,0,3,2)
...
#define WRITE_RGB16(ROW,UV_ROW,C1)	\
	rgb_y = fastconvert_RGB_Y_tab[ y_ptr[y_stride*(ROW) + 0] ];						\
	...
	*(uint16_t *) (x_ptr+((ROW)*x_stride)+0) = C1(r[ROW], g[ROW], b[ROW]);	\
	rgb_y = fastconvert_RGB_Y_tab[ y_ptr[y_stride*(ROW) + 1] ];				\
	b[ROW] = (b[ROW] & 0x7) + ((rgb_y + b_u##UV_ROW) >> SCALEBITS_OUT);		\
	g[ROW] = (g[ROW] & 0x7) + ((rgb_y - g_uv##UV_ROW) >> SCALEBITS_OUT);	\
	r[ROW] = (r[ROW] & 0x7) + ((rgb_y + r_v##UV_ROW) >> SCALEBITS_OUT);		\
...
#define YV12_TO_RGB16(SIZE,C1,C2,C3,C4)		\
	int rgb_y; 												\
	int b_u0 = fastconvert_B_U_tab[ u_ptr[0] ];								\
	int g_uv0 = fastconvert_G_U_tab[ u_ptr[0] ] + fastconvert_G_V_tab[ v_ptr[0] ];		\
	int r_v0 = fastconvert_R_V_tab[ v_ptr[0] ];								\
	WRITE_RGB16(0, 0, C1)										\
	WRITE_RGB16(1, 0, C1)
...
#define YV12_TO_RGB16I(SIZE,C1,C2,C3,C4)		\
	...
	int g_uv0 = fastconvert_G_U_tab[ u_ptr[0] ] + fastconvert_G_V_tab[ v_ptr[0] ];		\
	int r_v0 = fastconvert_R_V_tab[ v_ptr[0] ];								\
    int b_u1 = fastconvert_B_U_tab[ u_ptr[uv_stride] ];						\
	int g_uv1 = fastconvert_G_U_tab[ u_ptr[uv_stride] ] + fastconvert_G_V_tab[ v_ptr[uv_stride] ];	\
	int r_v1 = fastconvert_R_V_tab[ v_ptr[uv_stride] ];						\
    WRITE_RGB16(0, 0, C1)										\
	WRITE_RGB16(1, 1, C1)										\
    WRITE_RGB16(2, 0, C1)										\
	WRITE_RGB16(3, 1, C1)										\
...
#define WRITE_RGB(SIZE,ROW,UV_ROW,C1,C2,C3,C4)	\
	rgb_y = fastconvert_RGB_Y_tab[ y_ptr[(ROW)*y_stride + 0] ];						\
	...
	rgb_y = fastconvert_RGB_Y_tab[ y_ptr[(ROW)*y_stride + 1] ];									\
...
MAKE_COLORSPACE(fastconvert_yv12_to_rgb555_c,  2,2,2, YV12_TO_RGB16,  MK_RGB555, 0,0,0)
MAKE_COLORSPACE(fastconvert_yv12_to_rgb565_c,  2,2,2, YV12_TO_RGB16,  MK_RGB565, 0,0,0)
MAKE_COLORSPACE(fastconvert_yv12_to_bgr_c,     3,2,2, YV12_TO_RGB,    2,1,0, 0)
MAKE_COLORSPACE(fastconvert_yv12_to_bgra_c,    4,2,2, YV12_TO_RGB,	  2,1,0,3)
MAKE_COLORSPACE(fastconvert_yv12_to_abgr_c,    4,2,2, YV12_TO_RGB,    3,2,1,0)
MAKE_COLORSPACE(fastconvert_yv12_to_rgba_c,    4,2,2, YV12_TO_RGB,    0,1,2,3)
MAKE_COLORSPACE(fastconvert_yv12_to_argb_c,    4,2,2, YV12_TO_RGB,    1,2,3,0)
MAKE_COLORSPACE(fastconvert_yv12_to_yuyv_c,    2,2,2, YV12_TO_YUYV,   0,1,2,3)
MAKE_COLORSPACE(fastconvert_yv12_to_uyvy_c,    2,2,2, YV12_TO_YUYV,   1,0,3,2)

MAKE_COLORSPACE(fastconvert_yv12_to_rgb555i_c, 2,2,4, YV12_TO_RGB16I, MK_RGB555, 0,0,0)
MAKE_COLORSPACE(fastconvert_yv12_to_rgb565i_c, 2,2,4, YV12_TO_RGB16I, MK_RGB565, 0,0,0)
MAKE_COLORSPACE(fastconvert_yv12_to_bgri_c,    3,2,4, YV12_TO_RGBI,   2,1,0, 0)
MAKE_COLORSPACE(fastconvert_yv12_to_bgrai_c,   4,2,4, YV12_TO_RGBI,	  2,1,0,3)
MAKE_COLORSPACE(fastconvert_yv12_to_abgri_c,   4,2,4, YV12_TO_RGBI,   3,2,1,0)
MAKE_COLORSPACE(fastconvert_yv12_to_rgbai_c,   4,2,4, YV12_TO_RGBI,   0,1,2,3)
MAKE_COLORSPACE(fastconvert_yv12_to_argbi_c,   4,2,4, YV12_TO_RGBI,   1,2,3,0)
MAKE_COLORSPACE(fastconvert_yv12_to_yuyvi_c,   2,2,4, YV12_TO_YUYVI,  0,1,2,3)
MAKE_COLORSPACE(fastconvert_yv12_to_uyvyi_c,   2,2,4, YV12_TO_YUYVI,  1,0,3,2)
...
void
fastconvert_yv12_to_yv12_c(uint8_t * y_dst, uint8_t * u_dst, uint8_t * v_dst,
...
void fastconvert_colorspace_init(void)
{
	...
	for (i = 0; i < 256; i++) {
		fastconvert_RGB_Y_tab[i] = FIX_OUT(RGB_Y_OUT) * (i - Y_ADD_OUT);
		fastconvert_B_U_tab[i] = FIX_OUT(B_U_OUT) * (i - U_ADD_OUT);
		fastconvert_G_U_tab[i] = FIX_OUT(G_U_OUT) * (i - U_ADD_OUT);
		fastconvert_G_V_tab[i] = FIX_OUT(G_V_OUT) * (i - V_ADD_OUT);
		fastconvert_R_V_tab[i] = FIX_OUT(R_V_OUT) * (i - V_ADD_OUT);
	}
}

void fastconvert_safe_packed_conv(uint8_t * x_ptr, int x_stride,
				 uint8_t * y_ptr, uint8_t * u_ptr, uint8_t * v_ptr,
				 int y_stride, int uv_stride,
				 int width, int height, int vflip,
