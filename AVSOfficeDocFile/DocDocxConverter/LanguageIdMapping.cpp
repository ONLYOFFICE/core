...
namespace DocFileFormat
{
	...
	LanguageIdMapping::LanguageIdMapping (XMLTools::XMLElement<WCHAR>* parentElement, LanguageType type) : PropertiesMapping(NULL)
	{
...
namespace DocFileFormat
{
	void LanguageIdMapping::Apply( IVisitable* lid )
	{
		if ( dynamic_cast<LanguageId*>( lid )->Code != Nothing )
		{
			...
			XMLTools::XMLAttribute<WCHAR>* att = NULL;

			switch ( _type )
			{
			case Default:
				{
					att = new XMLTools::XMLAttribute<WCHAR>( _T( "w:val" ), langcode.c_str() );
				}
				break;

			case EastAsian:
				{                    
					att = new XMLTools::XMLAttribute<WCHAR>( _T( "w:eastAsia" ), langcode.c_str() );
				}
				break;

			case Complex:
				{
					att = new XMLTools::XMLAttribute<WCHAR>( _T( "w:bidi" ), langcode.c_str() );
				}
				break;
