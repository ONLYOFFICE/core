...
namespace DocFileFormat
{
	ParagraphPropertiesMapping::ParagraphPropertiesMapping(XmlUtils::CXmlWriter* writer, ConversionContext* context, WordDocument* document, CharacterPropertyExceptions* paraEndChpx, bool isParagraphStyleNeeded) : PropertiesMapping( writer ), m_context(NULL), _pPr(NULL), _framePr(NULL), _sepx(NULL), _paraEndChpx(NULL), _sectionNr(0), m_document(NULL), _isParagraphStyleNeeded(isParagraphStyleNeeded)
	{
		...
		_pPr = new XMLTools::XMLElement<WCHAR>( _T( "w:pPr" ) );
		_framePr = new XMLTools::XMLElement<WCHAR>( _T( "w:framePr" ) );
...
namespace DocFileFormat
{
	void ParagraphPropertiesMapping::Apply( IVisitable* visited )
	{
		...
		XMLTools::XMLElement<WCHAR> ind( _T( "w:ind" ) );
		XMLTools::XMLElement<WCHAR> numPr( _T( "w:numPr" ) );
		XMLTools::XMLElement<WCHAR> pBdr( _T( "w:pBdr" ) );
		XMLTools::XMLElement<WCHAR> spacing( _T( "w:spacing" ) );
		XMLTools::XMLElement<WCHAR>* jc = NULL;

		if ( this->_isParagraphStyleNeeded )
		{
			//append style id , do not append "Normal" style (istd 0)
			XMLTools::XMLElement<WCHAR>  pStyle( _T( "w:pStyle" ) );
			XMLTools::XMLAttribute<WCHAR> styleId( _T( "w:val" ), StyleIdentifierMap[0] );
		...
		{
			XMLTools::XMLElement<WCHAR>* rPr = new XMLTools::XMLElement<WCHAR>( _T( "w:rPr" ) );
		...
		for ( list<SinglePropertyModifier>::iterator iter = papx->grpprl->begin(); iter != papx->grpprl->end(); iter++ )
		{
			...
			case sprmPDyaLine:
				{
					...
					XMLTools::XMLAttribute<WCHAR> line( _T( "w:line" ), FormatUtils::IntToWideString( abs( lspd.dyaLine ) ).c_str() );
					spacing.AppendAttribute( line );

					XMLTools::XMLAttribute<WCHAR> lineRule( _T( "w:lineRule" ), _T( "auto" ) );
			...
			case sprmPJc80:
				{ 
					RELEASEOBJECT( jc );
					jc = new XMLTools::XMLElement<WCHAR>( _T( "w:jc" ) );
					XMLTools::XMLAttribute<WCHAR> jcVal( _T( "w:val" ), FormatUtils::MapValueToWideString( iter->Arguments[0], &Global::JustificationCode[0][0], 10, 15 ).c_str() );
			...
			case sprmPBrcBetween80:
				{
					XMLTools::XMLElement<WCHAR> betweenBorder( _T( "w:between" ) );
			...
			case sprmPBrcBar80:
				{
					XMLTools::XMLElement<WCHAR> barBorder( _T( "w:bar" ) );
			...
			case sprmPChgTabs:
				{
					XMLTools::XMLElement<WCHAR> tabs( _T( "w:tabs" ) );

					int pos = 0;

					//read the removed tabs
					byte itbdDelMax = iter->Arguments[pos];

					pos++;

					for( int i=0; i < itbdDelMax; i++ )
					{
						XMLTools::XMLElement<WCHAR> tab( _T( "w:tab" ) );

						//clear
						XMLTools::XMLAttribute<WCHAR> tabsVal( _T( "w:val" ), _T( "clear" ) );
						tab.AppendAttribute( tabsVal );

						//position
						XMLTools::XMLAttribute<WCHAR> tabsPos( _T( "w:pos" ), FormatUtils::IntToWideString( FormatUtils::BytesToInt16( iter->Arguments, pos, iter->argumentsSize ) ).c_str() );
					...
					//read the added tabs
					byte itbdAddMax = iter->Arguments[pos];
					...
					for ( int i = 0; i < itbdAddMax; i++ )
					{
						...
						//justification
						XMLTools::XMLAttribute<WCHAR> tabsVal( _T( "w:val" ), FormatUtils::MapValueToWideString( tbd.jc, &Global::TabStop[0][0], 7, 8 ).c_str() );
						tab.AppendAttribute( tabsVal );

						//tab leader type
						XMLTools::XMLAttribute<WCHAR> leader( _T( "w:leader" ), FormatUtils::MapValueToWideString( tbd.tlc, &Global::TabLeader[0][0], 8, 11 ).c_str() );
						tab.AppendAttribute( leader );

						//position
						XMLTools::XMLAttribute<WCHAR> tabsPos( _T( "w:pos" ), FormatUtils::IntToWideString( FormatUtils::BytesToInt16( iter->Arguments, ( pos + (i * 2) ), iter->argumentsSize ) ).c_str() );
						tab.AppendAttribute( tabsPos );
			...
			case sprmPDcs:
				{
					...
					appendValueAttribute( this->_framePr, _T( "w:lines" ), (byte)( ( pDcs & 0xF8 ) >> 3 ) );
				}
				break;
		...
		if ( _sepx != NULL )
		{
			XMLTools::XMLElement<WCHAR> sectPr( _T( "w:sectPr" ) );
