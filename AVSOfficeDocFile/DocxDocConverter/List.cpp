...
{
  List::List( short _numID ):
  numID(_numID), level(0) 
  {
  }

  /*========================================================================================================*/

  List::List( const list<TextItemPtr>& _textItems, short _numID ):
  textItems(_textItems), numID(_numID), level(0)
  {
  ...
  textItems(_list.textItems), numID(_list.numID), level(_list.level)
  {
  }
  ...
  {
    for ( list<TextItemPtr>::iterator iter = this->textItems.begin(); iter != this->textItems.end(); iter++ )
	{
	  if ( typeid(**iter) == typeid(Paragraph) )
	  {
	    Paragraph* paragraph = dynamic_cast<Paragraph*>( iter->get() );
	  ...
	  {
	    List* subList = dynamic_cast<List*>( iter->get() );
  ...
  vector<ITextItem*> List::GetAllParagraphs()
  {
	...
	for ( list<TextItemPtr>::iterator iter = this->textItems.begin(); iter != this->textItems.end(); iter++ )
	{
	  if ( typeid(**iter) == typeid(Paragraph) )
	  {
	    allParagraphs.push_back( iter->get() );  
	  }
	  else
	  {
	    vector<ITextItem*> textItemParagraphs = (*iter)->GetAllParagraphs();

        for ( vector<ITextItem*>::iterator textItemIter = textItemParagraphs.begin(); textItemIter != textItemParagraphs.end(); textItemIter++ )
		{
  ...
  vector<PapxInFkp> List::GetAllParagraphsProperties( vector<unsigned int>* allParagraphsOffsets ) const
  {
	...
	{
	  vector<TextItemPtr> allParagraphs = this->GetAllParagraphsCopy();

	  for ( vector<TextItemPtr>::const_iterator iter = allParagraphs.begin(); iter != allParagraphs.end(); iter++ )
	  {
	    Paragraph* paragraph = dynamic_cast<Paragraph*>( iter->get() );
  ...
  vector<Chpx> List::GetAllRunProperties( vector<unsigned int>* allRunsOffsets ) const
  {
	...
	if ( allRunsOffsets != NULL )
	{
	  ...
	  for ( vector<TextItemPtr>::const_iterator iter = allParagraphs.begin(); iter != allParagraphs.end(); iter++ )
	  {
	    vector<Chpx> paragraphRunProperties;
        vector<unsigned int> paragraphRunsOffsets;
