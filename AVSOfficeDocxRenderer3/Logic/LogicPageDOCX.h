...
namespace NSDocx
{
	...
	public:
		...
		CDocxParagraph::CLine& GetCurrentLine()
		{
	...
	protected:
		...
		void CommandText(double& x, double& y, double& width, double& height, double& baselineoffset, double& dCurrentBaselineOffset)
		{
			...
			CDocxParagraph::CLine& oCurrentLine = GetCurrentLine();
			...
			if (0 == lCountWords)
			{
				...
				CDocxParagraph::CSpan& oSpan = oCurrentLine.m_listSpans.GetTail();
			...
			CDocxParagraph::CSpan& oTail = oCurrentLine.m_listSpans.GetTail();
	...
	public:
		virtual void ConvertToDocxPage(NSDocx::CFormat& oFormat)
		{
			...
			CDocxParagraph* pLogicParagraph = NULL;

			if (0 != oFormat.m_listSections.GetCount())
			{
				CSection& oSectionLast = oFormat.m_listSections.GetTail();
				IDocxItem* pItem = oSectionLast.m_arItems.GetTail();

				if (NULL != pItem)
				{
					if (IDocxItem::ebitParagraph == pItem->m_eType)
					{
						pLogicParagraph = (CDocxParagraph*)pItem;
			...
			while (NULL != pos)
			{
				...
				case ILogicItem::elitTextParagraph:
					{
						...
						{
							CDocxParagraph* pParagraph = new CDocxParagraph(pCurrentPar->m_oParagraph);
				...
				case ILogicItem::elitTable:
					{
						CDocxTable* pTable = new CDocxTable();
				...
				case ILogicItem::elitGraphic:
					{
						...
						CDocxGraphicObject* pImage = new CDocxGraphicObject();						
		...
		virtual void WriteTable(NSDocx::ILogicItem* pItem)
		{
		...
		virtual void CloseFile(NSDocx::CFormat& oFormat)
		{
		}
		...
		virtual void WriteImage(IUnknown* punkImage, double& x, double& y, double& width, double& height)
		{
			...
			else
			{
				...
				CDocxParagraph::CLine& oCurLine = m_oText.GetCurrentLine();
				...
				{
					CDocxParagraph::CSpan& oTail = oCurLine.m_listSpans.GetTail();
