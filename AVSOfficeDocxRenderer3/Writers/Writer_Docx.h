...
#pragma comment(lib, "../Common/DocxFormat/Lib/Release/DocxFormat.lib")

#include "../AVSOfficeDocxFile2/BinWriter/BinWriters.h"
...
namespace NSDocx
{
	...
	public:
		void CreateDocument(CWriter* pWriter)
		{
			...
			if (0 != pFormat->m_oMeta.m_listAnnotation.GetCount())
			{
				...
				while (NULL != pos)
				{
					IDocxItem* pItem = pFormat->m_oMeta.m_listAnnotation.GetNext(pos);

					switch (pItem->m_eType)
					{
					case IDocxItem::ebitParagraph:
						{
							WriteParagraph((CDocxParagraph*)pItem);
							break;
						}
					case IDocxItem::ebitTable:
						{
							WriteTable((CDocxTable*)pItem);
							break;
						}
					case IDocxItem::ebitGraphic:
						{
							WriteImage((CDocxGraphicObject*)pItem);
							break;
						}
					case IDocxItem::ebitNumbering:
						{
							WriteNumbering((CDocxNumbering*)pItem);
							break;
						}
					default:
						break;
			...
			while (NULL != posSect)
			{
				...
				while (NULL != pos)
				{
					IDocxItem* pItem = oSection.m_arItems.GetNext(pos);
	...
	protected:
		...
		AVSINLINE void WriteImage(NSDocx::CDocxGraphicObject* pImage)
		{
			CString strImage = _T("");
		...
		AVSINLINE void WriteParagraph(NSDocx::CDocxParagraph* pParagraph)
		{
			...
			while (NULL != posLine)
			{
				NSDocx::CDocxParagraph::CLine& oLine = pParagraph->m_arLines.GetNext(posLine);
		...
		AVSINLINE void WriteSpan(NSDocx::CDocxParagraph::CSpan* pSpan)
		{
		...
		AVSINLINE void WriteTable(NSDocx::CDocxTable* pTable)
		{
			m_oWriter.WriteString(pTable->m_strTableXml);
		}
		AVSINLINE void WriteNumbering(NSDocx::CDocxNumbering* pNumbering)
		{
