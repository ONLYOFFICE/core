...
private:
	...
	{
		m_oDocument.SetTextAssociationType( NSDocxRenderer::TextAssociationTypeNoFrames );
		...
		if( TRUE == oXmlReader.OpenFromXmlString( bsXMLOptions ) )
		{
			if( TRUE == oXmlReader.ReadRootNode( _T( "DocxRenderer" ) ) )
			{
				...
				{
					m_oDocument.SetTextAssociationType( (NSDocxRenderer::TextAssociationType)XmlUtils::GetInteger(textFormatting) );
				}
			}
		}
	}
