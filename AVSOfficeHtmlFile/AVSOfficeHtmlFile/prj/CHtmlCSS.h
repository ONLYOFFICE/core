	...
	public:
		...
		CString get_ElementCSS(htmlayout::dom::element &oHtmlNode)
		{
			...
			for ( int nIndex = 0; nIndex < m_aCssList.GetSize(); nIndex++ )
			{
				...
				USES_CONVERSION;
				if ( oHtmlNode.test( W2A(((CString &)oCss.get_Selector()).GetBuffer() )) )
					aCSSs.Add( oCss );
			}
			...
			if ( 0 == nSize )
				...
				return aCSSs.GetData()[0].get_Properties();
			else
			{
				...
				for ( int nIndex = 0; nIndex < nSize; nIndex++ )
				{
					sResult += aCSSs.GetData()[nIndex].get_Properties();
				}
		...
		CSimpleArray<CHtmlCSS*> get_CSS(htmlayout::dom::element &oHtmlNode)
		{
			...
			for ( int nIndex = 0; nIndex < m_aCssList.GetSize(); nIndex++ )
			{
				...
				USES_CONVERSION;
				if ( oHtmlNode.test( W2A(((CString &)pCss->get_Selector()).GetBuffer() )) )
					aCSSs.Add( pCss );
			}
