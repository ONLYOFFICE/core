...
{
	void Converter::TransitionConvert(const DWORD SldNum, CDocument& pDocument)const
	{
	...
	void Converter::TransitionParse(const Odp::Content::TransitionFilter& pTransition, const DWORD SldNum, const std::string& SlideId, CDocument& pDocument)const
	{
		std::string type = pTransition.Type.get_value_or("");
		if(type != "")
		{
			NSPresentationEditor::CTransition& lDestTransition = pDocument.m_arSlides[SldNum].m_oSlideShow.m_oTransition;
			...
			lDestTransition.m_dSpeed = 500;//1;//GetTransitionSpeedFromStr(pTransition.spd.get_value_or("fast"));
		}
	}
}