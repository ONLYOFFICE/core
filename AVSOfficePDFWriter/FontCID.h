...
{
    CIDFontDefAttr pAttr = (CIDFontDefAttr)pFontDef->pAttr;

	for ( unsigned int nIndex = 0; nIndex < pAttr->pW->nCount; nIndex++) 
	{
		CIDWidth *pW = (CIDWidth *)ListItemAt( pAttr->pW, nIndex);
		FreeMem( pFontDef->oMMgr, pW);
    }
...
void CIDFontDefFreeFunc   (FontDef pFontDef)
{
	...
	if ( pAttr->pFontData )
	{
		...
		pAttr->pFontData = NULL;
	}

	if ( pAttr->pCIDtoGIDMap )
	{
		StreamFree( pAttr->pCIDtoGIDMap );
		pAttr->pCIDtoGIDMap = NULL;
	}

	CIDFontDefFreeWidth( pFontDef );
...
FontDef       CIDFontDefNew         (MMgr oMMgr, char *sName, FontDefInitFunc pInitFn)
{
    ...
    UtilsStrCpy( pFontDef->sBaseFont, sName, pFontDef->sBaseFont + LIMIT_MAX_NAME_LEN);
    ...
    UtilsMemSet( (BYTE *)pFontDefAttr, 0, sizeof(CIDFontDefAttrRec) );

	pFontDefAttr->pW = ListNew( oMMgr, DEF_CHAR_WIDTHS_NUM );
	if ( !pFontDefAttr->pW ) 
	{
        FreeMem( pFontDef->oMMgr, pFontDef);
        FreeMem( pFontDef->oMMgr, pFontDefAttr);
        return NULL;
    }

	pFontDef->nMissingWidth = 500;

	pFontDefAttr->nDW      = 1000;
	pFontDefAttr->anDW2[0] = 880;
	pFontDefAttr->anDW2[1] = -1000;

	pFontDefAttr->pW2 = ListNew( oMMgr, DEF_CHAR_WIDTHS_NUM );
	if ( !pFontDefAttr->pW2 )
	{
		ListFree( pFontDefAttr->pW );
        FreeMem( pFontDef->oMMgr, pFontDef);
        FreeMem( pFontDef->oMMgr, pFontDefAttr);
        return NULL;
	}

	pFontDefAttr->nCIDtoGIDtype = -1;
	pFontDefAttr->pCIDtoGIDMap  = NULL;

    return pFontDef;
}
unsigned long CIDFontDefLoadFontData(FontDef pFontDef, wchar_t *wsFilePath, CIDFontFileType eCIDType)
{
	if ( !pFontDef || FontDefCID != pFontDef->eType )
		return OK;

	CIDFontDefAttr pAttr = (CIDFontDefAttr)pFontDef->pAttr;
	pAttr->eType = eCIDType;

	if ( NULL != wsFilePath )
	{
		// открываем файл
		HANDLE hFile = CreateFile( wsFilePath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		...
		pAttr->pFontData = TempFileStreamNew( pFontDef->oMMgr );
		if ( !pAttr->pFontData )
			return AVS_OFFICEPDFREADER_ERROR_NOT_ENOUGH_MEMORY;

		StreamWrite( pAttr->pFontData, pnFile, nFileSize, true );
...
{	
	...
	pAttr->anDW2[0] = anDW2[0];
	pAttr->anDW2[1] = anDW2[1];
}
short         CIDFontDefGetCIDWidth (FontDef pFontDef, unsigned short nCid)
{
	...
	for ( unsigned int nIndex = 0; nIndex < pAttr->pW->nCount; nIndex++) 
	{
		...
		if ( nCid >= pW->unCid_First && nCid <= pW->unCid_Last )
			return pW->shWidth;
    }
    // Не найден в массиве PDF_CID
	return pAttr->nDW;
}
unsigned long CIDFontDefAddWidth    (FontDef pFontDef, const CIDWidth *pWidths)
{
	...
	while ( 0xFFFF != pWidths->unCid_First ) 
	{
		CIDWidth *pCurWidth = (CIDWidth*)GetMem( pFontDef->oMMgr, sizeof (CIDWidth) );
        unsigned long nRet = OK;

        if ( !pCurWidth )
			return pFontDef->oError->nErrorNo;

		pCurWidth->unCid_First = pWidths->unCid_First;
		pCurWidth->unCid_Last  = pWidths->unCid_Last;  
		pCurWidth->shWidth     = pWidths->shWidth;

		if ( OK != ( nRet = ListAdd( pAttr->pW, pCurWidth ) ) ) 
		{
			FreeMem( pFontDef->oMMgr, pCurWidth);
            return nRet;
        }
...
unsigned long CIDFontDefAddWidth    (FontDef pFontDef, CIDWidth oWidth)
{
	...
	pCurWidth->unCid_First = oWidth.unCid_First;
	pCurWidth->unCid_Last  = oWidth.unCid_Last;  
	pCurWidth->shWidth     = oWidth.shWidth;
...
unsigned long CIDFontDefAddWidth2   (FontDef pFontDef, const CIDWidth2 *pWidths)
{
	...
	while ( 0xFFFF != pWidths->unCid_First ) 
	{
		CIDWidth2 *pCurWidth = (CIDWidth2*)GetMem( pFontDef->oMMgr, sizeof (CIDWidth2) );
        unsigned long nRet = OK;
		...
		pCurWidth->unCid_First = pWidths->unCid_First;
		pCurWidth->unCid_Last  = pWidths->unCid_Last;  
		pCurWidth->shW         = pWidths->shW;
		pCurWidth->shVx        = pWidths->shVx;
		pCurWidth->shVy        = pWidths->shVy;

		if ( OK != ( nRet = ListAdd( pAttr->pW2, pCurWidth ) ) ) 
		{
			FreeMem( pFontDef->oMMgr, pCurWidth);
            return nRet;
        }
...
unsigned long CIDFontDefAddWidth2   (FontDef pFontDef, CIDWidth2 oWidths2)
{
	...
	pCurWidth->unCid_First = oWidths2.unCid_First;
	pCurWidth->unCid_Last  = oWidths2.unCid_Last;  
	pCurWidth->shW         = oWidths2.shW;
	pCurWidth->shVx        = oWidths2.shVx;
	pCurWidth->shVy        = oWidths2.shVy;
...
unsigned long CIDFontDefSetCIDtoGID (FontDef pFontDef, char *sName)
{
	...
	pAttr->nCIDtoGIDtype = 0;
	UtilsStrCpy( pAttr->sCIDtoGIDMap, sName, pAttr->sCIDtoGIDMap + LIMIT_MAX_NAME_LEN );
...
unsigned long CIDFontDefSetCIDtoGID (FontDef pFontDef, BYTE *pBuffer, int nSize)
{
	...
	pAttr->nCIDtoGIDtype = 1;
	pAttr->pCIDtoGIDMap = TempFileStreamNew( pFontDef->oMMgr );
	if ( !pAttr->pCIDtoGIDMap )
		return AVS_OFFICEPDFREADER_ERROR_NOT_ENOUGH_MEMORY;

	StreamSeek( pAttr->pCIDtoGIDMap, 0, SeekSet );
	StreamWrite( pAttr->pCIDtoGIDMap, pBuffer, nSize, true );
...
static FontDict      CIDFontTypeNew               (FontDict pParent, Xref pXref)
{
	...
	Encoder pEncoder = pAttr->pEncoder;
...
static FontDict      СIDFontType0New              (FontDict pParent, Xref pXref)
{
    unsigned long nRet = OK;
    FontAttr pAttr = (FontAttr)pParent->pAttr;
    FontDef pFontDef = pAttr->pFontDef;
...
static Dict          CreateCMap                   (Encoder pEncoder, Xref pXref)
{
    unsigned long nRet = OK;
...
static unsigned long CIDOnWrite                   (Dict pObj, Stream pStream)
{
	return OK;
...
FontDict Type0FontNew  (MMgr oMMgr, FontDef pFontDef, Encoder pEncoder, Xref pXref, PToUnicode pToUnicode = NULL)
{
    unsigned long nRet = OK;
    ...
    // проверяем типы фонта и кодировки
	...
	if ( FontDefCID != pFontDef->eType && FontDefTrueType != pFontDef->eType ) 
	{
	...
	pAttr->eWritingMode   = pEncoderAttr->eWritingMode;
	pAttr->pTextWidthFn   = CIDTextWidth;
	pAttr->pMeasureTextFn = CIDMeasureText;
    ...
    nRet += DictAddName( pFont, "Type", "Font");
	nRet += DictAddName( pFont, "BaseFont", pFontDef->sBaseFont );
    nRet += DictAddName( pFont, "Subtype", "Type0");

    if ( FontDefCID == pFontDef->eType ) 
	{
        nRet += DictAddName( pFont, "Encoding", pEncoder->sName );
    } 
	else 
	{
		pAttr->pCMapStream = CreateCMap( pEncoder, pXref );

		if ( pAttr->pCMapStream ) 
		{
            nRet += DictAdd( pFont, "Encoding", pAttr->pCMapStream );
        } 
		else
            return NULL;
    }
    ...
    Array oDescendantFonts = ArrayNew( oMMgr );
    if ( !oDescendantFonts )
        return NULL;

    if ( OK != DictAdd( pFont, "DescendantFonts", oDescendantFonts ) )
        return NULL;

	if ( FontDefCID == pFontDef->eType ) 
	{
		pAttr->pDescendantFont = СIDFontType0New( pFont, pXref );
		pAttr->eType = FontType0CID;
    } 
	else 
	{
		pAttr->pDescendantFont = CIDFontType2New( pFont, pXref );
		pAttr->eType = FontType0TT;
    }

    if ( !pAttr->pDescendantFont )
        return NULL;
    else
		if ( OK != ArrayAdd( oDescendantFonts, pAttr->pDescendantFont ) )
            return NULL;

    return pFont;
}
FontDict Type0FontNew2 (MMgr oMMgr, FontDef pFontDef, Encoder pEncoder, Xref pXref, PToUnicode pToUnicode = NULL)
{
    unsigned long nRet = OK;
    ...
    // проверяем типы фонта и кодировки
	if ( pEncoder && EncoderTypeDoubleByteStream != pEncoder->eType ) 
	{
		SetError( pFont->oError, AVS_OFFICEPDFWRITER_ERROR_INVALID_ENCODER_TYPE, 0);
        return NULL;
    }
	...
	pFont->pHeader.nObjClass |= OSUBCLASS_FONT;
	pFont->pBeforeWriteFn     = CIDBeforeWrite;
	pFont->pWriteFn           = CIDOnWrite;
	pFont->pFreeFn            = CIDOnFree;
	pFont->pAttr              = pAttr;
...
//	pAttr->eWritingMode   = pEncoderAttr->eWritingMode;
	pAttr->pTextWidthFn   = CIDTextWidth2;
	pAttr->pMeasureTextFn = CIDMeasureText2;
	pAttr->pFontDef       = pFontDef;
	pAttr->pEncoder       = pEncoder;
	pAttr->pToUnicode     = pToUnicode;
	pAttr->pXref          = pXref;
	...
	{
		CMapEncoderStreamAttr pEncoderAttr = (CMapEncoderStreamAttr)pEncoder->pAttr;
		if ( 0 == pEncoderAttr->nType )
		{
			nRet += DictAddName( pFont, "Encoding", pEncoder->sName );
		} 
		else // if ( 1 == pEncoderAttr->nType )
		{
			Dict pEncoderData = DictStreamNew( oMMgr, pXref );

			if ( !pEncoderData )
				return NULL;

			nRet += DictAdd( pFont, "Encoding", pEncoderData );

			if ( OK != StreamWriteToStream( pEncoderAttr->pStream, pEncoderData->pStream, STREAM_FILTER_NONE, NULL ) )
				return NULL;
#ifndef _DEBUG
			pEncoderData->nFilter = STREAM_FILTER_FLATE_DECODE;
#endif

			nRet += DictAddName( pEncoderData, "Type", "CMap" );
			if ( strlen( pEncoderAttr->sCMapName ) > 0 )
				nRet += DictAddName( pEncoderData, "CMapName", pEncoderAttr->sCMapName );

			if ( strlen( pEncoderAttr->sRegistry ) > 0 && strlen( pEncoderAttr->sOrdering ) > 0 )
			{
				Dict pInfo = DictNew( oMMgr );
				nRet += DictAdd( pInfo, "Registry", StringNew( oMMgr, pEncoderAttr->sRegistry, NULL ) );
				nRet += DictAdd( pInfo, "Ordering", StringNew( oMMgr, pEncoderAttr->sOrdering, NULL ) );
				nRet += DictAddNumber( pInfo, "Supplement", pEncoderAttr->nSupplement );

				nRet += DictAdd( pEncoderData, "CIDSystemInfo", pInfo );
			}

			nRet += DictAddNumber( pEncoderData, "WMode", (int)pEncoderAttr->eWritingMode );

			if ( strlen( pEncoderAttr->uUseCMap.sName ) > 0 )
			{
				nRet += DictAddName( pEncoderData, "UseCMap", pEncoderAttr->uUseCMap.sName );
			}
			else if ( StreamValidate( pEncoderAttr->uUseCMap.pStream ) )
			{
				Dict pUseCMap = DictStreamNew( oMMgr, pXref );

				if ( !pUseCMap )
					return NULL;

				if ( OK != StreamWriteToStream( pEncoderAttr->uUseCMap.pStream, pUseCMap->pStream, STREAM_FILTER_NONE, NULL ) )
					return NULL;

#ifndef _DEBUG
				pUseCMap->nFilter = STREAM_FILTER_FLATE_DECODE;
#endif
				nRet += DictAdd( pEncoderData, "UseCMap", pUseCMap );
			}
		}
	}
	...
	pAttr->pDescendantFont = CIDFontTypeNew( pFont, pXref );
	...
	if ( NULL != pToUnicode )
	{
		...
		pToUnicode->pWriteFn( pToUnicode, pToUnicodeDict->pStream );

#ifndef _DEBUG
		pToUnicodeDict->nFilter = STREAM_FILTER_FLATE_DECODE;
#endif

		nRet += DictAdd( pFont, "ToUnicode", pToUnicodeDict );
	}
