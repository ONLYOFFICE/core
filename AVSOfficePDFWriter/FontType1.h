...
static unsigned long LoadType1   (FontDef pFontDef, CStringW wsFilePath/*, Stream pStream*/)
{
	...
	// Загружаем шрифт
	FT_Face pFace = NULL;
	if ( FT_New_Face( pLibrary, W2A( wsFilePath ), 0, &pFace ) || NULL == pFace ) 
	{
...
static unsigned long LoadAfm     (FontDef pFontDef, Stream pStream)
{
    Type1FontDefAttr pAttr = (Type1FontDefAttr)pFontDef->pAttr;
...
static unsigned long LoadFontData(FontDef pFontDef, Stream pStream, short nType)
{
    Type1FontDefAttr pAttr = (Type1FontDefAttr)pFontDef->pAttr;

	pAttr->nType = max( 0, min( 2, nType ) );
...
FontDef       Type1FontDefLoad           (MMgr oMMgr, Stream pAFM, Stream pFontData, short nType)
{
    unsigned long nRet = OK;
...
static unsigned long Type1FontOnWrite         (Dict pObj, Stream pStream)
{
	unsigned long nRet = OK;
...
FontDict Type1FontNew  (MMgr oMMgr, FontDef pFontDef, Encoder pEncoder, Xref pXref, PToUnicode pToUnicode)
{
    unsigned long nRet = OK;
