...
public: typedef enum {l_none,
						...
						l_ptablmdot, //\ptablmdot 	Absolute position tab with a leading that uses middle dot symbols (ХХХХХ).
						} Leader;
...
public: CString RenderToRtf(RenderParameter oRenderParameter)
		{
			...
			//switch ( m_nTextWrapBreak ) //не воспринимаетс€ word
			//{
...
private: CString renderTextToXML( CString sParam )
		{
			CString sResult;
			if( _T("Text") == sParam )
				sResult.AppendFormat( _T("<w:t xml:space= \"preserve\">%s</w:t>"), Utils::PrepareToXML( m_sChars ) );
			else if( _T("Math") == sParam )
				sResult.AppendFormat( _T("<m:t xml:space= \"preserve\">%s</m:t>"), Utils::PrepareToXML( m_sChars ));
			return sResult;
		}
