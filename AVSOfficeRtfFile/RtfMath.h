...
{
private: CString m_sRtfName;
private: CString m_sOOXName;
public: RtfMathSpecProp oProperty;
public: RtfParagraph m_oVal;
public: RtfShapePtr m_oPicture;
private: static const TCHAR* mc_aRtfControlWords[];
private: static const int mc_nRtfControlWordsSize;
private: static const TCHAR* mc_aOOXControlWords[];
private: static const int mc_nOOXControlWordsSize;
public:  RtfMath( )
		{
		}
public:  RtfMath(const RtfMath& oMath )
		{
			(*this) = oMath;
		}
public:  RtfMath& operator=(const RtfMath& oMath )
		{
			m_sRtfName = oMath.m_sRtfName;
			m_sOOXName = oMath.m_sOOXName;
			m_aArray.RemoveAll();
			m_aArray.Append( oMath.m_aArray );
			//for( int i = 0; i < oMath.m_aArray.GetCount(); i++ )
			//{
			//	RtfMath* oCopyMath = new RtfMath();
			//	(*oCopyMath) = (*oMath.m_aArray[i]);
			//	m_aArray.Add( oCopyMath );
			//}
			return (*this);
		}
public: bool IsEmpty()
...
public: static bool IsRtfControlWord( CString sControlW )
		{
...
public: static bool IsOOXControlWord( CString sControlW )
		{
			//можно бинарный поиск вставить
...
public: void SetRtfName(CString sName)
		{
			m_sRtfName = sName;
			m_sOOXName = sName;
			m_sOOXName.Insert(1, _T(":"));
		}
public: void SetOOXName(CString sName)
		{
			m_sOOXName = sName;
			m_sRtfName = sName;
			m_sRtfName.Remove(':');
...
public: CString RenderToRtf(RenderParameter oRenderParameter)
		{
			...
			sResult.AppendFormat(_T("{\\%s"), m_sRtfName);
