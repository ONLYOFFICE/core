...
public: CodePage m_eCodePage;
public: int m_nAnsiCodePage;
public: int m_nDeffFont;
public: int m_bHypCaps;//(doNotHyphenateCaps)\hyphcaps*	Switches hyphenation of capitalized words (default is on). Append 1 or leave control word by itself to toggle property on; append 0 to turn it off.
public: int m_bAutoHyp;//(autoHyphenation)\hyphauto*	Switches automatic hyphenation (default is off). Append 1 or leave control word by itself to toggle property on; append 0 to turn it off.
public: int m_nMaxHypen;//(consecutiveHyphenLimit)\hyphconsecN	N is maximum number of consecutive lines that are allowed to end in a hyphen. 0 means no limit.
public: int m_nHyphenationRight; //(hyphenationZone)\hyphhotzN	Hyphenation hot zone in twips (amount of space at right margin in which words are hyphenated).
public: int m_nTabWidth; //(defaultTabStop)\deftabN	Default tab width in twips (default is 720, i.e., 0.5").
public: int m_nZoom; //\viewscaleN 	Zoom level of the document; the N argument is a value representing a percentage (default is 100).


//Page Borders
public: int m_bDorderSurroundHeader; //(bordersDoNotSurroundHeader)\pgbrdrhead	Page border surrounds header.
public: int m_bDorderSurroundFotter; //(bordersDoNotSurroundFooter)\pgbrdrfoot	Page border surrounds footer.

public: int m_bAlignBordersAndEdges; //(alignBordersAndEdges)\pgbrdrsnap	Align paragraph borders and table edges with page border.
...
public: int m_bGutterAtTop; //(gutterAtTop)\gutterprl	Parallel gutter.

public: int m_bSwitchMargins; //\margmirror	Switches margin definitions on left and right pages. Used in conjunction with \facingp.
public: int m_bLandScape; //\landscape	Landscape format.
public: int m_nBeginningPageNumber; //\pgnstartN	Beginning page number (default is 1).

//Compatibility Options
public: int m_nDisplayBackground;//(displayBackgroundShape)\viewbkspN	Boolean:0	Background shapes will not show in Page Layout View (default if omitted).1	Background shapes will show in Page Layout View.
public: int m_bUseTabAlignment;//\useltbaln	Do not forget last tab alignment.

//footnote, endnote property
public: CAtlArray<int> m_aSpecialFootnotes;
public: int m_nFootnoteNumberingFormat;
public: int m_nFootnoteStart; //\ftnstartN	Beginning footnote number (default is 1).
public: typedef enum{ fr_None,
...
public: FootnoteRestart m_eFootnoteRestart;
public: typedef enum{ fp_None,
...
public: FootnotePlacement m_eFootnotePlacement;

public: CAtlArray<int> m_aSpecialEndnotes;
public: int m_nEndnoteNumberingFormat;
public: int m_nEndnoteStart; //\aftnstartN	Beginning endnote number (default is 1).
public: typedef enum{ er_None,
...
public: EndnoteRestart m_eEndnoteRestart;
public: typedef enum{ ep_None,
...
public: EndnotePlacement m_eEndnotePlacement;

//äëÿ ñõîðàíåíèÿ â RTF
public: int m_bSplitPageBrake; //\spltpgpar


public: int m_bHtmlAutoSpace; //\htmautsp	Use HTML paragraph auto spacing.

public: RtfDocumentProperty()
		{
			SetDefault();
		}
public: int GetType()
...
public: void SetDefault()
		{
			...
			//âû÷èñëÿåì ñòàíäàðòíûé default ansi codepage äëÿ ñèñòåìû
			TCHAR codepage[7];
...
public: int m_bBidi; //\rtlsect	This section will snake (newspaper style) columns from right to left.
public: int m_nPaperSourceFirst; //\binfsxnN	N is the printer bin used for the first page of the section. If this control is not defined, then the first page uses the same printer bin as defined by the \binsxnN control.
public: int m_nPaperSourceOther; //\binsxnN	N is the printer bin used for the pages of the section.
public: int m_bRtlGutter; //\rtlgutter	Gutter is positioned on the right.


public: int m_bEndnotes; // \endnhere	Endnotes included in the section.
public: int m_nStyle; // \dsN	Designates section style. If a section style is specified, style properties must be specified with the section.
...
public: SectionBreak m_eSectionBreak;

//Columns
public: int m_nColumnNumber; //\colsN	Number of columns for "snaking" (default is 1).
public: int m_nColumnSpace;  //\colsxN	Space between columns in twips (default is 720).
public: int m_bColumnLineBetween; //\linebetcol	Line between columns.
		...
		public: class CollumnVar
				{
					...
					public: int m_nColumnWidth;  //\colwN 	Width of column in twips; used to override the default constant width setting for variable-width columns.
					public: CollumnVar()
						{
			...
			public: CAtlArray< CollumnVar > m_aCollumnProperty;
			public: ColumnProperty()
					{
					}
			public: ColumnProperty( const ColumnProperty& oColumnProperty )
					{
						(*this) = oColumnProperty;
					}
			public: ColumnProperty& operator=( const ColumnProperty& oColumnProperty )
					{
						m_aCollumnProperty.RemoveAll();
						m_aCollumnProperty.Append( oColumnProperty.m_aCollumnProperty );
						return (*this);
					}
			public: void SetDefault()
					{
...
public: ColumnProperty m_oCollumnProperty;
...
public: FootnotesJust m_eFootnotesJust;
public: int m_nFootnotesStart; //\sftnstartN	Beginning footnote number (default is 1).
public: typedef enum {
			fr_none,
...
public: FootnotesRestart m_eFootnotesRestart;
public: typedef enum {
			ff_none,
...
public: FootnotesFormat m_eFootnotesFormat;

public: int m_nEndnotesStart; //\saftnstartN	Beginning endnote number (default is 1).
public: typedef enum {
			er_none,
...
public: EndnotesRestart m_eEndnotesRestart;
public: typedef enum {
			ef_none,
...
public: EndnotesFormat m_eEndnotesFormat;

//Line Numbering
public: int m_nLineModulus; //\linemodN	Line-number modulus amount to increase each line number (default is 1).
public: int m_nLineX; //\linexN	Distance from the line number to the left text margin in twips (default is 360).The automatic distance is 0. 
public: int m_nLineStart; //\linestartsN	Beginning line number (default is 1).
public: typedef enum {
			lnr_none,
...
public: LineNumberRestart m_eLineNumberRestart; 


//Page Information
public: int m_nPageWidth;//\pgwsxnN	N is the page width in twips. A \sectd resets the value to that specified by \paperwN in the document properties.
public: int m_nPageHeight;//\pghsxnN	N is the page height in twips. A \sectd resets the value to that specified by \paperhN in the document properties.
public: int m_nMarginLeft;//\marglsxnN	N is the left margin of the page in twips. A \sectd resets the value to that specified by \marglN in the document properties.
public: int m_nMarginRight;//\margrsxnN	N is the right margin of the page in twips. A \sectd resets the value to that specified by \margrN in the document properties.
public: int m_nMarginTop;//\margtsxnN	N is the top margin of the page in twips. A \sectd resets the value to that specified by \margtN in the document properties.
public: int m_nMarginBottom;//\margbsxnN	N is the bottom margin of the page in twips. A \sectd resets the value to that specified by \margbN in the document properties.
public: int m_nGutterMarginWidth;//\guttersxnN	N is the width of the gutter margin for the section in twips. A \sectd resets the value to that specified by \gutterN from the document properties. If Facing Pages is turned off, the gutter is added to the left margin of all pages. If Facing Pages is turned on, the gutter is added to the left side of odd-numbered pages and the right side of even-numbered pages.
public: int m_bSwitchMargin;//\margmirsxn	Switches margin definitions on left and right pages. Used in conjunction with \facingp.
public: int m_bLandscapeFormat;//\lndscpsxn	Page orientation is in landscape format. To mix portrait and landscape sections within a document, the \landscape control should not be used so that the default for a section is portrait, which may be overridden by the \lndscpsxn control.
public: int m_bTitlePage;//\titlepg	First page has a special format.
public: int m_nHeaderTop;//\headeryN	Header is N twips from the top of the page (default is 720).
public: int m_nFotterBottom;//\footeryN	Footer is N twips from the bottom of the page (default is 720).

//Page Numbers
public: int m_nPageNumberStart;//\pgnstartsN	Beginning page number (default is 1).
public: int m_bPageNumberContinuos;//\pgncont	Continuous page numbering (the default).
public: int m_bPageNumberRestart;//\pgnrestart	Page numbers restart at \pgnstarts value.
public: int m_nPageNumberX;//\pgnxN	Page number is N twips from the right margin (default is 720). This control word is understood but not used by current versions (6.0 or later) of Word.
public: int m_nPageNumberY;//\pgnyN	Page number is N twips from the top margin (default is 720). This control word is understood but not used by current versions (6.0 or later) of Word.
public: typedef enum {
	pnf_none,
...
public: PageNumberFormat m_ePageNumberFormat;
...
public: VerticalAlignment m_eVerticalAlignment;

//Text Flow
public: int m_nTextFollow;//\stextflowN	Section property for specifying text flow:0	Text flows left to right and top to bottom,1	Text flows top to bottom and right to left, vertical,2	Text flows left to right and bottom to top,3	Text flows right to left and top to bottom,4	Text flows left to right and top to bottom, vertical,5	Text flows top to bottom and left to right, vertical (for Mongolian)
...
public: int m_nBorderArt;//\brdrartN	Page border art; the N argument is a value from 1 through 165 representing the number of the border.
public: int m_nBorderMeasure ;//\pgbrdroptN	8	Page border measure from text. Always display in front option is set to off.,32	Page border measure from edge of page. Always display in front option is set to on.,40	Page border measure from edge of page. Always display in front option is set to off.
public: int m_nBorderAlign ;//\pgbrdrsnap	Align paragraph borders and table edges with page border.

//footer - header
public: TextItemContainerPtr m_oHeaderLeft;
public: TextItemContainerPtr m_oHeaderFirst;
public: TextItemContainerPtr m_oHeaderRight;
public: TextItemContainerPtr m_oFooterLeft;
public: TextItemContainerPtr m_oFooterFirst;
public: TextItemContainerPtr m_oFooterRight;

public: CString m_sIDHeaderLeft; //äëÿ OOX
public: CString m_sIDHeaderFirst;
public: CString m_sIDHeaderRight;
public: CString m_sIDFooterLeft;
public: CString m_sIDFooterFirst;
public: CString m_sIDFooterRight;

public: RtfSectionProperty()
		{
			SetDefault();
...
public: void SetDefault()
		{
			...
			m_oCollumnProperty.m_aCollumnProperty.SetCount( m_nColumnNumber );
			...
			DEFAULT_PROPERTY_DEF( m_nFotterBottom, 720 )
			...
			//footer - header íàäî îáíóëÿòü âðó÷íóþ
			//m_oHeaderLeft = TextItemContainerPtr();
			//m_oHeaderFirst = TextItemContainerPtr();
			//m_oHeaderRight = TextItemContainerPtr();
			//m_oFooterLeft = TextItemContainerPtr();
