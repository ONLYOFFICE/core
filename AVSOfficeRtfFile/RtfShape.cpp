				...
				if( st_inline == m_eShapeType )
				{
					if( NULL != m_oPicture )
					{
						...
						else
						{
							...
							//	sResult.AppendFormat( _T("{\\*\\picprop %s}"), sPictProp );
				...
				if( false && RENDER_TO_OOX_PARAM_SHAPE_WSHAPE != oRenderParameter.nType && NULL != m_oPicture ) //картинки пишем как drawing
				{
					if( st_inline == m_eShapeType )
					{
						if( PROP_DEF != m_oPicture->m_nWidthGoal && PROP_DEF != m_oPicture->m_nHeightGoal )
						{
							...
							sResult.AppendFormat( _T("<pic:blipFill><a:blip r:embed=\"%s\"/>"), m_oPicture->RenderToOOX(oRenderParameter) );
							CString sCrop;
							...
							if( false == sCrop.IsEmpty() )
								sResult.AppendFormat( _T("<a:srcRect %s/>"), sCrop );
	...
	if( false == IsValid() )
		return _T("");
			...
			if( _T("") == oRenderParameter.sValue )
				...
				sResult.AppendFormat( _T("<%s"),oRenderParameter.sValue );
			...
			if( false == sStyle.IsEmpty() )
			{
				...
				sResult.AppendFormat( _T(" style=\"%s\""),sStyle );
			}
			...
			sResult.AppendFormat( _T(" adj=\"%s\""),sAdjust );
			}
			//Geometry
			if( m_aWrapPoints.GetCount() > 0 )
			{
				...
				for( int i = 1; i < (int)m_aWrapPoints.GetCount(); i++ )
			...
			if( false == sStroke.IsEmpty() )
				sResult.AppendFormat( _T("<v:stroke %s/>"), sStroke );
			...
			if( m_oPicture != 0 )
			{
				...
				if( _T("") == sPicture )//если не сохранилась картинка, то весь shape будет бесполезным
					return _T("");
				sResult.AppendFormat( _T("<v:imagedata r:id=\"%s\""), sPicture );
				if( PROP_DEF != nCropLeft )
