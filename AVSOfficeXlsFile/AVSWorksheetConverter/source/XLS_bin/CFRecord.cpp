...
{
	file_ptr = static_cast<DWORD>(stream->getStreamPointer()); // Assume that files have size < 4Gb
	*stream >> type_id_;
	WORD size_short;
	*stream >> size_short;
	size_ = size_short;
	...
	ULONGLONG rec_data_pos = stream->getStreamPointer();
	...
	if(global_info->decryptor && 0 != size_)
	{
		switch (type_id_) // this would decrease number of checks
		{
			...
			case rt_RRDHead:
				break;
			case rt_BoundSheet8:
				global_info->decryptor->Decrypt(data_ + sizeof(DWORD), size_ - sizeof(DWORD), rec_data_pos + sizeof(DWORD));
				break;
			default:
...
void CFRecord::save(CFStreamPtr stream)
{
	file_ptr = static_cast<DWORD>(stream->getStreamPointer()); // Assume that files have size < 4Gb
	*stream << type_id_;
	WORD size_short = static_cast<WORD>(size_);
	*stream << size_short;
	if(data_)
	{
...
const DWORD CFRecord::getStreamPointer() const
{
	if(-1 == file_ptr)
	{
...
// Checks whether the specified number of bytes present in the non-read part of the buffer
...
// Checks whether the specified number of bytes fits in max size of the buffer
// Doesn't generate an exception
...
// Skip the specified number of bytes without reading
void CFRecord::skipNBytes(const size_t n)
{
	ASSERT(data_); // This throws if we use skipNBytes instead of reserveNBytes
	checkFitRead(n);
	rdPtr += n;
}
...
// Skip the specified number of bytes filled them in with zeros
void CFRecord::reserveNBytes(const size_t n)
{
	reserveNBytes(n, static_cast<DWORD>(0));
}
...
CFRecord& CFRecord::operator>>(_bstr_t& val)
{
	throw EXCEPT::LE::WrongAPIUsage("This function may only be called by mistake.", __FUNCTION__);
}
...
// Registers data receiver place and reserves n bytes for it
void CFRecord::registerDelayedDataReceiver(CFStream::DELAYED_DATA_SAVER fn, const size_t n, const CFRecordType::TypeId receiver_id)
{
	...
	item.fn = fn;
	item.data_place = size_ + sizeof(WORD)/*size_short*/ + sizeof(CFRecordType::TypeId); // set offset relative to record beginning. 
	item.data_size = n;
...
// Registers delayed source data as any user-defined DWORD
void CFRecord::registerDelayedDataSource(const DWORD data,  const CFRecordType::TypeId receiver_id)
{
	ASSERT(!data_); // This throws if used after Commit or while reading of binary
...
void CFRecord::registerDelayedFilePointerAndOffsetSource(const DWORD offset,  const CFRecordType::TypeId receiver_id)
{
	ASSERT(!data_); // This throws if used after Commit or while reading of binary
	CFStream::SourceItem item;
	item.data = offset;
