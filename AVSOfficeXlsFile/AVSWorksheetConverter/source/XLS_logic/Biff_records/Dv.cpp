...
void Dv::readFields(CFRecord& record)
{
	DWORD flags;
	record >> flags;
	valType = static_cast<BYTE>(GETBITS(flags, 0, 3));
	errStyle = static_cast<BYTE>(GETBITS(flags, 4, 6));
	fStrLookup = GETBIT(flags, 7);
	fAllowBlank = GETBIT(flags, 8);
	fSuppressCombo = GETBIT(flags, 9);
	mdImeMode = static_cast<BYTE>(GETBITS(flags, 10, 17));
	fShowInputMsg = GETBIT(flags, 18);
	fShowErrorMsg = GETBIT(flags, 19);
	typOperator = static_cast<BYTE>(GETBITS(flags, 20, 23));
