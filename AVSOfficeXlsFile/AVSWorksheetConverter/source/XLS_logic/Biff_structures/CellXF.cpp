...
void CellXF::load(CFRecord& record)
{
	...
	alc = static_cast<BYTE>(GETBITS(flags1, 0, 2));
	fWrap = GETBIT(flags1, 3);
	alcV = static_cast<BYTE>(GETBITS(flags1, 4, 6));
	fJustLast = GETBIT(flags1, 7);
	trot = static_cast<BYTE>(GETBITS(flags1, 8, 15));
	cIndent = static_cast<BYTE>(GETBITS(flags1, 16, 19));
	fShrinkToFit = GETBIT(flags1, 20);
	iReadOrder = static_cast<BYTE>(GETBITS(flags1, 22, 23));
	...
	border.dgLeft = static_cast<BYTE>(GETBITS(flags2, 0, 3));
	border.dgRight = static_cast<BYTE>(GETBITS(flags2, 4, 7));
	border.dgTop = static_cast<BYTE>(GETBITS(flags2, 8, 11));
	border.dgBottom = static_cast<BYTE>(GETBITS(flags2, 12, 15));
	border.dgDiag = static_cast<BYTE>(GETBITS(flags3, 21, 24));

	border.icvLeft = 0 != border.dgLeft ? static_cast<BYTE>(GETBITS(flags2, 16, 22)) : 0;
	border.icvRight = 0 != border.dgRight ? static_cast<BYTE>(GETBITS(flags2, 23, 29)) : 0;
	border.icvTop = 0 != border.dgTop ? static_cast<BYTE>(GETBITS(flags3, 0, 6)) : 0;
	border.icvBottom = 0 != border.dgBottom ? static_cast<BYTE>(GETBITS(flags3, 7, 13)) : 0;
	border.icvDiag = 0 != border.dgDiag ? static_cast<BYTE>(GETBITS(flags3, 14, 20)) : 0;
	
	border.grbitDiag = static_cast<BYTE>(GETBITS(flags2, 30, 31));

	fHasXFExt = GETBIT(flags3, 25);
	fill.fls = static_cast<BYTE>(GETBITS(flags3, 26, 31));
