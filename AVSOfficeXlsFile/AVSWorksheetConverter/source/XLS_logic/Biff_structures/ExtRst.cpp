...
void ExtRst::load(CFRecord& record)
{
	...
	record >> phs >> rphssub;
	for(WORD i = 0; i < rphssub.getRunsNumber(); ++i)
	{
		PhRuns run;
		record >> run;
	...
	{
		record.skipNBytes(cb - (data_end - data_start)); // trash for unknown reason
	}
...
{
	return 2/*reserved*/ + sizeof(WORD) /*cb*/ + sizeof(WORD) * 2/*phs*/ + rphssub.getSize() + (rgphruns.size() ? rgphruns.size() * sizeof(short) * 3 : 0);
}
