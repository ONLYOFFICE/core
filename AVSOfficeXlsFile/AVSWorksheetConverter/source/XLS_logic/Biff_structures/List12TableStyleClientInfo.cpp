...
{	
	...
	fFirstColumn = static_cast<BYTE>(GETBITS(flags, 0, 1));
	fLastColumn = static_cast<BYTE>(GETBITS(flags, 1, 2));
	fRowStripes = static_cast<BYTE>(GETBITS(flags, 3, 4));
	fColumnStripes = static_cast<BYTE>(GETBITS(flags, 5, 6));
	fDefaultStyle  = static_cast<BYTE>(GETBITS(flags, 9, 10));
