...
{
	red = static_cast<BYTE>(GETBITS(raw_data, 0, 7));
	green = static_cast<BYTE>(GETBITS(raw_data, 8, 15));
	blue = static_cast<BYTE>(GETBITS(raw_data, 16, 23));
...
void OfficeArtCOLORREF::setXMLAttributes(MSXML2::IXMLDOMElementPtr own_tag)
{
	...
	{
		own_tag->setAttribute(L"palette_index", STR::int2str((static_cast<WORD>(green) << 8) + red).c_str());
	}
	else if(fSysIndex)
	{
		own_tag->setAttribute(L"system_index", STR::int2str((static_cast<WORD>(green) << 8) + red).c_str());
	}
