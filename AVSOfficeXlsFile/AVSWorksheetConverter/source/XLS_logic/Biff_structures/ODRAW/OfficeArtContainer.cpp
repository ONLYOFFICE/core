...
OfficeArtContainer::OfficeArtContainer(const BYTE recVer, const WORD recType, const OfficeArtClientAnchorType anchor_type)
	...
	while(record.getRdPtr() < container_beginning_ptr + rh_own.recLen)
	{
		...
		if(art_record)
		{
			...
			if(record.getRdPtr() != child_beginning_ptr + rh_child.recLen)
			{
				if(record.getRdPtr() < child_beginning_ptr + rh_child.recLen)
				{
					Log::warning(STR::int2str(child_beginning_ptr + rh_child.recLen - record.getRdPtr(), 10) + 
						" bytes were not processed while reading from OfficeArt record of type 0x" + 
						STR::int2hex_str(rh_child.recType, sizeof(rh_child.recType)));
				}
				else
				{
					throw EXCEPT::RT::WrongBiffRecord("Wrong data parsed in OfficeArt record of type 0x" + 
						STR::int2hex_str(rh_child.recType, sizeof(rh_child.recType)), record.getTypeString());
				}
			}
		}
		else // If the found record is not implemented or unknown
		{
			...
			record.skipNBytes(rh_child.size()); // skip header
			record.skipNBytes(rh_child.recLen); // skip record data
			Log::warning("Unsupported OfficeArtRecord skipped (recType=0x" + 
				STR::int2hex_str(rh_child.recType, sizeof(rh_child.recType)) + ")");
		}
	}
