...
{
	xml_tag->setAttribute(L"opid", STR::int2hex_str(opid, sizeof(opid)).c_str());
	xml_tag->setAttribute(L"fBid", fBid);
	xml_tag->setAttribute(L"fComplex", fComplex);
	xml_tag->setAttribute(L"op", STR::int2hex_str(op).c_str());
}
...
{
	const WORD* op = record.getCurData<WORD>();
	WORD opid = GETBITS(*op, 0, 13);
...
{
	record.skipNBytes(op); // default is to skip complex data
};
