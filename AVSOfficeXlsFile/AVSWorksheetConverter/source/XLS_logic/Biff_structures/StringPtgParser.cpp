...
const bool StringPtgParser::parseToPtgs(const std::wstring& assembled_formula, Rgce& rgce, RgbExtra& rgb, MSXML2::IXMLDOMElementPtr parent)
{
	PtgPtr last_ptg;
	...
	std::wstring tag_name = static_cast<wchar_t*>(parent->GetnodeName());
	for(std::wstring::const_iterator it = assembled_formula.begin(), itEnd = assembled_formula.end(); it != itEnd;)
	{
		...
		else if(SyntaxPtg::extract_RightParenthesis(it, itEnd))
		{
			...
			else if(ptg_stack.size() && (func_var = boost::dynamic_pointer_cast<PtgFuncVar>(ptg_stack.top())))
			{
				...
				func_var->setParamsNum(static_cast<BYTE>(num_parameters));
				last_ptg = ptg_stack.top();
