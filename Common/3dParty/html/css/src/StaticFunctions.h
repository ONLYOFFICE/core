...
namespace NSCSS
{
    namespace NS_STATIC_FUNCTIONS
    {
        ...
        inline std::vector<std::vector<CNode>> GetEnumeration(const std::vector<CNode>& arNodes)
        {
            if (arNodes.empty() || arNodes[0].Empty())
                return std::vector<std::vector<CNode>>();
            ...
            switch (arNodes.size())
            {
                case 1:
                    return {{arNodes[0]}};
                case 2:
                    return {{arNodes[0]}, {arNodes[1]}, {arNodes[0], arNodes[1]}};
                case 3:
                    return {{arNodes[0]}, {arNodes[1]}, {arNodes[2]}, {arNodes[0], arNodes[1]}, {arNodes[0], arNodes[2]}, {arNodes[1], arNodes[2]}, {arNodes[0], arNodes[1], arNodes[2]}};
                default:
                {
                    std::vector<std::vector<CNode>> arTempValues = GetEnumeration(std::vector<CNode>(&arNodes[1], &arNodes[arNodes.size() - 1]));
                    std::vector<std::vector<CNode>> arValues;

                    for (std::vector<CNode>& arItem : arTempValues)
                    {
                        std::vector<CNode> arTemp({arNodes[0]});
                        arTemp.insert(arTemp.end(), arItem.begin(), arItem.end());
                        arValues.push_back(arTemp);
                    }
        ...
        inline CNode NormalizationCNode(const CNode& oNode)
        {
            CNode oTempNode;
            size_t posSpace = oNode.m_sName.find(L' ');
            if (posSpace != std::wstring::npos)
                oTempNode.m_sName = oNode.m_sName.substr(0, posSpace);
            posSpace = oNode.m_sClass.find(L' ');
            if (posSpace != std::wstring::npos)
                oTempNode.m_sClass = oNode.m_sClass.substr(0, posSpace);
            posSpace = oNode.m_sId.find(L' ');
            if (posSpace != std::wstring::npos)
                oTempNode.m_sId = oNode.m_sId.substr(0, posSpace);
            oTempNode.m_sStyle = oNode.m_sStyle;
            return oTempNode;
        }

        inline std::vector<CNode> NormalizationCNodes(const std::vector<CNode>& arNodes)
        {
            std::vector<CNode> arValues;

            for (const CNode& oNode : arNodes)
                arValues.push_back(NormalizationCNode(oNode));

            return arValues;
        }

        inline std::vector<std::string> GetSelectorsList(const std::vector<CNode>& arNodes)
        {
            if (arNodes.empty())
            ...
            switch (arNodes.size())
            {
                case 1:
                {
                    ...
                    sValues.push_back(wstringToString(arNodes[0].m_sName));
                    const bool& bIsThereClass = !arNodes[0].m_sClass.empty();
                    const bool& bIsThereId = !arNodes[0].m_sId.empty();

                    if (bIsThereClass)
                        sValues.push_back(wstringToString(arNodes[0].m_sClass));
                    if (bIsThereId)
                        sValues.push_back(wstringToString(arNodes[0].m_sId));
                    if (bIsThereClass)
                        sValues.push_back(wstringToString(arNodes[0].m_sName + L'.' + arNodes[0].m_sClass));
                    if (bIsThereId)
                        sValues.push_back(wstringToString(arNodes[0].m_sName + L'#' + arNodes[0].m_sId));
                    if (bIsThereClass && bIsThereId)
                    {
                        sValues.push_back(wstringToString(arNodes[0].m_sName + L'.' + arNodes[0].m_sClass + L'#' + arNodes[0].m_sId));
                        sValues.push_back(wstringToString(arNodes[0].m_sName + L'.' + arNodes[0].m_sClass + L'#' + arNodes[0].m_sId));
                    }
                    return sValues;
                }
