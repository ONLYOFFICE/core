...
{
#if defined(_WIN32) || defined (_WIN64)
            return std::wstring ((wchar_t *) pStr);
#else
    ...
    const UTF16 *pStrUtf16_Conv = pStr;
    UTF32 *pStrUtf32_Conv = pStrUtf32;

    ConversionResult eUnicodeConversionResult = ConvertUTF16toUTF32 (&pStrUtf16_Conv
                                        , &pStrUtf16_Conv[nLength]
                                        , &pStrUtf32_Conv
                                        , &pStrUtf32 [nLength]
                                        , strictConversion);
...
static bool stringWStringToUtf16 (const std::wstring& aSrc, std::vector<UTF16> &result)
{
#if defined(_WIN32) || defined (_WIN64)
    ...
    uint32_t nLength = aSrc.length();
    result.resize(nLength + 1, 0);
    UTF16* pStrUtf16 = &result[0];
...
{
        unsigned char* pBuffer = (unsigned char*)aSrc.c_str();
        long lCount = aSrc.length();

	if (sizeof(wchar_t) == 4)
	{
                wchar_t* pUnicodeString = new wchar_t[lCount + 1];
                long lIndexUnicode = 0;

                long lIndex = 0;
		while (lIndex < lCount)
		{
                        unsigned char byteMain = pBuffer[lIndex];
			if (0x00 == (byteMain & 0x80))
			{
				// 1 byte
                                pUnicodeString[lIndexUnicode++] = (wchar_t)byteMain;
				++lIndex;
			}
			else if (0x00 == (byteMain & 0x20))
			{
				...
				int val = (int)(((byteMain & 0x1F) << 6) |
					(pBuffer[lIndex + 1] & 0x3F));
                                pUnicodeString[lIndexUnicode++] = (wchar_t)(val);
				lIndex += 2;
			}
        ...
        else
        {
                ...
                while (lIndex < lCount)
                {
                        ...
                        {
                                // 1 byte
                                *pUnicodeString++ = (wchar_t)byteMain;
                                ++lIndex;
                        }
                        else if (0x00 == (byteMain & 0x20))
                        {
...
public:
    ...
    // They also are compiled if you have explicitly turned off refcounting
        ...
        MYTYPE& assign(const MYTYPE& str, MYSIZE nStart, MYSIZE nChars)
        {
            ...
            // value.  Thanks to Ullrich Pollдhne for catching this bug
...
//		}
    ...
    void Fmt(const CT* szFmt, ...)
    {
        ...
        *this = str;
        ...
        do
        {
            ...
            va_start(argList, szFmt);
            res = ssvsprintf(str.GetBuffer(nLen), nLen-1, szFmt, argList);
            va_end(argList);
    ...
    void FormatV(const CT* szFormat, va_list argList)
    {
        ...
        int nLen  = sslen(szFormat), res = 0;

        nLen +=  STD_BUF_SIZE;
        str.clear();

        res = ssvsprintf(str.GetBuffer(nLen), nLen-1, szFormat, argList);
