...
namespace XLSB
{
    ...
    BaseObjectPtr PCDH14::clone()
    {
        return BaseObjectPtr(new PCDH14(*this));
    }

    void PCDH14::readFields(XLS::CFRecord& record)
    {
        BYTE    flags;
        _INT32  value;
        ...
        fMeasureSet             = GETBIT(flags, 1);
        fHierarchizeDistinct    = GETBIT(flags, 2);
        fIgnorable              = GETBIT(flags, 3);

        record >> cihdb;

        for(size_t i = 0; i < cihdb; ++i)
        {
            record >> value;
