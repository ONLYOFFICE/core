...
namespace Geometry
{
	...
	// выдает угол при вершине a (от 0 до 180)
	...
	// выдает угол при вершине a (между горизонтальной прямой и отрезком)
	...
	// считает периметр эллипса с заданными радиусами
	double GetEllipseLength(double dEllipseRadiusX, double dEllipseRadiusY)
	{
		// очень приблеженная формула периметра эллипса: pi*(a+b)
		// double dEllipseLength = 3.14159265359*(dLetterRadiusX + dLetterRadiusY);

		// Если выбирать из приближенных формул, наиболее точных для любых эксцентриситетов, 
		// то наиболее точной, вероятно, будет формула Кантрела (David Cantrell) 
	...
	/************************************************************************/
	/* Быстрое определние внутри региона точка иил нет    
	   true - внутри
	/************************************************************************/
	...
	// класс, позволяющий быстро производить вычисление координат у повернутых точек
	...
	public:
		...
		double GetSin(double dAngle)
		{
			...
			// TODO: надо сюда добавить специальные "быстрые" случаи
	...
	// класс, позволяющий производить различные вычисления с polyline'ом
	class CPolylineManager
	{
		double* m_pPoints; // координаты точек (пары double'ов)
		double* m_pLengths; // частичные суммы длин (pLength[index] - длина от начала polyline, до точки с индексом Index)
