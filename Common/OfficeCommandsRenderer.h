...
public:
	...
	STDMETHOD(PathCommandArcTo)(float fX, float fY, float fWidth, float fHeight, float fStartAngle, float fSweepAngle)
	{
		...
		// теперь угол - 0..360 - по часовой стрелке
		double dA = fWidth / 2;
		double dB = fHeight / 2;
...
// чтобы нарисовать Shape по VML'ой XML, нужно:
//#include "Shape.h" // <-- находится в OfficeDrawing
//CShape oShape;
//CMetricInfo oMetric;
//oMetric.m_lMillimetresHor = PageWidth;
//oMetric.m_lMillimetresVer = PageHeight;
//oShape.LoadFromXML(xml);
//CString strXml = oShape.ToXml(lLeft, lTop, lWidth, lHeight, oMetric, 0.0, 1.0);

//теперь по этой хмл создать DrawGraphicPath и им нарисовать