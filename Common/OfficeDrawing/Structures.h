...
// по идее никакие пол€ заполн€ть не надо. Ёто все дл€ теста.
// потом можно закоментарить - и просто заполн€ть CTextAttributes...
// при этом добавить флаг - пропустить ли настройки
// (у нас все будет выставл€тьс€ по первой букве) - вот допишем эффект в имадж студию - тогда 
// все настройки и подхватим
...
struct STextPFRun
{
	...
	CString ToString()
	{
		...
		if ((hasBullet_ || bulletHasFont_ || bulletHasColor_ || bulletHasSize_)/* && (0x0F == bulletFlag)*/)
			CDirectory::WriteValueToNode(_T("bulletflag"), (DWORD)bulletFlag, &oWriter);

        if (bulletChar_)
		{
			//CStringW str(bulletChar);
			char pData[2] = { 'Х', 0 };
			CStringW str(pData);
		...
		if (indent_/* || bIsExt*/)
			CDirectory::WriteValueToNode(_T("indent"), (LONG)(indent/* + lIndentLevel*/), &oWriter);
...
public:
	// метод, который проставл€ет все настройки из схем контейнера
