...
namespace Painter
{
	...
	// если какие-то градиентные константы меняются - надо сделать изменения в функции IsBrushTypeGradient()

	// constants for brush texture mode
	const int c_BrushTextureModeStretch =	 0;
	const int c_BrushTextureModeTile =		 1;
	const int c_BrushTextureModeTileCenter = 2;

	// constants for format alpha mode
	...
	// определяем, является ли указанный nType - типом gradient brush'а или нет
	...
	// Draw blur string for shadow and edge text paint
	...
	public:
		...
		BOOL IsEqual(CBrush* pBrush)
		{
			...
			/*return ((Type == pBrush->Type) && 
				...
				(Rectable == pBrush->Rectable) && (Rect == pBrush->Rect));*/
		}
	...
	public:
		...
		Gdiplus::Brush* GetBrush(CTextureManager* manager = 0, RectF* pRect = 0, int index = 0, BOOL bAddIfNotExists = FALSE)
		{
			...
			// create gradient brush from angle
			...
			else if (nBrushType >= c_BrushTypeHorizontal && nBrushType <= c_BrushTypeDiagonal2 && pRect)
			{
				...
				Gdiplus::LinearGradientBrush* brush = GetLinearGradientBrushAngle(90 - angle, pRect);//GetLinearGradientBrushHVD(nBrushType - c_BrushTypeHorizontal, pRect);
	...
	private:
		...
		Gdiplus::TextureBrush* GetTexureBrush( Painter::CTexture* pTexture, Gdiplus::RectF* pRect)
		{
			...
			if( !pTextureBitmap )
			{
				...
				// делаем композицию с альфой (она до 5 раз работает быстрее, чем ImageAttribute)
				if( nAlpha < 255 )
				{
					...
					if( pBuffer )
					{
						...
						//if( ippStsNoErr == ippiAlphaPremul_8u_AC4R( pBGRA, nStride, pBuffer, nWidth*4, ImageSize ) )
						//{
						//	IppiSize TempSize = {1, nWidth*nHeight};
						//	ippiSet_8u_C1R( BYTE(nAlpha), pBuffer + 3, 4, TempSize );
						//	
						//	pBGRA = pBuffer;
						//	nStride = nWidth*4;
						//}
	...
	public:
		...
		void Draw(Graphics* Gr, Gdiplus::Font* font, StringFormat* format, BSTR Text, int len, RectF& rectDraw)
		{
			...
			if (Visible == -1)
			{
				Gdiplus::Color color((BYTE)(Alpha), GetRValue(Color), GetGValue(Color), GetBValue(Color));
				SolidBrush solid(color);
//				Brush* pBrush = GetBrush();		// old version
				RectF temp = rectDraw;
				int offset = int(Dist);
	...
	// transforms (should be removed from this section)
