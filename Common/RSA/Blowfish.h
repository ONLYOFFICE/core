...
public:
	...
	{
		DWORD dwTemp = dwInputLength % 8;	// find out if uneven number of bytes at the end
		if (0!=dwTemp)
	...
	DWORD Encrypt(LPBYTE pSrcBuffer, LPBYTE pDstBuffer, DWORD dwSize)
	{
		...
		if (bSameDest)	// if encoded data is being written into input buffer
		{
			for (DWORD dwCount = 0 ; dwCount < dwOutSize ; dwCount += 8)
			{
		 		if (dwCount >= dwSize - 7)	// if not dealing with uneven bytes at end
				{
					LPBYTE pTemp = pSrcBuffer + dwSize ;	// point at byte past the end of actual data
		...
		{               // input to output buffer prior to encrypting
			for (DWORD dwCount = 0 ; dwCount < dwOutSize ; dwCount += 8)
			{
		 		if (dwCount < dwSize - 7)	// if not dealing with uneven bytes at end
				{
		 		...
		 		{
		 			DWORD dwReminder = dwOutSize - dwSize ;	// number of remaining data bytes
