...
namespace VLong
{
	...
	class CFlexUnit // Provides storage allocation and index checking
	{
	protected:
		//буфер
		TUnit* m_pData;
	...
	public:
		...
		//
		#define BIT_PER_UNIT			(8*sizeof(TUnit))						// Number of bits in an unsigned
		#define Lo(x)					((x) & ((1<<(BIT_PER_UNIT/2))-1))	// lower half of unsigned
		#define Hi(x)					((x) >> (BIT_PER_UNIT/2))			// upper half
		#define LH(x)					((x) << (BIT_PER_UNIT/2))			// make upper half
		inline void MulFast(const CFlexUnit<TUnit> &nVal1, const CFlexUnit<TUnit> &nVal2, size_t nLimitBit)
		{
			// *this = (x*y) % (2**keep)			
			size_t nLimit = (nLimitBit + BIT_PER_UNIT - 1)/BIT_PER_UNIT; // size of result in words
			Reserve(nLimit); 
	...
	public:
		...
		void SHL()
		{
			//BOOL bCarry = 0;
			//BOOL bCarryNext = 0;
			//TUnit* ptrThisData = m_pData;
			//for (size_t i = 0; i<=nSize; i++, ptrThisData++)
			//{	
			//	bCarryNext = (0!=((*ptrThisData)>>(BIT_PER_UNIT - 1)));
			//	*ptrThisData = ((*ptrThisData)<<1) + (bCarry ? 1 : 0);
			//	bCarry = bCarryNext;
			//}
			TUnit nCarry = 0;
		...
		{
			TUnit nCarry = 0;
			//здесь именно long i потому что ждем когда i станет отрицательным
		...
		{
			//Используем функцию Set так как может понадобиться расширить буффер
			size_t nDelta = nCount/BIT_PER_UNIT; 
			nCount %= BIT_PER_UNIT;
			
			size_t nOldUsedSize = m_nUsedSize;//может измениться в функции Set
			for (size_t i=0; i<nOldUsedSize; i++)
			{
		...
		long Compare(const CVLong &oVLong) const
		{
			...
			else if (m_bNegative)
			{
				if (oVLong.IsZero())
					...
					//поскольку и тот и другое не IsZero здесь NULL!=m_pValue у обоих
	...
	public:
		...
		inline size_t ToBuffer(LPBYTE pBuffer)
		{
			...
			//memcpy(pBuffer, (LPBYTE)m_pValue->GetData(), nRet);
			//переворачиваем
	...
	//
	template< typename TUnit >
	class CMontgomery // class for montgomery modular exponentiation
	{
