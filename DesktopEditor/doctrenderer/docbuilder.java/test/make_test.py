...
if __name__ == "__main__":
    ...
    if args.builder_dir:
        ...
        subprocess.call([java, '-cp', os.path.join(builder_dir, 'docbuilder.jar') + os.pathsep + 'build/classes', 'Program'], cwd=os.getcwd())
    else:
        makedirs('build/classes')
        subprocess.call([javac, '-d', 'build/classes', '-cp', '../build/libs/docbuilder.jar', java_file], cwd=os.getcwd())
