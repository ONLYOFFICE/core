...
void CFontList::Add(const std::wstring& sFontPath, CFontStream* pStream, int nFlag, bool biBold, bool biItalic)
{
	if (!pStream)
		return;
	...
	for ( int nIndexFace = 0; nIndexFace < nFacesCount; nIndexFace++ )
	{
		...
		INT bBold   = biBold   ? 1 : (pFace->style_flags & FT_STYLE_FLAG_BOLD ? 1 : 0);
		INT bItalic = biItalic ? 1 : (pFace->style_flags & FT_STYLE_FLAG_ITALIC) ? 1 : 0;
