...
struct Segment
{
	...
	Segment();
	Segment(const std::vector<PointD>& points, bool isCurve,
...
struct Curve
{
	...
	std::vector<double> GetCurveLineIntersection(double px, double py,
												double vx, double vy) const;
	
	std::vector<Curve> Subdivide(double t) const;
	Curve	DivideAtTime(double time, std::vector<Segment>& segments,
						std::vector<Curve>& curves);
	
	int	SolveCubic(size_t coord, int value, std::vector<double>& roots,
...
public:
	...
	std::vector<std::vector<int>> FindCurveBoundsCollisions(const std::vector<Curve>& curves1,
															...
															double tolerance,
															bool bothAxis = false);
	std::vector<std::vector<int>> FindBoundsCollisions(const std::vector<std::vector<double>>& bounds1,
	...
	int	 AddCurveIntersection(const Curve& curve1, const Curve& curve2, bool flip,
							  int recursion = 0, int calls = 0, double tMin = 0,
							  double tMax = 1, double uMin = 0, double uMax = 1);
