	...
	void BlendTo(CGraphicsLayer* pLayer, SrcPixelFormatRenderer& oSrc, int nBlendMode)
	{
		...
		for (unsigned int unY = 0; unY < unSrcH; ++unY)
		{
			...
			for (unsigned int unX = 0; unX < unSrcW; ++unX)
			{
				...
				if (uchAlpha)
				{
					SrcPixelFormatRenderer::blender_type::blend_pix(nBlendMode, pDstBuffer, pSrcBuffer[order_type::R], pSrcBuffer[order_type::G], pSrcBuffer[order_type::B], uchAlpha, 0);
				}
