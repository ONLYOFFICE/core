#ifndef _WMF_CODE_PAGE_950_H
#define _WMF_CODE_PAGE_950_H

//-- CP950 Encoding -----------------------------------------------------------------------------

#define MSCP950_UNDEFINED 0x0000
#define MSCP950_LEAD_CHAR 0xFFFF // Символ, который означает, что данный код - старший байт
...
struct TMSCP950_Pair
{
	unsigned short ushCode;
	unsigned short ushUnicode;
};

static const TMSCP932_Pair c_aoUnicodeMapCP950WithLeadByte[] =
{	
...
static unsigned short CP950_Find(unsigned short ushCode)
{
	TMSCP950_Pair *pPair = (TMSCP950_Pair *)c_aoUnicodeMapCP950WithLeadByte;

	while ( 0xFFFF != pPair->ushCode )
	{
		if ( ushCode == pPair->ushCode )
			return pPair->ushUnicode;

		pPair++;
	}

	return 0x0020; // Пробел
}

static std::wstring CP950_CharToUnicode(const char *sString)
{
	int nLen = (int)strlen(sString);
	wchar_t *wsString = new wchar_t[nLen + 1];
	if (NULL == wsString)
	...
	{
		unsigned char  unCode = sString[nIndex];
		unsigned short ushUnicode = c_anUnicodeMapCP950[unCode];

		if (-1 == nLeadByte)
		{
			if (MSCP950_LEAD_CHAR != ushUnicode)
			{
				wsString[nIndex] = ushUnicode;
				nLeadByte = -1;
			}
			else
			{
		...
		else
		{
			...
			wsString[nIndex] = CP950_Find(ushCode);
		}
	}

	std::wstring wsResult(wsString);
	delete[]wsString;

	return wsResult;
}

#endif /* _WMF_CODE_PAGE_950_H */