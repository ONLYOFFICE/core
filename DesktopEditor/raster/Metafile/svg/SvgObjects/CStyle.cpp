...
namespace SVG
{
	...
	void CSvgCalculator::SetData(CObjectBase &oSvgObject) const
	{
		const std::map<std::wstring, NSCSS::CElement *> *pData = m_pInternal->GetData();
		const std::vector<NSCSS::CNode> arSelectors = oSvgObject.GetFullPath();
		...
		for (size_t i = 0; i < arSelectors.size(); ++i)
		{
			...
			oSvgObject.SetData(arSelectors[i].m_mAttrs, i + 1);

			for (const NSCSS::CElement* oElement : arFindElements)
				oSvgObject.SetData(oElement->GetStyle(), i + 1);

			oSvgObject.SetData(arSelectors[i].m_sStyle, i + 1, true);
		}
	}
}
