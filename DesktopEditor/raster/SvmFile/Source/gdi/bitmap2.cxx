	...
	if( ImplReadDIBInfoHeader( rIStm, aHeader, bTopDown ) && aHeader.nWidth && aHeader.nHeight && aHeader.nBitCount )
	{
		...
		if( pAcc )
		{
			...
			if( ZCOMPRESS == aHeader.nCompression )
			{
...
#endif
				pData = (BYTE*) malloc( nUncodedSize );
				...
				#endif
				pIStm = pMemStm = rIStm.CreateInstance();
				pMemStm->SetBuffer( (char*) pData, nUncodedSize );
				nOffset = 0;
			}
			else
				pIStm = &rIStm;
...
#endif
			BYTE* pBuffer = (BYTE*) malloc( rHeader.nSizeImage );
				...
				case( 1 ):
				{
					...
					for( ; nCount--; nY += nI )
					{
						...
						for( long nX = 0L, nShift = 8L; nX < nWidth; nX++ )
						{
							...
							rAcc.SetPixel( nY, nX, static_cast<BYTE>(( cTmp >> --nShift ) & 1) );
						}
					}
				}
				break;
				...
				case( 4 ):
				{
					...
					for( ; nCount--; nY += nI )
					{
						...
						for( long nX = 0L, nShift = 2L; nX < nWidth; nX++ )
						{
							...
							rAcc.SetPixel( nY, nX, static_cast<BYTE>(( cTmp >> ( --nShift << 2UL ) ) & 0x0f) );
						}
					}
				}
				break;
