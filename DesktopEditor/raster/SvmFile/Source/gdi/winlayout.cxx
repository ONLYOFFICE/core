...
#endif
#include <hash_map>
typedef stdext::hash_map<int,int> IntMap;
#include <set>
...
public:
    ...
    virtual int     GetNextGlyphs( int nLen, sal_GlyphId* pGlyphs, SvmPoint& rPos, int&,
...
#endif
    LOGFONT aLogFont;
    ::GetObject( mhFont, sizeof(LOGFONTA), &aLogFont);
    aLogFont.lfHeight = (LONG)(mfFontScale * aLogFont.lfHeight);
    aLogFont.lfWidth  = (LONG)(mfFontScale * aLogFont.lfWidth);
    hHugeFont = ::CreateFontIndirect( &aLogFont);
    ...
    if( (mnGlyphCount != mnCharCount) || bVertical )
    {
        ...
        while( rArgs.GetNextRun( &i, &j, &bIsRTL ) )
        {
            do
            {
                ...
                if( bIsRTL )
                {
					cChar = SVMCore::GetMirroredChar( cChar );
                ...
                if( bVertical )
                {
					sal_UCS4 cVert = SVMCore::GetVerticalChar( cChar );
                    if( cVert )
                        cChar = cVert;
                }
...
int SimpleWinLayout::GetNextGlyphs( int nLen, sal_GlyphId* pGlyphs, SvmPoint& rPos, int& nStart,
    long* pGlyphAdvances, int* pCharIndexes ) const
{
    ...
    SvmPoint aRelativePos( nXOffset, 0 );
    ...
    while( nCount < nLen )
    {
        ...
        if( mbDisableGlyphs )
        {
            if( mnLayoutFlags & SAL_LAYOUT_VERTICAL )
            {
                ...
                {
#endif
					nGlyphIndex |= SVMCore::GetVerticalFlags( cChar );
...
void SimpleWinLayout::DrawText( SalGraphics& rGraphics ) const
{
    ...
    SvmPoint aPos = GetDrawPosition( SvmPoint( mnBaseAdv, 0 ) );

     // #108267#, limit the number of glyphs to avoid paint errors
	UINT limitedGlyphCount = SVMCore::Min( 8192, mnGlyphCount );
#ifdef AVS
    ...
    else
    {
        ...
        else
        {
            ...
            for( unsigned int i = 0; i < limitedGlyphCount; ++i )
            {
                ...
                aPos = GetDrawPosition( SvmPoint( nXPos, 0 ) );
            }
        }
    }
#endif
