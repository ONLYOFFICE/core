...
namespace NSDocxRenderer
{
	void CDropCap::ToXml(NSStringUtils::CStringBuilder& oWriter)
	{
		...
		oWriter.AddInt(static_cast<LONG>((m_dBaselinePos - m_dTop) * c_dMMToDx * 0.96));
		...
		//oWriter.WriteString(L"<w:position w:val=\"-6\" />");
		oWriter.WriteString(L"<w:sz w:val=\"");
		oWriter.AddInt(static_cast<LONG>((double)nFontSize * 0.95));
		oWriter.WriteString(L"\" />");
		oWriter.WriteString(L"</w:rPr>");
