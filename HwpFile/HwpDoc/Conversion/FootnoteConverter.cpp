...
std::vector<unsigned short> CFootnoteConverter::GetNotWrittenIds(bool bNeedClear)
{
	if (!bNeedClear)
		return m_arNotWrittenIds;

	std::vector<unsigned short> arNotWrittenIds{m_arNotWrittenIds};
	m_arNotWrittenIds.clear();
...
void CFootnoteConverter::CreateFootnote(const CCtrlNote* pNote)
{
	...
	m_oFootnoteXml.WriteString(L"<w:footnote w:id=\"" + std::to_wstring(++m_unCountFootnotes) + L"\">");
	m_arNotWrittenIds.push_back(m_unCountFootnotes);

	for (const CHWPPargraph* pParagraph : pNote->GetParagraphs())
	{
		m_oFootnoteXml.WriteString(L"<w:p>");
		for (const CCtrl* pCtrl : pParagraph->GetCtrls())
		{
			if (nullptr != dynamic_cast<const CCtrlAutoNumber*>(pCtrl))
				m_oFootnoteXml.WriteString(L"<w:r><w:footnoteRef/></w:r>");
			else if (nullptr != dynamic_cast<const CParaText*>(pCtrl))
			{
				...
				m_oFootnoteXml.WriteString(L"<w:r><w:t xml:space=\"preserve\">");
				m_oFootnoteXml.WriteEncodeXmlString(wsText);
				m_oFootnoteXml.WriteString(L"</w:t></w:r>");
			}
		}
		m_oFootnoteXml.WriteString(L"</w:p>");
	}

	m_oFootnoteXml.WriteString(L"</w:footnote>");
}

bool CFootnoteConverter::SaveToFile(const std::wstring& wsDirectory)
{
	if (0 == m_oFootnoteXml.GetCurSize())
		return false;
