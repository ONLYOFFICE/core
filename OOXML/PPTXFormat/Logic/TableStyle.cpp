...
namespace PPTX
{
	namespace Logic
	{		
		void TableStyle::fromXML(XmlUtils::CXmlNode& node)
		{
			m_name = node.GetName();
		...
		void TableStyle::toXmlWriter(NSBinPptxRW::CXmlWriter* pWriter) const
		{
		...
		void TableStyle::toPPTY(NSBinPptxRW::CBinaryFileWriter* pWriter) const
		{
		...
		void TableStyle::fromPPTY(NSBinPptxRW::CBinaryFileReader* pReader)
		{
		...
		void TableStyle::SetTheme(const smart_ptr<PPTX::Theme> theme)
		{
			m_Theme = theme;
			if(tblBg.IsInit())
