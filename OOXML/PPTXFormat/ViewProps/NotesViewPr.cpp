...
namespace PPTX
{
	...
	{
		void NotesViewPr::fromXML(XmlUtils::CXmlNode& node)
		{
			CSldViewPr = node.ReadNode(L"p:cSldViewPr");

			FillParentPointersForChilds();
		}
		std::wstring NotesViewPr::toXML() const
		{
		...
		void NotesViewPr::toPPTY(NSBinPptxRW::CBinaryFileWriter* pWriter) const
		{
			pWriter->WriteRecord1(0, CSldViewPr);
		}
		void NotesViewPr::fromPPTY(NSBinPptxRW::CBinaryFileReader* pReader)
		{
		...
		void NotesViewPr::toXmlWriter(NSBinPptxRW::CXmlWriter* pWriter) const
		{
