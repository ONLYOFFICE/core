...
namespace XLSB
{
    ...
    BaseObjectPtr SXEDIT::clone()
    {
        return BaseObjectPtr(new SXEDIT(*this));
    }

    //SXEDIT = BrtBeginSXEdit BrtSXTupleItems PIVOTRULE14 BrtEndSXEdit
    const bool SXEDIT::loadContent(BinProcessor& proc)
    {
        if (proc.optional<BeginSXEdit>())
        {
            m_BrtBeginSXEdit = elements_.back();
            elements_.pop_back();
        }
