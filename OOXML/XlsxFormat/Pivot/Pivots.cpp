...
//------------------------------------
	...
	XLS::BaseObjectPtr CColumnRowItem::toBin()
	{
		...
		auto ptr2(new XLSB::ISXVIS(m_arrItems.size()));
		ptr->m_ISXVIS = XLS::BaseObjectPtr{ptr2};
		auto ptr3(new XLSB::BeginISXVIs(m_arrItems.size()));
...
//------------------------------------
	...
	XLS::BaseObjectPtr CFieldItem::toBin()
	{
...
//------------------------------------
	...
	XLS::BaseObjectPtr CReference::writeAttributes()
	{
		...
		if (m_oByPosition.IsInit())
			ptr->prFilter.itmtypeAVERAGE = m_oByPosition.get();
...
//------------------------------------
	...
	XLS::BaseObjectPtr CPivotTableStyleInfo::toBin()
	{
		...
		if(m_oShowColStripes.IsInit())
			ptr->fRowStripes = m_oShowColStripes.get();
...
//------------------------------------
	...
	XLS::BaseObjectPtr CPivotCacheField::writeAttributes()
	{
		...
		if (m_oName.IsInit())
			ptr->stFldName.value() = m_oName.get();

		if (m_oCaption.IsInit())
			ptr->stFldCaption.value() = m_oCaption.get();
...
//------------------------------------
	...
	XLS::BaseObjectPtr CSharedItems::toBin()
	{
		...
		for(auto i:m_arrItems)
		{
			...
			if(valueBool)
			{
				...
				ptr->m_arSource.push_back(parsePCDI(element));
				continue;
			}
	...
	XLS::BaseObjectPtr CSharedItems::parsePCDI(XLS::BaseObjectPtr object)
	{
		if(object->get_type() == XLS::typePCDI)
		{
			auto ptr1(new XLSB::PCDI);
			ptr1->m_source = object;
			return XLS::BaseObjectPtr{ptr1};
		}
		else if(object->get_type() == XLS::typePCDIA)
		{
