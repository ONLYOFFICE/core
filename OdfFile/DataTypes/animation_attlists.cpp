...
namespace odf_types {
	...
	void anim_transition_filter_attlist::apply_from(const anim_transition_filter_attlist& Other)
	{
		...
		_CP_APPLY_PROP2(smil_target_element_);
	}
	...
	void anim_transition_filter_attlist::serialize(CP_ATTR_NODE)
	{
		...
		CP_XML_ATTR_OPT(L"smil:target-element",		smil_target_element_);
	}
	...
	void anim_set_attlist::apply_from(const anim_set_attlist& Other)
	{
		_CP_APPLY_PROP2(smil_fill_);
		_CP_APPLY_PROP2(smil_target_element_);
		_CP_APPLY_PROP2(smil_attribute_name_);
	...
	void anim_set_attlist::serialize(CP_ATTR_NODE)
	{
		CP_XML_ATTR_OPT(L"smil:fill",				smil_fill_);
		CP_XML_ATTR_OPT(L"smil:target-element",		smil_target_element_);
		CP_XML_ATTR_OPT(L"smil:attribute-name",		smil_attribute_name_);
	...
	void anim_animate_motion_attlist::serialize(CP_ATTR_NODE)
	{
		CP_XML_ATTR_OPT(L"smil:fill",				smil_fill_);
		CP_XML_ATTR_OPT(L"smil:target-element",		smil_target_element_);
		CP_XML_ATTR_OPT(L"smil:path",				svg_path_);
	}
