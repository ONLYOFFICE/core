...
for item in cmap_names:
  ...
  cmap_values[item]["code"] = "const char c_arr" + item.replace("-", "_") + "[] = \"" + "\\n".join(arr_wc_str_cmap) + "\";"
...
content_cpp_file.append("  {")
for item in cmap_names:
  content_cpp_file.append("    g_memory_cmap.insert(std::pair<std::string, TCMapData>(\"" + item + "\", { c_arr" + item.replace("-", "_") + ", (unsigned int)strlen(c_arr" + item.replace("-", "_") + ") }));")
content_cpp_file.append("  }")
